...
This matches NSW VII (7.4.1), which states that the absolute Galois group of a `p`-adic local
field of degree `N` is generated by `N+2` elements and applies at `p=2`; it is exactly the
