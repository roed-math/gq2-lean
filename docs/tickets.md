...
| P-13 | B: §5 proofs — **decomposed → P-13a–g** (parallelizable; see [`p13-ticket-split.md`](p13-ticket-split.md)) | ⭐⭐⭐ | O | P-12 | B6, B7 | ◐ (Opus 2026-07-04: P-13a done; b–g open) |
...
| P-13e | B: §5.11 dévissage (mapping-cone 2-of-3 for `IsSelfDual` along a module SES) | ⭐⭐⭐ | F | — | — | ◐ (Opus/Fable 2026-07-04) **dévissage PROVED — std-3, no sorry** (`GQ2/Devissage.lean`): `selfdualW_two_of_three` = 2-of-3 for `IsSelfDualW` (word-internal form, `#H⁰w(A^∨)` for `#fixedPts`). Full stack: nine-term LES (arrows + all exactness, both the SES and its dualization), ElemDual pack (extension/separation/biduality/dual exactness), six χ-maps with free halves, all ladder squares (lemma_5_6 + eval + δ-cores), four-lemma, nine-window assembly; plus `H0w_eq_fixedPts`/`isSelfDual_iff_W` under `t.Generates`. **Remaining — statement decision for P-13 owner:** `lemma_5_11` as stated lacks `Generates`; the splice needs `hgen` added (consumer `prop_5_15` sits in the generated setting) — see the Devissage.lean docstring. Import of `GQ2.Devissage` into `GQ2.lean` also pending (hot file). |
...
| P-15b | B: Gauss signs — Lemma 6.8 (87)/(88) + Prop 6.9 (91) ×2 | ⭐⭐⭐ | O | P-15a | ∅ | ◐ (Fable, 2026-07-04) **`GQ2/GaussSigns.lean` created (sorry-free, in `GQ2.lean`, all exactly std-3)** — the assembly layer that consumes P-15a's now-complete Lemma 6.6: **`arf_qDouble_eq_zero`** (6.8's FINAL clause from (87)+(88): `arf q = s ∧ k ≡ s ⟹ arf(q_U) = 0`, via the proved Wall sign), **`zeroCount_qDouble_of_arf_zero`** (6.9-ramified's count from `arf(q_U) = 0`: `qDouble_nonsingular` + engine; includes `IsQuadraticFp2 (qDouble …)`), **`arf_eq_one_of_dvd`** (6.9-unramified's arithmetic half: `n ∣ zeroCount−1 ∧ n ∣ #V−zeroCount ∧ n ∤ 2^m−1 ⟹ arf q = 1` — the two Arf candidates differ from the packet constraints by exactly `2^m ∓ 1`), **`central_two_pow_smul_eq_one`** (central 2-power element on nontrivial faithful simple exp-2 module is trivial, via `exists_fixed_ne_zero` — the oddness source for tame images).  NB the file must NOT import `SectionSix` (P-15c cycle lesson) — all bricks are abstract in `(U, N, G)`.  **REMAINING (the actual §6.2 cores, from the paper's proofs, extracted to notes)**: (i) 6.9-unramified needs the free-action orbit-divisibilities (`n ∣ zeroCount−1` etc., orbit-stabilizer for the `SubMulAction` off `0`) + the classification input `ord_n(2) = 2m` (simple faithful cyclic ⟹ `V ≅ 𝔽₂[x]/(f)`, `deg f = ord`) to feed the pinch — the End-field/`D`-line layer (paper Lemma 6.7's trace forms) is the honest route and also gives (87); (ii) 6.8's (87) needs the Hermitian model `q = Tr(x*ᵗHx)` + diagonalization on the hypothesized isotypic data, (88) the `ω₂`-cycle count; (iii) **statement-layer flag for P-20**: the frozen `prop_6_9_*` do NOT carry the isotypic/End-field hypotheses that `lemma_6_8` carries — proving them as stated needs the Clifford/Schur layer internally (or a documented amendment à la `lemma_6_16`). |
| P-15c | B: Shapiro ledger — 6.15 free (104) + involution (105) | ⭐⭐⭐ | O | — | ∅ | ◐ (Opus, 2026-07-03): **free (104) PROVED, std-3** — `GQ2/ShapiroLedger.lean` `lemma_6_15_free_aux`. Engine: `smul_zmodTwo` (Aut(𝔽₂)=1 ⟹ `Z1 N 𝔽₂` are homs `z1_mul/one/inv`), `H2ofFun_eq_of_sub_mem_B2` (junk-total ⟹ coboundary difference suffices), `lWord_mul` (transversal ℓ-cocycle), `shiftCorr`/`lWord_shift` (the `.out` discrepancy). The two raw cochains differ by `δ¹Λ`, `Λ(γ)=Σ_h α(ℓ_h γ)·β(c(γ̄⁻¹h))`, via the ℓ-cocycle split + `G/N` reindex (`sum_reindex_smul`). **Involution (105): routed, not formalized** — much larger (evensNormFun case-split × orientation ε × index-2 cor; paper (107)–(109)); route in the `ShapiroLedger` module docstring. **SPLICE BLOCKED (repo-wide P-15 issue, flagged in docstring):** own-file→`SectionSix` splice is a circular import (`ShapiroLedger` imports `SectionSix` for `graphPullback`/`RegRep`/orbit data) — needs the factor-set/orbit-data layer extracted from `SectionSix` into a lower shared file (coordinated refactor of the hot co-owned file, NOT done mid-parallel). So `SectionSix`'s two 6.15 sorries stay (allowlist unchanged); `lemma_6_15_free_aux` stands proved+consumable. |
...
| P-15h | B: §7 `lam_sq_vanish` ⟹ Prop 7.4 fully std-3 | ⭐⭐⭐ | O | P-15g | ∅ | ☐ **DEP P-15g CLEARED 2026-07-04; ASSESSED as the genuine hard holdout (Opus).** This is the ONLY remaining §7 sorry (`SectionSeven.lean:1148`); `prop_7_4` = std-3 + sorryAx-through-it, no B-axioms. **Assessment**: (1) The reduction — `σ(t):=λ(t²)` is a `Y`-inv hom on `K⊓S` killing `R` — is SOLID (same algebra as `lemma_7_2`'s `(kl)²=[l,k]k²l²` + `lam_comm_vanish` + `lemma_7_2`'s `r²=1`), ~50 ln, confident. (2) BUT the reduction is **necessary-not-sufficient**: `σ=λ∘sq` vanishes NOT because it is a `Y`-inv hom (those can be nonzero on `(K⊓S)/R`) but because it extends to the quadratic form `q=λ∘sq` on all of `K` whose polar `b=λ∘[·,·]` is NONSINGULAR on `V` (the very form `prop_7_4` outputs) — so there is NO clean abstract checkpoint short of the full averaging. (3) Genuinely needs a **tame-averaging infrastructure tier that does not exist yet** (NO proved §7 lemma uses the tame data `π,cH`): `L` acts trivially on the chief 2-factor `V=P/S` ⟹ `V` is an `𝔽₂[H]`-module (`H=Y/L`, `H` tame via `cH`); `H¹(H,V^∨)=0` by char-2 odd-averaging (`f:=Σ_{a}c(a)`, `|A|` odd ⟹ `|A|·x=x`) over tame inertia `C_e=⟨t⟩` (`tame_odd_order`/`zpowers_normal_of_tame`) with `(V^∨)^{C_e}=0` from the chief condition; **ramified/unramified CASE SPLIT** (`C_e` acts nontrivially vs. through odd cyclic `C_n`); cocycle `d_y=y·σ₀−σ₀ ∈ V^∨` extension ⟹ `Y`-normal index-2 `X` with `R≤X≤K` ⟹ contradicts `lemma_7_1_dual`. Est ~400–450 ln of NEW subtle module/cohomology Lean (Mathlib's `groupCohomology` is an impedance mismatch vs. the concrete quotient-group `V`); correctly a **dedicated multi-session build ("averaging tier"), not a one-shot** — flagged rather than gambled to keep the build green + census clean. Original route: char-2 odd-averaging (route in the helper's docstring, `SectionSeven.lean`): `σ(t) = λ(t²)` is a `Y`-invariant hom on `K⊓S` (uses proved `lam_comm_vanish`) killing `R`; if `σ ≠ 0`: base functional `g₀` (dual separation as in 7.3), principal `V^∨`-valued cocycle `c(y) = y·g₀ − g₀`, shear-vanishing from quadratic invariance ⟹ `c` factors through `H* = Y/Y_V`; average over `⟨τ*⟩` (ramified: `tame_odd_order` + `zpowers_normal_of_tame` + `(V^∨)^{⟨τ*⟩} = 0` by chief) or over the odd cyclic `H*` (unramified) — char-2 averaging `w = Σ c(a)` is division-free; generation `H = ⟨cH σ, cH τ⟩` via `FreeProfiniteGroup.homEquiv` INJECTIVITY (no density lemma needed); the resulting `Y`-invariant functional contradicts `lemma_7_1_dual`.  Consumes g's `BlockModule` tools; the dep also serializes `SectionSeven.lean` ownership |
| P-15i | B: Lemma 6.21 — transgression splitting (116-consequence) | ⭐⭐⭐ | **F**+Opus | — | ∅ | ◐ (Opus, 2026-07-04): **mechanism + all infrastructure PROVEN std-3, splitting assembly reduced to a single isolated gap** (`GQ2/Transgression.lean`, sorry-count 1, `lake build` green 8584, gate green).  **Proven (8 lemmas, all `{propext,Classical.choice,Quot.sound}`)**: `exponent_two_of_nonsingular` (nonsingular ⟹ `v+v=0`); `bflat_bijective` (`B_q^♭ : V ≅ V^∨` via `LinearMap.BilinForm.toDual` on the `ZMod 2`-module `V`, additivity⟹linearity by `AddMonoidHom.toZModLinearMap`); `factorSet_spec`+`factorSet_cocycle` (the `H²(C,V)` factor set of `σ = surjInv`-patched-at-1, via `hconj`+`hi`); `polar_fibre` (`polar q u w = ξ(iu,iw)+ξ(iw,iu)`, the fibre antisymmetrization, char-2 grind); `xi_conj_cobound` (**reusable**: conjugation of a trivial-coeff 2-cocycle is the coboundary of `β_s(z)=ξ(s,z)+ξ(szs⁻¹,s)` — 3 cocycle instances); `polar_conj` (polar `C`-invariance, from `polar_fibre`+`xi_conj_cobound`); **`key_transgression`** (the (116) identity `polar q(f c d) v = A c v + A d(c⁻¹v) + A(c*d)v`, `A=mixedA`, proved by ONE char-2 `linear_combination` of **9** `hcocycle` instances with `hconj`-move normalization — full derivation `⟨F,X⟩+⟨X,F⟩ ⟹ (***): ξ(m⁻¹,X)+ξ(Xcd,m⁻¹)=ξ(m,Xcd)+ξ(X,m)`).  **Assembly `splitting_of_global_cocycle`**: descent (`f = δg` from a `B♭`-representable primitive, via `bflat_bijective`-injectivity + `polar_conj`) and the section `s c := i(ofAdd(g c))⁻¹·σc` (hom via `hconj`+`factorSet_spec`+fibre 2-torsion) are **fully proven**; the **one sorry** is the transgression vanishing `d₂ q = 0` — i.e. that a `B♭`-representable (additive) primitive `g` EXISTS.  **KEY FINDING (Fable's design gap):** `mixedA c` is **NOT additive** in `v` (defect `D_c(v,v')=ξ(i(c⁻¹v),i(c⁻¹v'))+ξ(iv,iv')`, a nonzero symmetric zero-diagonal fibre 2-cocycle), so `bflat_bijective` cannot extract `g` from `mixedA` directly; `δA=B♭f` (proven) does **not** give `[B♭f]=0` formally (the "no-spectral-sequence" hope relocates but does not remove the obstruction).  Closing the gap needs the genuine `d₂q=0` input from the *global* lift `ξ` (its base cochain `ξ(σ·,σ·)`), equivalently the additive primitive `Ã=A+κ` with `δκ=0` — the crux the paper gets from `q` being a permanent cycle.  Draft+mechanism in `GQ2/Transgression.lean` (on sorry-allowlist).  `q(c•v)=q v` also derivable but NOT needed (polar-invariance suffices, cleaner). |
