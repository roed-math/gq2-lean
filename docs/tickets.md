...
| P-13f | B: §5.15 duality assembly (`prop_5_15`; trivial-module Gram + simple via 5.12/5.13 + dévissage) | ⭐⭐⭐ | O | P-13b, P-13c, P-13d, P-13e | — | ◐ (Opus 2026-07-04) part (i): **card clauses** (`GQ2/TrivialSelfDual.lean`) + **`mixedB` bilinearity** & **tame `.z`** (`GQ2/MixedBilinear.lean`), all std-3. Finding: tame `.z` = `y₁(x₀)+y₀(x₁)+y₁(x₁)` **vanishes on cocycles** ⇒ trivial-module pairing is **purely wild**. Remaining: general-offset wild `.z` (extend `heisMarking_wildValue_z` off x₀-support, keeping Fox-deriv cross-terms) ⇒ bilinearity assembles the 3×3 Gram ⇒ perfection via `dualEval`; then simple factors (P-13d+5.13) & dévissage (P-13e) |
