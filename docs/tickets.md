...
| P-13f | B: §5.15 duality assembly (`prop_5_15`; trivial-module Gram + simple via 5.12/5.13 + dévissage) | ⭐⭐⭐ | O | P-13b, P-13c, P-13d, P-13e | — | ◐ (Opus 2026-07-04) part (i): **card clauses** + **full analytic pairing**, all std-3 (`GQ2/{TrivialSelfDual,MixedBilinear}.lean`). `mixedB` bilinearity; the general-offset **wild `.z` peel is DONE** (`heisMarking_{h0,x1sig,c0,wildValue}_z_cocycle` + `conjP_z_of_alzero`); closed form `mixedB_cocycle = y₂(x₂)+y₃(x₀)−y₀(x₃)+u₁.z` with the ω₂ scalar `u₁.z` confined to (3,3) (`heisMarking_u1_z_of_{x3,y3}_zero`) ⇒ **Gram det = 1 regardless of ω₂**; `elemDual_separates` gives nondegeneracy. `trivialSelfDual` clause 3 has ONE remaining `sorry`: the `Quotient.lift₂` descent + case bookkeeping (math done, plumbing only). Then simple factors (P-13d+5.13) & dévissage (P-13e). |
