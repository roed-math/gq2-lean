...
| P-13 | B: §5 proofs — **decomposed → P-13a–g** (parallelizable; see [`p13-ticket-split.md`](p13-ticket-split.md)) | ⭐⭐⭐ | O | P-12 | B6, B7 | ☑ **CLOSED (2026-07-05)** — ALL of P-13a–g done. `prop_5_15` PROVED (`GQ2/DualityAssembly.lean`, std-3 exactly, no sorryAx/B-axioms); `cor_5_17_card` now unconditional (std-3+B6+B7, no sorryAx). Every §5 proof obligation discharged. |
...
| P-13f | B: §5.15 duality assembly (`prop_5_15`; trivial-module Gram + simple via 5.12/5.13 + dévissage) | ⭐⭐⭐ | O | P-13b, P-13c, P-13d, P-13e | — | ☑ **CLOSED (Opus+parallel agent, 2026-07-05)** — **`prop_5_15` PROVED, std-3 exactly (no sorryAx, no B-axioms)**, `#print axioms` verified; `check_axioms.sh` all-pass. Composition: dévissage strong induction `prop_5_15_of_simple` (`GQ2/DevissageInduction.lean`, parallel agent) ∘ `selfDual_of_simple` (`GQ2/DualityAssembly.lean`): `tau_split_or_ramified` dichotomy → split (`selfDual_of_split_case`: shapes/normal form + P-13d providers + pairing λ(c)) / ramified (`selfDual_of_ramified`: `hTodd` DERIVED via `tau_powOmega2_smul_trivial`, pairing λ((1+U+U⁻¹)c) perfect via the unipotent operator `sigma2_pairing_operator_injective`). **De-`hU`'d** (hU is NOT derivable — S₃/C₅⋊C₄ admissible counterexamples; the h₀-row x₂-cancellation is in g₀-conjugate PAIRS via new `conjP_u_of_base_trivial`). Statement relocated FoxHeisenberg→DualityAssembly (same FQN, pointer left); `LocalLiftingDuality` imports it for `cor_5_17_card`. **Unblocks P-16c/P-16d.** Details: `docs/p13-ticket-split.md`. |
...
| P-25 | B: §3 boundary construction — Prop 3.10 (`Γ_A` half + local/Cor 3.12 half) + Prop 3.14 `Nonempty BoundaryMaps` (the orphaned `SectionThreeMarked.lean` sorries; see census) | ⭐⭐⭐ | O | P-09, P-10, P-21 | B5, B8, B3c, B10 (inherited via 1.1 / 3.2) | ◐ (Opus 2026-07-04) **`prop_3_10_gammaA` CLOSED — std-3, spliced** (`GQ2/BoundaryConstruction.lean`, own file): full marked iso `Γ_A(2) ≅ Π` (`prop_3_10_gammaA_proved`) via the τ=1 **word collapse** (`wildRelWord_eq`: wild relator (6) → pro-2 relator (20)), forward descent `phiP` + backward `PsiMax`, glued by density; `maxProPMk_gammaTau` (τ dies) + `piRelatorWord_maxA_eq_one` (M ⊨ Π's relation).  **`phiP` = Prop 3.14's `pro2A`** (with generator values) is banked.  ◐ (Opus 2026-07-05) **`prop_3_10_local_marked` CLOSED — std-3 + B3c + B8, spliced** (`GQ2/LocalMarked.lean` + `GQ2/DyadicNielsen.lean`, own files): the local anabelian iso `(Π,ν₂) ≅ (G_ℚ₂(2),ν_ur)` = `prop_1_1` (`G_ℚ₂(2)≅D₀`) ∘ the by-hand **Nielsen iso** `d0PiEquiv` (`D₀≅Π`, (23)/(24)), `ι`=`ztwoEquivPadic`, ν-compat by density on D₀'s 3 gens (the `Ztwo`-def barrier bypassed via the composite `zetaNuTwo = ζ∘ν₂`).  ◐ (Opus 2026-07-05) **`prop_3_14` ASSEMBLED — full 21-field `BoundaryMaps` bundle** (`GQ2/BoundaryMapsWitness.lean`, own file): `tameA`=`phiA`✓, `pro2A`=`phiP`✓, `compatA` (density)✓, **`surjA` fully proved**; G_ℚ₂ side `tameF`/`pro2F` (from `prop_3_2_local` [B10] / `local_marked` [B5]) + kernels + surjectivities ✓.  Reusable kit all **std-3**: `fiberProductExists` (fibred-product surjectivity), `proPKernel_image_ge` (surjective image of the pro-`p` kernel), `ker_nuT_le_proPKernel` (+`maxProPMk_tameTau`: `ν_t` = max pro-2 quotient of `T_tame`).  **ONE residual `sorry` = `tame_reciprocity`**: `ι(ν_t(tameF g)) = ν_ur(toAb g)`, the tame-quotient↔reciprocity **orientation bridge** (B10's `equiv` is unoriented, B5's `ν_ur` independent ⟹ genuine arithmetic: wild inertia unramified-trivial).  `compatF`/`surjF` follow from it.  **Needs a B10-strengthening or a `tame_reciprocity` axiom — census decision, FLAGGED TO USER.**  `prop_3_14` spliced (statement amended with `[CompactSpace/TotallyDisconnectedSpace AbsGalQ2]`); `SectionThreeMarked` now sorry-free.  Full build green (8660), guard all-pass, census 13. |
