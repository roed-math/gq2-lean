...
| CB-MP | build the `IsMarkedPresentation` instance for `GammaR n q R` (CB-1's created obligation; route `presentationLift` + `profinitePresentation`, one `FreeGroup ι` ↔ `FreeProfiniteGroup (Generator n)` seam) — **in whichever form GR1's redefinition supports**, plain or restricted | opus | `GQ2/Dyadic/Count/Presentation.lean` (new leaf) | CB-1 ✓, **GR1** | queued behind GR1 |
