...
| SD2 | `SourceNumerics n` + parameterized record + n=1 adapters (≈0.5k; + `sourceF_N` from the verified `*_local` leaf pack; ⚠ SD-R3 shape rule: the `hZcard` field is `#Z¹(V) = #V * SN.h1Mult #V`, NOT `#V^2`) | fable | `GQ2/Dyadic/SourceDataN.lean` | SD1 ✓, SD-R1–3 ✓ | **dispatched 2026-07-31** (wl worktree, branch `dyadic-sd2`) |
| SD3 | two-sided degree-n induction + ℚ₂ regression (**SHRUNK by SD-R3**: `prop_8_9_of_sources` is ~40-ln ∃-packaging over two `closedRecursionK_of_source` applications into SD-R2's solver; **zero omega seams left**; `blockFrameImpl`-free statement inherited) | opus | `GQ2/Dyadic/ThmFourTwoN.lean` | SD2 | queued |
