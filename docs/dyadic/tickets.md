...
| AS4 | `n = 1` wrapper: L-word machinery recovers the ℚ₂ theorem | opus | `GQ2/Dyadic/Instances/QTwo.lean` | AS1, WL-c | **dispatched 2026-08-01** → `~/claude/gq2-dyadic-as`, branch `dyadic-as4`. Unblocked: AS1 ✓ and WL-c ✓ are both done, and the file sits in `Instances/` so it is clear of CB-H2's concurrent `Count/` work. Briefed that a *failure* to recover at n=1 (general hypotheses unsatisfiable at n=1, or the frozen general word not specializing to the ℚ₂ word) is a **major finding**, to be nailed down as a witnessed refutation rather than forced. |
