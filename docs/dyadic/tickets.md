...
| CB-RES | discharge `ResolvesGammaRelators n q R w` for the five frozen families — CB-MP's `of_two` reduces each to two equations (`freeToProf (w 0) = tameRelatorGen n q` and `freeToProf (w 1) = (freeMarking n).eval R`); the tame one is mechanical, **the wild one is open wherever the branch word carries genuine ω₂ exponents** and needs the word lane's `ResolvedAt` / `eval_eq_evalZ` / `evalZ_eq_lift_heisToFree` | opus | `GQ2/Dyadic/Count/Resolve.lean` (new leaf) | CB-MP ✓ | **dispatched 2026-08-01** |
