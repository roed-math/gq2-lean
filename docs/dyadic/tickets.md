...
| CB-VAR | **the nonzero variation class, per branch** — CB-H2's single named follow-on, and it unblocks THREE clauses at once: it supplies `cardH2`'s `Nontrivial` hypothesis (via `nontrivial_H2_of_ne_zero`), `lem86`'s `hvar`, and thence `stageR136` (via `blockStageR136`). **CB-H2's sharpening of CB-2's scheduling note: `cardH2` and `lem86` SHARE their one non-generic input — they are not merely ordered.** Also discharge `hwildLevel` for `GammaR` from `AdmissibleR` §3. CB-H2 compiled the composition as a scratch check: `lem86N tfg (cardH2N_closed …) D rho S u hvar` **typechecks**, so `lem86` needs no new counting, and `stageR136_gammaR_of_hcard`'s only cohomological input is `hcard_R = 2` | opus | `GQ2/Dyadic/Count/Variation.lean` (new) | CB-H2 ✓ | **dispatched 2026-08-01** → `~/claude/gq2-dyadic-mc`, branch `dyadic-cbvar` |
