...
| WMP-c | Stokes/scalar/Hessian/phase: self-replication cancellation incl. every `T`-dependent central term + the deferred product certificate + `affinePhase` | fable→**opus** (quota) | `GQ2/Dyadic/Certificates/Mpc.lean` | WMP-b ✓ | **dispatched 2026-08-01** (ob worktree, `dyadic-wmpc`) — **the last wave-2 ticket**; carries WMP-b's five-item handoff, WM0-c's transferable `swapDifference` route for the S4.5-equal block order, and WW4's six-item affine-phase gap list (the campaign's last open mathematical item) |
