...
| CB-DET | ASK's `determinant` binder: the (83)-evaluation bridge LG5 → `GaussZResidueK`. LG-K's arithmetic is CLOSED; what is missing is the ℚ₂ `GaussZ/FinalD.lean` plumbing (over `Final`/`Local`/`Reduction`) having no K clone. **SD1 §9: state at `|V| * SN.h1Mult |V|`, NOT `|V|²`** | opus | `GQ2/Dyadic/GaussZ/FinalDK.lean` (new, name provisional) | ASK ✓, LG5 ✓ | queued |
