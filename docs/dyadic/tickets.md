...
| CB-H2 | **the count lane's new critical path**: the missing degree-2 rung of CB-1's comparison ladder — relate `ContCoh.H2 Γ A` to `WordH2`/`StokesH2` degree-generically (no `AbsGalQ2`, no B6 numerology; neither `WordCoh.card_H2_le_two` nor the ℚ₂ `obsH2` substitutes, and CB-2 measured why). Discharges CB-2's `hcomp`, and unblocks `cardH2` + `lem86` + `stageR136` (≈400 ln combined after, 0 before) | opus | `GQ2/Dyadic/Count/HTwo.lean` (new leaf) | CB-2 ✓ | **dispatched 2026-08-01** |
