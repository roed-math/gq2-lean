...
| CB-DD | dedup ticket named by AS4-b: **five** copies of the trivial `ZMod 2`-action (`Count.trivialSMulZmodTwo`, `Count.scalarAction`, inline in `HalfTorsorGammaA.lean` + `HalfTorsorGammaR.lean`, and `GammaRHom`'s) and **four** of `topGen_gammaR` (`Count/Routine.lean` §3, `Count/ProTwo.lean` §2, `Count/Presentation.lean`, `GammaRHom`). `Count/Scalar.lean`'s docstring rules the merge belongs to whichever ticket first has both in scope — **`GammaRHom.lean` is that home**. Also correct `CertificateMain.lean`'s stale obligation-table row (piece 3 was closed by CB-0, not unwritten) | opus | `GQ2/Dyadic/GammaRHom.lean` + the duplicate sites | AS4-b ✓ | **DISPATCHED 2026-08-02** (opus, ob worktree, branch `dyadic-cbdd`; WAH/WWH zero-statement-change zero-rename standard binding; import direction verified with Lake per MC5-swap's precedent; partial dedup acceptable with reasons) |
