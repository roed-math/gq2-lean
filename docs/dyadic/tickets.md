...
| CB-6 | **the last count-lane item: `hsep`'s candidate-side supplier**, correctly located by CB-5. Two routes: **(i) the degree-generic marking route on top of `sepWordN`** — port `exists_relatorFree_marking` / `mlift_of_relatorFree_marking` off `Fin 4`/`Marking` to arbitrary `ι`/`ρ` (**CB-5's recommendation**; `sepWordN` is its engine and is banked; note CB-4 declined this port for the *clause* and was right to, but the *supplier* needs it); or **(ii) a module-coefficient degree-2 rung** `H²(Γ,A) ↪ WordH²(A)` — which does not exist and which **CB-H2's scalar-only rung cannot supply**. ⚠ **Route (ii) is exactly what CB-3's standing warning says not to schedule blind.** Also owed: branch instantiation of CB-5 §5 at each frozen row (mechanical, CB-VAR §9's pattern) **at `Vmod`** | opus | `GQ2/Dyadic/Count/Marking.lean` (new) | CB-5 ✓, CB-4 ✓ | **dispatched 2026-08-01** → `~/claude/gq2-dyadic-lg`, branch `dyadic-cb6` |
