...
| CB-FR | CB-TR's items 3–5: `ResolvesAt` for the other four families, the **ℤ₂-resolver for the procyclic-N row** (`npcW` is not ω₂-only — the one genuinely open word-side item), and the per-branch resolver pin (`e = 3` vs `e = 1`) with a check that no branch's frozen Stokes payload is pinned at an unusable `e` | opus | `GQ2/Dyadic/Count/Frozen.lean` | CB-TR ✓ | **dispatched 2026-08-01** |
...
| WW6 | the `NpcJet ↔ WordCoh` bridge (CB1 Q4's silent blocker; WMP-c, WNP-c and WWH all converge on it — gates Mpc's `determinant`/`HessRelZTarget`) | opus | `GQ2/Dyadic/Word/NpcBridge.lean` (new leaf; NpcJet and Word/WordCoh stay closed) | — | **dispatched 2026-08-01** |
