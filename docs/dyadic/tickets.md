...
| CB-5 | **the two word-side (candidate) suppliers of CB-4's forks** — the arithmetic side is complete as of CB-4, so this is all that stands between the count lane and its candidate-side close. (a) `IsTwoSeparating` from `IsSelfDualN.cardH2`: compose `StokesQuasiIso.h2_inj` (`Word/Stokes.lean:982`) with `stokesDual_separating` (`Word/StokesDual.lean:400`) — the generic `sep_word`, **~5–20 lines, and this composition does not exist anywhere in the repo today** (the one genuinely new lemma either fork needs); (b) `IsRightSeparating` from `IsSelfDualN.pairing`: the generic clone of `b1_of_pair_cochain_B2`, needing a degree-generic `obs_inflation`/`mixedB` bridge — `WordCoh.obsH2_eq_of_factor` (`Word/WordCoh.lean:1224`) IS the degree-generic `obs_inflation`, and `mixedB_eq_relZPair` is already source-generic but `Fin 4`-shaped with `heisEta1` the dyadic replacement. **(b) is the larger of the two.** Neither blocks the arithmetic side | opus | `GQ2/Dyadic/Count/Separating.lean` (new) | CB-4 ✓ | **dispatched 2026-08-01** → `~/claude/gq2-dyadic-lg`, branch `dyadic-cb5` |
