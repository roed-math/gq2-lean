...
| AS3 | instances √2, √5 (M0) · √10, √−10 (Mpc, procyclic gate) | opus | `GQ2/Dyadic/Instances/{Sqrt2,Sqrt5,Sqrt10,SqrtNeg10}.lean` | AS1 ✓, WM0-c ✓, WM0-d ✓, WMP-c ✓, WMP-d ✓, **CB lane** | **UNBLOCKED 2026-08-02 — the CB lane's clause list is COMPLETE (CB-6, `4d7ea23`); this row is now on the critical path to G3.** [was: BLOCKED on the CB lane]; note **merge gate 9 is already CLOSED** (WMP-d) and √2/√5 are complete at both projector branches (WM0-d), so this is instantiation only ⚠ **Brief the agent with the two live instantiation traps**: ASK's instance-path trap (`GalK K` vs `↥K.fixingSubgroup` — `rw` fails where `exact` succeeds) and **CB-DET's `ContinuousSMul ↥U (MuN 2)` trap, which every consumer of `local_gauss_K` at a concrete K will hit** (defeq but not *reducibly* so; fix = name the two binders and pass them explicitly). Also fold in the row's two book-keeping hand-offs, both from the single `hsimp` it already carries: a `StokesDuality`-at-`Vmod` payload to `Count/Separating.lean` §5, and a `StokesDuality`-at-`T` payload to `hsepN_marking`. |
