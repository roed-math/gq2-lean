...
| MC-CoV | **errata item 3, promoted from question to ticket**: derive the compact-M marked change of variables (absent from packet and drafts; the procyclic recipe degenerates at r = 0) and discharge MC5's threaded `IsUnit ν'(C̄₀)` binder as a theorem | opus | `GQ2/Dyadic/MarkedCore/CompactCoV.lean` (new leaf) | MC5 ✓ | **dispatched 2026-08-01** (mc worktree, `dyadic-mccov`) |
...
| CB-1 | **the lane's load-bearing ticket** (post-spike): the `z1Equiv` comparison isomorphism carrying CB-S's word-complex counts into the recursion's cocycle vocabulary — the arithmetic already matches exactly, only the vocabulary transport is missing | opus | `GQ2/Dyadic/Count/Compare.lean` | CB-S ✓ | **dispatched 2026-08-01** (ww worktree, `dyadic-cb1t`; scope reduced 1400 → ~1100 by the spike) || CB-P | the candidate-side pro-2 bridge | opus | `GQ2/Dyadic/Count/ProTwo.lean` | CB1 ✓ | **done 2026-08-01** (ad1e488…c60613f, 4 commits → merged (+ orchestrator merge fix: `open TameSpec`, branch predated AS1-b), import registered; **701 ln vs the 700 budget — dead on**, 49 decls, all 12 headlines exactly std-3, **zero BLab, zero B8, no exception**, no `decide` at all; **CB1's Labute-free measurement HELD exactly** — the candidate leg generalized cleanly, `exists_pro2R` indeed does not generalize (it was the wrong composite) and `maxPro2Bridge` does; **NO new G-Lab docket item**; closes ALL FOUR pro-2 fields (`pro2`/`ker_pro2`/`hpro2`/`compat`) with a permanent typechecked field-fit example against `WordCertificate`'s literal slot types; the entire hypothesis surface is one `CorePresentation` record + `q ≠ 0` + `Even q` — **no rank pin, no q = 2 pin, no 1 ≤ α, no Demushkin hypothesis**; N0 pilot done at general rank 4+2h (not just h = 0), upgrading `Words/N0.lean`'s h = 0-only corollary; **701 lines covers ALL branches where ℚ₂ spent 504 on one word plus a ~500-ln twin** — because F3's `prop_3_4_two` already IS the generic pro-2 universal property and `map_gammaGen_tau_eq_one_of_isProP` already IS τ-death, so the analytic keystone needed no re-proof; ⚠ CB1 under-called one input (its "R-admissibility at finite 2-group levels, per branch, small" costs ZERO — F3 had done it generically); bonus: `topGen_gammaR` did not exist in the dyadic tree and is now proved; ⚠ **`presentedBy_DSq` does not exist in the repo** — a 3-line anonymous constructor from `sqLiftHom`/`_gen`/`dsq_hom_ext`, needs an owner before L instantiates; the other four branches each need one `CoreReindex` + a six-line restatement (**L is cheapest** — its `sqIdxMark` already IS `toCore` at all h; M0/Npc/Mpc need twisted dictionaries, all triangular so `ofCore` exists, and M0/Mpc carry `1 ≤ α`)) || CB-0 | the routine `WordCertificate` fields | opus | `GQ2/Dyadic/Count/Routine.lean` | CB1 ✓ | **done 2026-08-01** (30a539b+0a4a52f → merged, import registered; 480 ln (206 code / 275 doc) vs the 350 budget, 37 decls, all 16 headlines std-3, 0 sorries; **6 of `WordCertificate`'s 17 fields CLOSED** — `tameSpecialization` (generic + all five branches), `tfg`, the scalar trio, `htame` — verified at the √−2 pilot by SUBSTITUTION (`pilotSubstitute` re-elaborates an arbitrary pilot certificate, which only typechecks if the terms sit at exactly the right types); **the campaign was missing a real step**: `CertificateMain.lean:362` asserted `τ^{ω₂} = 1` in `T_q` "by Lem 3.1", but F3 landed Lemma 3.1 only in FINITE-IMAGE form and `T_q` is profinite — the composition existed NOWHERE, so no branch had any route to `TameSpecializes` (the only bridge takes `KillsWild`, which all five refute); `tqTau_zpowHat_omega2` is that missing profinite step and every branch now factors through it; ⚠ **deviation from the brief's premise**: only THREE branches state the tame value as `= τ^{ω₂}` (M0 states `τ^{ω₂}·eBlock…`, Mpc a fourteen-factor word) — what is uniform is the weaker `t.τ^ᶻω₂ = 1 ⇒ word dies`, so the generic step was built on THAT hook; ⚠⚠ **`hwild` is NOT routine and CB-0 REFUTED it rather than asserting it** (`not_isProP_two_ker_tameOfSpec_one`: at `R = 1` the candidate surjects onto ℤ/3 with every wild letter hitting a generator) — the honest shape is per-relator-family compactness, measured against three precedents (census axiom B10 at G_ℚ₂; AX4 structure field at G_K; ~80-ln `isProP_wildCore`/`_R` for the ℚ₂ candidates) → **CB-W opened**; ⚠ **caution for AS2**: AS1's inventory claims `proTwoWord` is "landed for all five" — the EXACTLY PARALLEL claim for `tameSpecialization` turned out to need a lemma that existed nowhere, so verify the composition before assuming it; erratum: `M0.lean:547` cites `killsWild_of_odd`, which exists nowhere) |
| CB-W | **`hwild`** — the one `WordCertificate` field with no owner after CB-0's refutation: a generic admissible-limit/compactness core over `gammaRelators n q R` with a `Pro2Core`-style hypothesis + five per-family discharges (generalizing the ℚ₂ candidates' `isProP_wildCore`/`_R`); **must be a THEOREM on the candidate side** even though it is an axiom (B10/AX4) on the arithmetic side | opus | `GQ2/Dyadic/Count/Wild.lean` | CB-0 ✓, F3b ✓ | **dispatched 2026-08-01** (wn0 worktree, `dyadic-cbw`) || CB-1…CB-6 | the lane's remaining 8 tickets (spine 1400, scalar 900, lifting 1600, stokes 1800, gauss 1900, gauss-L 1100, gauss-M 1300, five instantiations 5×500) — sizes from CB1 §, calibrated against SD-R's 54–69% realization and wave 2's 641–1963-ln rows | opus | `GQ2/Dyadic/Count/*` | **CB-S verdict** | queued behind the spike || F3b | Gate-B interface repair | opus | `GQ2/Dyadic/TameBoundary.lean` (in-place, approved) | AS1 ✓ | **done 2026-08-01** (0286de6+117d26d+7761179 → merged; +409/−100, decls 78 → 108, **all 45 audited prints exactly std-3**, every consumer (5 Words, 12 Certificates, CertificateMain) compiles UNEDITED and the WW5 hash pins re-verify byte-identical; `TameSpec.TameSpecializes` = the ledger's `specializeTame R = 1`, definitionally identical to AS1's §2b (`Iff.rfl`-checked); **scope beyond brief, and the reason is the finding**: through `KillsWild`, **eq. (27) joint surjectivity was UNREACHABLE at every frozen branch word — the packet's Thm 3.5 word side had NO instance at all**; F3b rebuilt the whole Prop 3.4 + Thm 3.5 chain over the satisfiable predicate, and `gammaR_boundary_surjective_of_spec` now instantiates at compact N; `KillsWild`'s DEFINITION is unchanged (changing it would falsify the five landed `not_killsWild` theorems) — it gains a warning docstring naming all five refutations plus the true one-way implication, and NO proof is duplicated (the nine KillsWild-shaped decls are one-line wrappers, definitionally the old terms, so downstream simp sets and rfls are unaffected); **TWO Gate-B routes, not one** — compact-M and Mpc do not state a bare `τ^{ω₂}` value (their kill-wild value carries an 𝓔-block, and an orbit norm for Mpc, collapsing only once `τ^{ω₂} = 1`), so they state death as an implication; ⚠ **SECOND FINDING, correcting the WN0-a ruling**: `killsWild_of_tau` is ALSO not a usable route — its own hypothesis `∀ G t, t.τ^ᶻω₂ = 1` is refuted by the same ZMod-8 witness, so it records an argument's SHAPE, not a route (the five lanes' `killsWild_of_tau` theorems are dead weight; and `killsWild_of_odd` does not exist under that name — the lanes' theorem is `eval_killWildLetters_*_eq_one_of_odd`); sub-namespace `TameSpec` forced (AS1's §2b holds the top-level names until AS1-b deletes it)) |
