...
| CB-LV | clerical follow-on to CB-FR2 (it named these as other owners' edits): retarget `Count/Presentation.lean`'s pilots off the literal `hord … ∣ 6` onto the split-level pair, add the three remaining split-target instantiations + the mpc `.hat` one (three lines each, exact template `resolvesAt_and_endpoint_nCompactFam_split`), and the `V`-side (`Vmod`) split twin (needs `[Finite E]`) | opus | `GQ2/Dyadic/Count/Presentation.lean` | CB-FR2 ✓ | **dispatched 2026-08-01** → `~/claude/gq2-dyadic-ob`, branch `dyadic-cblv` (CB-H2 landed, so `Count/` is free of the concurrency hold) |
