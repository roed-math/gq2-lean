...
| CB-TR | **the target-resolved redesign CB-RES identified** — reformulate clause (iii) so the resolver is chosen PER TARGET (`omega2Exp (exponent Q)` via `eval_eq_evalNat_of_dvd`), dissolving the seam; rebuild `z1Equiv`/`h1Equiv` and the `Γ_R` instance over it | opus | `Count/{Compare,Presentation}.lean` (reopened) | CB-RES ✓ | **dispatched 2026-08-01** |
...
| WW6 | the `NpcJet ↔ WordCoh` bridge (CB1 Q4: a silent blocker with no ticket anywhere; gates Mpc's `determinant` through two dependency levels) | opus | TBD | — | queued |
