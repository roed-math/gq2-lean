...
| FD2 | the even-`n` normal form: for `n = 2+2h` the cup form has `b e e = 0`, so WL-c's `⟨1⟩ ⊥ H^{⊥m}` splitting does not apply — prove the even-degree analogue (rank-2 non-alternating head ⊥ symplectic residue) and its `exists_cupFormK_normalForm` twin, reusing FD1's degree-general facts (i)/(ii) and WL-c's `exists_symplectic_equiv` | opus | `GQ2/Dyadic/FieldDataEven.lean` (new leaf; FieldData.lean stays closed) | FD1 ✓ | queued (dispatch with the AS wave — the M/N rows' item-(5) closure needs it) || WM0-d | final compact-M Hessian assembly `hessRelZ_mCompact_P{0,1}` | opus | `GQ2/Dyadic/Certificates/M0Assembly.lean` | WM0-c ✓ | **done 2026-08-01** (097c188+b78b5dd → merged, import registered; 823 ln, 44 decls, 0 sorries, 43/43 ≤ std-3 and **no `decide` needed at all**; **the half-day estimate HELD** — every ingredient WM0-c listed was present and the paper arithmetic closed as written (three mechanical errors on first build); both equations landed at general h: `P1 = q(c₀)+b_q(c₀,c₁)+Σⱼb_q(dⱼ,eⱼ)` and `P0 = q(c₁)+b_q(c₀,c₁)+Σⱼ…`; transport lemma `commR_mul_right_of_commute` with TWO honest side conditions (both free at the call site from WM0-c's `hessM_line_comm`; without `hLa` the honest conclusion is the conjugate); **new bookkeeping device**: the charged C-line `hessLineZ` has a diagonal product law, so the six factors chain into ONE line whose C-coordinate carries the power balance and whose fibre is the answer — a two-column sum instead of a commutator wrestle (reusable); P=0 non-vacuity added unasked and **correctly labelled a WITNESS class, not the honest ramified class**; ⚠ finding: no unconditional profinite twin of the pilot's `sqrtNegTwo_hess_eval` exists here — compact-N is resolver-immune (one ω₂-block, dies on the C-line) but compact-M's correction block is resolver-SENSITIVE, so the eval carries a branch hypothesis (the Hessian-level twin of WM0-c's Stokes deviation); ⚠ `mk` is `private` in M0.lean (relay); **AS3 now has √2/√5 complete at both branches** — word-side equations, both frozen endpoints, the functional identification, and equal Gauss residues (the block swap is an isometry); only the P=0 ramified-class discharge remains per-instance) |
| WM0-b | Fox certificate (reversed correction order) | opus | `GQ2/Dyadic/Certificates/M0Fox.lean` | WM0-a ✓ | **done 2026-08-01** (824a786…67cb949, 9 commits → merged, import registered; 1545 ln, 96 decls, 0 sorries, 28/28 headlines std-3; universal row `(0, P·S₂^{−2m}, P·S₂^{−m}+P, P·S₂^{−m}+P·S₂^{−2m}, S⁻¹+P)` = the frozen JSON entry-for-entry INCLUDING print order; 12 certificates incl. √2/√5/(2,0,2) both branches; **normal forms are LITERALLY the pilot's** (imports N0Fox, reuses its forms + ops — the frozen shared-with-WC-N0 note mechanized, strengthened to the same-row theorem on unramified simples); ⚠ THREE FINDINGS: the σ-column cancels over ℤ (the differentiated Prop 9.2 power balance — not a char-2 fact), NO α-hypothesis at first order (α is gate-C-only on this row), and **S₂ = 1 is a HYPOTHESIS not an interpretation** (unramified-simple certs carry hS₂; N never met this — no σ₂-letter); E_m^rev rows landed HONESTLY (nonzero; invisible in each specialization for two DIFFERENT reasons) + order-invisibility as a THEOREM chain, rejection correctly left to WM0-c; ⚠ new gotcha relayed mid-flight to WNP-b/WMP-b: `deltaC` silently collides with the frozen peripheral `GQ2.deltaC`; 7 more hoist candidates → cleanup queue) |
...
| WMP-d | complete the Mpc lane: the `mpcFam` Stokes family (none exists anywhere), `IsStokesEndpoint`, `stokesDuality_of_simple`, a scalar Gram — **plus `hlinrow`**, the linear copy's closed-form Fox row at σ-free offsets (whose hard piece is `E₂^pc`'s orbit-norm block), which is the last input of `sqrtNeg10ProductRowCert` and therefore **what turns merge gate 9 from pinned-and-wired into CLOSED**. Carries WMP-c's unverified intelligence: `e2W` holds `dW 2` an ODD number of times (1+2m), so unlike M0 this row will NOT die syntactically and the endpoint needs the tame row, pilot-style — verify first, a corrected count is equally a finding | opus | `GQ2/Dyadic/Certificates/MpcStokes.lean` (new leaf; Mpc.lean stays closed) | WMP-c ✓ | **dispatched 2026-08-01** (ob worktree, `dyadic-wmpd`) |
