...
| WC-M0 | F2 ✓, WW1–WW5 ✓ → WM0-a ✓ / b ✓ / c ✓ (+WM0-d) | **word-certificate layer COMPLETE 2026-08-01** (both projector branches + the S4.1 §9.4 order-rejection formula with its 𝔽₁₆ pin; the final single-equation assembly is WM0-d, in flight; the seventeenth-root pin is a `QuadraticFp2` API item, out of lane) |
...
| WM0-d | final compact-M Hessian assembly `hessRelZ_mCompact_P{0,1}` (WM0-c's documented residual: one transport lemma `commR (a·L) b = commR a b` + six-factor bookkeeping; every ingredient already in `Certificates/M0.lean`; est. half a day) | opus | `GQ2/Dyadic/Certificates/M0Assembly.lean` (new leaf; M0.lean stays closed) | WM0-c ✓ | **dispatched 2026-08-01** (wn0 worktree, `dyadic-wm0d`) |
