...
- **2026-08-02 (orchestrator, AS-F2 — four of five rows are now LITERAL, and the gate
  knows it):** merged ff `716672b`; verification green (3514 exit 0, both audits, census
  11). The three M-row literal theorems landed (specializing the `_nonvacuous` parents,
  prints byte-identical, conclusions fully literal via the `hqK` substitution), trust doc
  §C.7 records the measured table. Orchestrator hardening: **check-5b now gate-freezes 17
  capstones** (the 4 literal theorems added), probe retargeted to `import GQ2` (the
  Main-only import couldn't see Fields — first audit run failed for that plus a
  second stacked cause, an un-rebuilt olean tree; both fixed, and the orchestrator ate the
  `| tail` exit-mask trap once during diagnosis — penance logged), §C.0's stale field-object
  row rewritten. ⚠ AS-F2's structural finding for the queue: **the M-row headlines consume
  no `hdeg`** — the degree belongs to the unbuilt M-row `KSupply` producer (highest-value
  next step for these rows; would surface `MMixHypothesis` the way the pilot surfaced
  `NScalingHypothesis`). √5 remains the one non-literal row (blocked on the `private`
  residue-field bridge — owner one-worder — plus the norm form). **All dispatched work is
  now LANDED — nine landings today, zero in flight. G3 awaits the owner per AS5's packet.**
- **2026-08-02 (orchestrator, AS-F — literal fields are FEASIBLE, and the pilot theorem
  is now stated at the actual ℚ₂(√−2)):** merged, registered; verification green (3514
  exit 0, both audits incl. check-5b, census 11).
  **`Fields.sqrtNegTwo_candidate_equiv_galK_literal`** re-printed at root: byte-identical
  to AS2's headline — literal instantiation costs NOTHING on the census (B5-K/B10-K
  nowhere). `hdeg` at all five rows; `q_K = 2` + ramified-i at all four ramified rows —
  with two better-than-expected proofs (parity kills the residue-field dependency; the
  `1 + √a + i` witness is uniform, the earlier ζ₈ one retracted in-file). Blocked and
  honestly named: √5 (needs the norm form + the residue-field bridge, which is `private`
  in frozen `UnitFiltrationCounts.lean` — one-word owner call), `ramifiedData`, and the
  AX3/AX4 bundles = local CFT = the owner-approved endstate. ⚠ Two NEW process traps
  broadcast: `lake env lean` does not apply lakefile `leanOptions` (per-file gate must be
  `lake build <module>`), and bash heredocs corrupt `ℚ̄₂` notation bytes. **AS-F2
  dispatched** (opus, lg, `dyadic-asf2`): the three ~30-min literal specializations
  (√2/√10/√−10 — all inputs in hand now that AS3-b settled the instance files) + a
  text-only trust-doc addendum citing the literal pilot as evidence nothing else in the
  headline is field-arithmetic. Owner queue additions: the √5 ticket (250–400 ln, split
  ramified-i off cheap), the de-private one-worder.
- **2026-08-02 (orchestrator, AS5 — THE CAMPAIGN'S FINAL THEOREM IS IN THE TREE; G3 IS
  REACHABLE):** merged `82a2e6c`, Main registered; verification green (build 3512 exit 0;
  `check_dyadic` all-OK with **D4 ACTIVE for the first time** — merge-gate 7's hash half
  now pins envelope↔LaTeX↔Lean digests for all five words; `check_axioms` green with the
  new **5b block**: 13 dyadic capstones at frozen per-declaration sets, B5-K/B10-K in
  none, 4 n=1 twin pairs identical to the ℚ₂ capstone, growth trip-wire exercised).
  **`ramifiedI_candidate_equiv_galK` re-verified at root: exactly std-3 ∪
  {B1,B6,B7,B9,B11a}**, and **`no_frozen_Npc` is a std-3 THEOREM** — the honest, recorded
  delta from the packet's fully-general statement. The trust boundary is now a document
  (`literature-axioms-dyadic.md`) with the complete residual surface and a §D checklist
  for the owner. **G3 packet, in order: trust doc §D → §A → §B → §C (especially C.2's
  `hdet` 9-of-11 and C.6's Npc gap) → `#check` the two §5/§2 theorems → one command
  `bash scripts/check_dyadic.sh` re-verifies everything mechanically.** AS-F (literal-field
  feasibility, opus) still in flight — merges independently, not a G3 blocker. Board
  archival happens AT G3 (owner gate), not before. Owner queue standing: (a) Gauss lane
  (`hdet`), (b) R-stage (`hsplit`/`hZcount`), (c) ASK leaves, (e) `piAb` lemma, plus (new,
  low) the M-row marked-core composite and the Npc dictionary; CB-SG-Q unchanged.
- **2026-08-02 (orchestrator, MC-CoV-split — the split stands, the adoption falls):**
  merged ff `47bfa91`, `CoVDischarge` registered, `Cores.lean` debt note amended;
  verification green (3511 exit 0, both audits, census 11, 9 protected prints
  byte-identical). The discharge layer moved to its correct altitude (`public import
  …MarkedCore.M` ALONE — it had been two levels too high by authorship accident), retiring
  MC5-swap's build-cycle constraint. **The adoption half was REFUTED with Lake-checked
  witnesses**: all six sites are `{α h}`-generic, the discharge is rank-four-only AND
  false at `α = 1` (`mUnit 1 = −1 = χ_M(B̄)` — a χ-trivial direction missing `C̄₀`), and
  there are ZERO consumers (`Instances/Cores.lean` doesn't import `MarkedCore.Certificate`
  — instances reach `GalK` via `KSupply`). Orchestrator judgment call: KEEP the split
  (placement independently correct, zero cost) — the refutation, not the file, is the
  ticket's finding. ⚠ AS3-b's `mRelWord_of_commute` homing destination proved unreachable
  (`commP_eq_one_of_commute` lives above `MarkedCore/Cores.lean` in `HandleMix.lean`) —
  in-file note corrected. Queue notes (NOT dispatched): compact-row `_of_chiKer`
  companions incl. the K-facing `_KM` (no consumer yet); the `BoundaryFrame` Z₂-homing
  (frozen-tree ticket). In flight: AS5 (fable), AS-F (opus).
- **2026-08-02 (orchestrator, AS3-b — the ν-binders are GONE, and the freeze exception
  went unused):** merged `d2ecef9`, Cores registered; verification green (3510 exit 0, both
  audits, census 11, twins identical; nonvacuous prints re-verified at root). **All four
  AS3 headlines now have ν-free `_nonvacuous` corollaries at std-3 + {B1,B6,B7}** — the
  abstract slot is demonstrably inhabited at every AS3 row, a G3 exhibit. The authorized
  `MaxProP`/`BoundaryFrame` freeze exception was NOT needed — single-element marking made
  commutativity free (`Commute.zpow_zpow`), the relator collapsing to `2e₀+2^α e₂ = 0`.
  The hoist landed hash-verified (sha256-identical blocks, 85 FQNs re-checked); QTwo's
  inventory is all-green (3 rows corrected, not 1); `CertificateMain` matched. **MC-CoV-split
  hold RELEASED and dispatched** (opus, ww, `dyadic-mcsplit`, retargeted at Cores §4/§8 per
  AS3-b's routing note). Remaining homing debt (owner-visible minor): the two
  `Z₂`-procyclicity facts belong in `BoundaryFrame.lean` — needs a frozen-tree ticket.
  In flight: AS5 (fable), AS-F + MC-CoV-split (opus).
- **2026-08-02 (orchestrator, AS2 — THE PILOT LANDS: the first `WordCertificate` ever
  constructed, and the honest clause count is 9-of-11):** merged + registered `ca01f17`;
  verification green (build 3509 exit 0, both audits, census 11, twins identical); trap 5
  fired a third time (~30 citations re-pointed; prints re-verified at root — headline
  **std-3 ∪ {B1,B6,B7,B9,B11a}**, the `WordCertificate` itself **exactly std-3**, B5-K/B10-K
  nowhere). The pilot's writing-down surfaced: ⚠⚠ **the CB lane's "COMPLETE" is 9-of-11
  clause suppliers** — candidate-side `gaussZ_*` have NO supplier (`hdet` binder; CB1's
  priced gauss tickets never opened) — caveat attached to CB-6's row; a **missing datum**
  in the marked-core composite (`piAb`+`hpiNu` — `toAbK` doesn't factor through G_K(2));
  and the **320k-unfold elaboration trap** (pin every `{α h q n}` implicit), broadcast to
  the in-flight AS3-b and folded into AS5's brief. **AS5 DISPATCHED on FABLE** (as
  worktree, branch `dyadic-as5`: `Dyadic/Main.lean` + `literature-axioms-dyadic.md` + WW5
  acceptance appendices + check-5 extension — the last stop before G3) and **AS-F
  DISPATCHED on OPUS** (lg worktree, branch `dyadic-asf`, BOUNDED feasibility-first:
  literal-field objects for the five quadratic fields, `hdeg`/`hqK`/`ramified` witnesses;
  an honest "mathlib lacks X" report is an acceptable outcome). **Owner-visible queue
  from AS2's findings, deliberately NOT self-dispatched (scope calls): (a) the
  candidate-side Gauss lane (~1900+ ln, discharges `hdet`), (b) the candidate R-stage lane
  (`hsplit`/`hZcount`), (c) ASK's K-side counting leaves (`hexact`/`hstokes`), (e) the
  `piAb` direct-factor lemma.**
- **2026-08-02 (orchestrator, CB-TRN — the "bounded nice-to-have" OVERDELIVERED):** merged
  `0cc4828`, registered; verification green (3508 jobs exit 0, both audits, census 11, twins
  identical). The low-priority transport ticket returned: **`SourceDataN.transport` fully
  generic over any `ContinuousMulEquiv` (all 21 fields, exactly std-3)**, the unconditional
  n=1 `wordCertificateLSq` (prints exactly `sourceR_N`'s list — packaging costs zero
  axioms), and a **bonus THIRD unconditional proof of the n=1 theorem**
  (`candidateGroup_lSq_equiv_absGalQ2_via_wordCertificate`) whose print I re-verified at
  root as **byte-identical to `main_presentation_literal_roe_unconditional`** — the first
  route whose conclusion never mentions `GQ2.GammaR`. AS5's five-branches-plus-n=1
  uniformity is now executable. No trap-5 collision this time (the file touches nothing
  CB-DD deleted). Doc consequence: AS4's `QTwo.lean` inventory table and
  `CertificateMain.lean`'s obligation table are stale in two more rows — **folded into
  AS3-b's Task 3 by orchestrator message** (AS3-b already owns the QTwo docstring edit;
  CertificateMain text-only added to its remit). Remaining fleet: AS2 (fable) + AS3-b
  (opus) in flight; AS5 next on AS2's landing; MC-CoV-split still held behind AS3-b.
- **2026-08-02 (orchestrator, AS3 — THE QUADRATIC TABLE LANDS, AND "INSTANTIATION ONLY"
  WAS FALSE):** merged `39565dd` + orchestrator commit `c2df453` (registration + trap-5
  repair); verification green (build 3507 exit 0, both audits, census 11, twins identical).
  Four packet-Thm-1.1 headlines at √2/√5/√10/√−10, **all printing std-3 + exactly
  {B1, B6, B7}** — the first non-ℚ₂ instances, surface strictly smaller than the frozen
  capstone's, identical across the four fields, re-verified at root post-merge. The row's
  "instantiation only" premise was WRONG: per-branch `CoreReindex` dictionaries existed
  only for compact N — AS3 built compact-M (factors through `nReindex` via a triangular
  twist) and procyclic-M (does NOT factor; the σ-offset convention surfaces at dictionary
  level) from scratch. Trap 5 fired exactly as planned: 33 citations of CB-DD's deleted
  trio, re-pointed by the orchestrator in `c2df453`, headline prints verified unchanged.
  **MC-CoV-split's hold condition (AS3 merged) is met but the hold EXTENDS behind AS3-b**,
  which owns the instance files the split's consumer updates would touch. Dispatched:
  **AS3-b** (opus, ww worktree, branch `dyadic-as3b`) = ν-satisfiability witness (the
  `maxProPQuotient` commutativity lemma authorized as NEW-DECLS-ONLY in its foundational
  home + discharge `hnuSigma`/`hnuWild` at all four rows — makes the headlines demonstrably
  non-vacuous, G3-relevant), the core hoist (`MCompactCore`/`MProcyclicCore` → a proper
  home as a PURE MOVE preserving fully-qualified names), and CB-DD's deferred
  `Instances/QTwo.lean:69` stale-row one-liner.
- **2026-08-02 (orchestrator, CB-DD — the dedup held the WAH/WWH standard, and the copy
  count was wrong in BOTH directions):** merged fast-forward `4e2759e` (6 files,
  +106/−130); full verification green (build 3503 exit 0, `check_dyadic`, `check_axioms`
  census 11, twins identical). **8 duplicate declarations deleted, 9 consumers re-pointed,
  zero statement changes, zero renames**; all 162 decls of the 5 touched files
  print-audited before/after — the diff is exactly the 8 deletions. Findings worth
  scanning for later: AS4-b's fourth `topGen_gammaR` copy hid under a different name
  (`Count.gammaGen_topGen`); `GQ2.Roe.topGen_gammaR` is a same-name DIFFERENT theorem
  (not a copy); the two `HalfTorsor*` inline copies are kept by POLICY not Lake (the probe
  compiled — but frozen-ℚ₂ A6, and the redirect would create the tree's only ℚ₂→Dyadic
  import edge); the trivial-action SHAPE has 12 sites repo-wide, the 7 uncounted being
  deliberately different (global `instance`s at concrete groups vs the canonical
  anti-shadowing `def`). ⚠ the same stale obligation row CB-DD fixed in
  `CertificateMain.lean` survives at `Instances/QTwo.lean:69` (lane off-limits mid-flight)
  — orchestrator applies the one-liner after the AS wave merges.
- **2026-08-02 (owner + orchestrator): LOW-PRIORITY WAVE RELEASED — OPUS ONLY.** Owner:
  "feel free to start low priority jobs as long as they're running on Opus." Dispatched in
  parallel with the in-flight AS wave: **CB-DD** (ob worktree, branch `dyadic-cbdd`; the
  WAH/WWH standard is binding — zero statement changes, zero renames of survivors,
  consolidation home `GammaRHom.lean`, import direction verified with Lake before any move,
  partial dedup acceptable with documented reasons; plus the `CertificateMain.lean`
  stale-obligation-table fix) and **CB-TRN** (lg worktree — warmest cache — branch
  `dyadic-cbtrn`; explicitly BOUNDED: it buys only the n=1 `WordCertificate` uniformity for
  AS5's five-branches-plus-n=1 story, the n=1 theorem being already proved twice, so any
  structural wall = stop and report, an impossibility note is a fine outcome). Two holds,
  both reasoned: **MC5-swap has NO dispatchable remainder** (its row: the code swap is
  IMPOSSIBLE by build cycle, docs half landed — the residue IS MC-CoV-split), and
  **MC-CoV-split is HELD until AS3 merges** (it re-signatures the six compact-M certificate
  sites AS3 is instantiating against right now; dispatching it concurrently would
  manufacture an interface collision worse than trap 5). Merge policy: completion order,
  AS-first tie-break; duplicate-name one-liners repaired at merge per trap 5.
...
| ASK | K-side `*Local` supply package (B6/B7 enter HERE, not in the spine) | opus | `GQ2/Dyadic/Instances/KSupply.lean` | SD1 ✓, LG5 ✓ | **done 2026-08-01** (3 commits → merged, registered, 3494 jobs green, census 11, capstone twins identical; 482 ln, 20 decls, 0 sorries, 0 new axioms; ⚠ **`SourceDataN` has 21 fields — the board/memo prose UNDERCOUNTED**; **10 proved at Γ = G_K**, 11 carried (`pro2`/`ker_pro2`/`hpro2`/`nu_compat` + the nine recursion leaves); `compat` is **derived** from B10-K, not assumed, so **merge gate 6 holds**; `surj` is literally the same F3 theorem the candidate side uses; **endpoint `candidate_equiv_galK_of_supply` = packet Thm 1.1 at a supplied K** (AS1's `2 ≤ q`/`Even q` discharged from q_K = 2^f), and `source_carrier` collapsed to `refl` — it was only data because ASK was queued; **the one piece of new mathematics is the scalar block at G_K**: `card_hom_zmodTwo_galK : #Hom_c(G_K,𝔽₂) = 2^([K:ℚ₂]+2)` via CB-2's generic `Hom_c ≅ Z¹` then `≅ H¹` (trivial action ⇒ no coboundaries) + FD1's Euler count, so **`ScalarHilbertCertificate` is PROVED, not carried** — note the asymmetry, CB-2 left `cardH2` structurally short on the CANDIDATE side, so this clause closes at G_K and not at Γ_R; **axiom surface of the K-side main theorem is exactly {B1, B6, B7} — STRICTLY SMALLER than the frozen `thm_4_2` capstone's set**, and **B5-K/B10-K appear nowhere** (parametrized over the bundles, per AS1's posture); ⚠ **board correction — SD-R3's five-item cut is right for BUDGET but its single framing conflates two different reasons**: the four `prop_8_9*` capstones genuinely need no clone (two-sided restatement dissolves the B-side special-casing; `closedRecursionK_of_source` is the one-sided generic producer SD3 applies twice), but **`rhoPrime_surjective` is NOT structurally unnecessary — it is ALREADY CLONED as `GQ2.Dyadic.rhoPrimeK_surjective` (`Recursion/Bridge.lean:159`) by SD-R3 itself** (its own "budget-grep false negative" rescue); **design finding on the `stokes` binder: the right follow-on is a Γ-GENERIC `prop_5_16` over a `TateDualityG Γ 2` bundle + an Euler-char hypothesis, NOT a K-clone** — both ℚ₂ inputs are available at K, LG2's `PairingK.lean` already generalized the neighbouring layer that way, **and the generic version serves the candidate side too** (~1.3k ℚ₂ lines avoided); the `determinant` binder deliberately does NOT consume `local_gauss_K` — LG-K's *arithmetic* is closed but the `GaussZ/FinalD.lean` plumbing has no K clone, and inlining a hypothesis named after LG5 would have **hidden** the gap (SD1 §9 applies: state the K bridge at `|V| * SN.h1Mult |V|`, **not** `|V|²`); ⚠ **gotcha for AS2–AS5**: `MarkedRecipBundle`'s R6 instance-path trap is live — `GalK K` and `↥K.fixingSubgroup` are the same type by different instance paths, so `rw [FieldData.card_H1_zmodTwo K]` FAILS where `exact` succeeds; statements at `↥(galKProfinite K)` need `show` before any `rw`) |
| CB-SG | make `prop_5_16` Γ-generic | opus | `GQ2/Dyadic/LiftingDualityG.lean` (renamed from the dispatched `Phase/StokesGeneric.lean` at merge — namespace/path now match, per CB-SG's own recommendation) | ASK ✓, LG2 ✓ | **done 2026-08-01 — AND IT REFUTED THE NAIVE CLAUSE** (3 commits → merged, renamed, registered, 3497 jobs green, census 11; 709 ln, 34 decls, 0 sorries, no new axiom); ⚠⚠ **THE NAIVE Γ-GENERIC CLAUSE (ii) IS FALSE, WITH A LEAN WITNESS**: the ℚ₂ Euler characteristic has factor `2^{v₂(#A)}` but at `G_K` it is `2^{d·v₂(#A)}`, `d = [K:ℚ₂]`, so the true value is **`#Z¹(A) = #A^{d+1} · #fixedPts C (A′)`, NOT `#A²·#fixedPts`** — `card_Z1_zmod2G_eq_eight_iff` proves the ℚ₂ shape holds **iff d = 1**, i.e. **a verbatim K-clone would have shipped a FALSE clause for every K ≠ ℚ₂**; two independent in-repo corroborations: the true value is ASK's own `card_hom_zmodTwo_galK` reached by a different route, and **`standardNumerics` ALREADY has `h1Mult V = V^n`, `tMult T = T^{n+1}` — `StokesDualityCertificate`'s clauses 1 and 4 are EXACTLY the degree-shifted clause (ii); the abstract numerics were designed for this and nobody had derived it**; the generic statement adds only `D : TateDualityG Γ 2` and `hE : LocalEulerChar Γ d` over the ℚ₂ signature — **no G_K-only hypothesis** — and §9 proves faithfulness (`localEulerChar_absGalQ2` IS B7; `prop_5_16_of_gen` re-derives all six ℚ₂ clauses verbatim, `#A²` included since d+1 = 2), so **the generalization is not weaker**; **measurement (as asked)**: the real substrate is `LocalLiftingDuality.lean` at **601 ln / ~25 decls, not 1.3k** (the 1.3k is `Phase140/Local.lean`, which this ticket does NOT replace) — **11 ℚ₂ decls were genuinely ambient-free and are used VERBATIM at zero cost**, 21 retyped (19 mechanical, 2 needed real thought), 13 new; **bonus finding: `smul_muN_two_trivial` is not arithmetic** — its proof uses only `#μ₂ = 2`, so **every** group acts trivially on `MuN 2`, LG4a's anchoring convention is satisfied vacuously and no `Γ →ₜ* GalQ2` is needed anywhere in §1–§8; clause (iii) and the cup skeleton are **Euler-free** (B6 alone); **the payoff is BIGGER than ASK scoped it — the same four pieces are consumed by `MStageCount.lean`, `RStage/Local.lean` and `Half139Local.lean`, so `prop_5_16_gen` serves ASK's `exactLifting` binder too, not just `stokes`**; ASK's `stokes` binder is **NOT discharged but its only ambient-mathematical blocker is gone** — what remains in those 1108 lines is carrier plumbing (104 `AbsGalQ2` mentions) plus two conclusions that must change shape to `SN.tMult(#T)·#fixedPts` and `#V·SN.h1Mult(#V)`, precisely what `StokesDualityCertificate` already asks for; all three briefed traps were real (instance-path hit TWICE, routed around via `localEulerCharacteristic_open (GalKsub K)` — **cleaner than `@`-threading**; `smul` trap dodged; anchoring vacuous); written `module`-style deliberately, since plain-import would have trapped the generic result where no module file could consume it |
| **CB-SG-Q** | ⚠ **OWNER DECISION — CB-SG corrects ASK's recommendation on the candidate side.** ASK's case for going generic rested on "the generic version serves the candidate side too". The generic **statement** does apply at `Γ = GammaR n q R` (verified — no hidden G_K hypothesis), **but the candidate side cannot discharge two of its hypotheses today**: (1) the `MuN 2` action is free (§1 proves any such action is trivial, so declaring the trivial one is canonical and can never clash); (2) **`TateDualityG (GammaR n q R) 2` is the real cost** — the candidate side has `IsDemushkin 2 G` (`dim H¹ < ∞`, `#H²(𝔽₂) = 2`, cup nondegenerate on `H¹(𝔽₂)`), but `TateDualityG` needs μ₂-coefficient perfectness in **all three degree pairs for every finite 2-torsion module**, and the only producers in the repo are the **B6 axiom `tateDualityAt` (gated on an open finite-index embedding into `G_ℚ₂` — i.e. exactly what the campaign is trying to prove, so CIRCULAR here)** and its two derived kernels; **Demushkin ⇒ PD²-at-2 (Serre GC I §4.5, NSW 3.7.4) is NOT in the repo**; (3) `LocalEulerChar (GammaR n q R) n` — follows from (2), or directly from the one-relator pro-2 presentation. **Two options, CB-SG deliberately did not choose: (a) prove `IsDemushkin → TateDualityG` for the candidate carrier, or (b) weaken `prop_5_16_gen` to whatever the residues actually need at the scalar level.** | — | — | CB-SG ✓ | **OPEN — owner call** |
| CB-DET | the K-side (83)-evaluation bridge, LG5 → `GaussZResidueK` | opus | `GQ2/Dyadic/GaussZ/FinalDK.lean` | ASK ✓, LG5 ✓ | **done 2026-08-01 — ASK's `determinant` BINDER IS DISCHARGED** (7 commits → merged, registered, 3501 jobs green, census 11; 1004 ln / 616 code, 19 decls + an acceptance `example`, 0 sorries, no new axiom); §8's `example` has `KSupply.determinant`'s field type **character-for-character** and is closed by `affineDeterminant_galK`; **the degree-shift audit: three numeric constants, TWO MOVE, one does not — and `standardNumerics` was CORRECT on all three, no refutation**; ⚠⚠ **the substantive finding is sharper than SD1 §9's flag**: the ℚ₂ `#Z¹ = #V²` **is not a squaring law at all** — read forwards it is a theorem with **no numeric input**, `card_vcocycle_eq_mul_card_H1 : #Z¹ = #B¹ · #H¹`, from `B¹ ≅ V` (freeness) and `Z¹/B¹ ≅ H¹`, **both already Γ-generic in the ℚ₂ files**; `#H¹ = #V` holds **only in degree 1**, and at K LG5's Euler clause gives `#H¹ = (#V)^n`, hence `#Z¹ = #V^{n+1}` — so **`SourceNumerics.h1Mult V = V^n` is FORCED, not chosen**, and `hZcard`'s `#V * SN.h1Mult #V` is exactly right; **a verbatim `#V²` clone would have been false for every K ≠ ℚ₂**; ⚠ **this independently CONFIRMS CB-SG by a different route** — CB-SG derived `#Z¹(A) = #A^{d+1}·#fixedPts` from the Euler characteristic, CB-DET derives `#Z¹ = #V^{n+1}` from `B¹≅V` + `Z¹/B¹≅H¹` + LG5's Euler clause, and **the two coincide where they overlap** (`#fixedPts = 1` on the Gauss lane's simple modules); the Gauss value's `(−1)^n` is **not a sign convention** but LG5's parity split (`arf = n mod 2` when unramified), which is why the unramified twin case-splits on `Even n` while the ramified twin has no parity condition at any degree; all three pinned `rfl`-equal to `standardNumerics`; **structural finding: this lane is DECOUPLED from `Phase140/Local.lean`** — the ℚ₂ chain used `hZcard_local` **only** for `Finite Z¹`, never for the value, and §2 gets finiteness from `#V ≠ 0` + `#H¹ ≠ 0` (LG5's Euler clause), so **the (83)-bridge needs no K-clone and no generic successor of `Phase140/Local.lean` — CB-SG's lane and this one do not block each other**; **measurement**: the ℚ₂ chain is 35 decls / 885 code lines, of which **24 of 35 are ambient-free and used VERBATIM** (all 15 of `Reduction.lean` — its entire 206 lines cost nothing), 5 retyped, 6 **replaced rather than cloned**; mine is 616 code lines, **nowhere near 1.3k**; axioms: `affineDeterminant_galK` = std-3 ∪ {B6, B7, B9, B11a} with **B6 entering exactly once, where the bundle is built** — a subset of ASK's {B1,B6,B7} ∪ LG5's, so **the discharge does not enlarge the K-side main theorem's axiom surface**; ⚠⚠ **NEW TRAP — broadcast to AS2–AS5**: `ContinuousSMul ↥U (MuN 2)` **fails to synthesize at a concrete `U = GalKsub K`** even though each half synthesizes alone (its `SMul` argument is pinned by the preceding `DistribMulAction` binder while mathlib's `Subgroup.continuousSMul` concludes at the `SMulMemClass` path — defeq but **not reducibly** so, and instance search runs at reducible transparency); **every consumer of `local_gauss_K` at a concrete K will hit this, and nobody had consumed it before**; fix = name the two binders and pass them explicitly. Second variant of ASK's R6 trap: `sourceBoundaryMapK` takes its source as a bundled `ProfiniteGrp`, which drags that topology into elaboration and poisons downstream synthesis — `boundaryMapSub` (§7) is the firewall; **design note (orchestrator)**: to *replace* `KSupply.determinant` rather than keep it carried, `KSupply` gains two pin fields `hgu`/`hgr` alongside `hdeg`/`hhom` — or they are supplied at AS2–AS5 instantiation, where they are `rfl`; CB-DET did not edit `KSupply.lean`, so **`KSupply` §6 item 4 is now STALE** |
...
| MC-CoV-split | **OPTIONAL tidiness, not on any critical path**: split `CompactCoV` at §6 (the discharge below, the three MC5-facing restatements above) so `Certificate.lean` can import the discharge and the compact-`M` sites cite the theorem instead of taking the datum. MC5-swap established this is the only unblocking route. **Deliberately not dispatched**: MC-M's residual list is already correct without it, and AS3 can discharge the datum at its instances directly from `CompactCoV`'s landed `mCoVPin_discharge_sqrt_{two,five}` — the split changes ergonomics, not what is provable | opus | `GQ2/Dyadic/MarkedCore/CompactCoV.lean` + the two consumers | MC5-swap ✓ | **done 2026-08-02 — THE SPLIT LANDED, THE ADOPTION REFUTED TWICE OVER** (2 commits `187c35d`/`47bfa91` → merged ff, registered + debt-note amendment; new `MarkedCore/CoVDischarge.lean` 551 ln, 30 decls, 0 sorries, all std-3; `CompactCoV` 590 → 104 ln with its public surface unchanged via `public import`; verification green — build 3511 exit 0, both audits, census 11; **all 9 protected headline prints byte-identical before/after**). The split is independently correct and KEPT (orchestrator judgment call): the discharge layer needs `public import …MarkedCore.M` ALONE — Lake-verified; it had sat two levels too high by accident of authorship — and `Certificate.lean` importing it builds green (3015 jobs), so **MC5-swap's build cycle is retired as a live constraint**. ⚠⚠ **The adoption half is REFUTED with witnesses, and the ticket's premise was wrong twice**: (a) all six certificate sites are in `Certificate.lean` (none in `M.lean`) and all six are `{α h}`-generic with `hα : 1 ≤ α` — the datum CANNOT be swapped for the discharge: at `h = 1` the citation is a TYPE ERROR (`DM α 1` vs `DM α 0`; MC3's frame API is rank-four only; at variable `?h` it diverges, the trap-5 pattern), and at `α = 1` the discharge is **FALSE, not merely unavailable** — Lake-checked witness: `mUnit 1 = −1 = χ_M(B̄)`, so `chiM 1 h (B̄D̄) = 1` is a χ-trivial direction missing `C̄₀` while `mNu_frame` leaves the triple free (`Certificate.lean`'s deliberate general-`h` posture is now PROVED, and sharpened); (b) **there are no consumers to update** — repo-wide grep puts every caller of the M-side entry points inside `{Certificate,M,CompactCoV}.lean`; `Instances/Cores.lean` does not even import `MarkedCore.Certificate` (instances reach `GalK` via `KSupply`). No signature changed; `Certificate.lean` byte-identical in code, docs corrected (the now-false "can never be an import downward" retired). ⚠ optional fold-in SKIPPED with a finding: **AS3-b's recorded homing destination for `mRelWord_of_commute` is unreachable** — its proof needs `commP_eq_one_of_commute`, which lives ABOVE `MarkedCore/Cores.lean` in `HandleMix.lean` (only other copy is `private` in `Roe/Labute/Levelwise.lean`); in-file note amended by orchestrator (correct homes: beside the dependency in `HandleMix.lean`, or move it down first — wide rebuild); consumer side free (single unqualified use under an `open`). → new ticket candidate recorded, NOT dispatched (no consumer yet, would be gold-plating): compact-row `_of_chiKer` companions for the 3 uncovered entry points, notably the K-facing `marked_matching_certificate_KM` — now cheaply addable at the right altitude |
...
| CB-FR2 | the forced follow-through | opus | `GQ2/Dyadic/Count/Frozen.lean` | CB-FR ✓ | **done 2026-08-01** (8 commits → merged; 1095 → 1315 ln, 79 decls, **79/79 std-3**, 0 sorries, no new axiom (two kernel `decide`s added for witness computations); **the procyclic-N row now resolves with NO side condition at all** — `npcResolver` branches on the exponent (`omega2Exp N` at ω₂, `1 + padicOmega2Exp (η−1) N` at η̂) and `resolvesAt_npcFamOf` carries **exactly the same pair** `hN ≠ 0`, `hord` that the four ω₂-only rows carry: no 2-group restriction, no η-congruence, no relation between branch datum and target; the exponent-level form has **zero** hypotheses; `npcFam` is untouched (`npcFamOf_const` is `rfl`) so nothing was re-pinned; ⚠ **`orderOf ∣ 3` is NOT dischargeable — it is FALSE**, refuted at §6's generality using **the campaign's own witness** (`CardH2GammaA.datum`, whose `Bg ⧸ D.M` has order 2); `TameQ.odd_order` gives `Odd`, not `∣ 3`, and only for the τ-image — **and it is not needed**: `orderOf_wordLift_radT_dvd_exponent` is hypothesis-free at level `2·exp(Bg ⧸ D.M)`, which is automatically nonzero AND even, the evenness being the lift level itself; **the `6` was never a fact about the target, only the arithmetic a CONSTANT resolver needed to be honest**; mpc `.hat` closed cheaper than expected (one resolver serves both rows — the `.hat` exponent IS `EtaData.toZhat`, and `epsZ_mpcW` was already display-generic); CB-FR's two negatives still bound the claims (both are about CONSTANT resolvers; `npcResolver_ne_const` proves the escape is real); **§10 gives all five rows as matched `(hres, hend)` pairs at one arbitrary nonzero even level — nothing mathematical remains on the target side**) |
| CB-LV | level retarget in `Count/Presentation.lean` | opus | `GQ2/Dyadic/Count/Presentation.lean` | CB-FR2 ✓ | **done 2026-08-01 for item 1; items 2–3 CORRECTLY REFUSED as mis-scoped (orchestrator dispatch error, see below)** (2 commits → merged, 3495 jobs green; 92+/30−, all 37 decls std-3-or-subset, no new axiom, no sorries); **item 1 done**: the four §8 pilots now take a *generic* level — `sqrtNegTwo_{tcocycle_card,hZcard}_gammaR` take `{e} (he : Odd e)` instead of the pinned `e = 3`, and the `_nCompact` pair takes `{N} (hN : N ≠ 0) (hv : N.factorization 2 ≠ 0) (hord : ∀ x, orderOf x ∣ N)` instead of the literal `∣ 6`, with `Odd (omega2Exp N)` coming from `omega2Exp_modEq_one` read mod 2 (**inlined deliberately, not named — `Count.odd_omega2Exp` already exists in `Frozen.lean` in the same namespace and a second decl would collide**); the wrappers now compose `nCompact_{tcocycle_card,hZcard}` directly because `Compare.lean`'s `sqrtNegTwo_*` **pin `e = 3` in their own statements** and that file is not CB-LV's; **"no theorem was weakened" was verified MECHANICALLY, not asserted** — all four pre-CB-LV statements re-derived verbatim from the new ones (`e := 3`, `N := 6`) and compiled sorry-free at std-3; ⚠ **DISPATCH ERROR (orchestrator): items 2–3 were sent to the wrong owner.** `Count/Frozen.lean:6` **imports** `Count/Presentation.lean`, so Presentation is UPSTREAM of Frozen and cannot see `resolvesAt_and_endpoint_*`, `splitLevel_ne_zero_and_even`, `orderOf_wordLift_*`, `mpcFamOf` or `npcResolver` — nor the four other branch families' `Certificates/` files. Doing them in Presentation would mean re-proving Frozen §3/§6/§8 under second names. **CB-FR2's own list was right that these are "other owners'" edits, but only item (3) was Presentation's; (1) and (2) are Frozen §10 items — i.e. CB-FR2's OWN file.** CB-LV stopped rather than improvise (as briefed) **and wrote + compiled all five in scratch so the follow-up is copy-paste**: `/private/tmp/.../scratchpad/CblvSplit.lean`, all sorry-free at std-3; ⚠ **CB-FR2's "needs `[Finite E]`" UNDERSTATES the `Vmod` twin**: `splitLevel_ne_zero_and_even` is stated *only* for `Bg ⧸ D.M`, so the V-side level `2 * Monoid.exponent E` has **no nonzero/even lemma at all** — the fix is one generalisation `twoMulExponent_ne_zero_and_even` with **`[Group G]`, not `[Monoid G]`** (`Monoid.exponent_ne_zero_of_finite` wants `LeftCancelMonoid` — CB-LV hit this), after which the twin is three lines and `splitLevel_ne_zero_and_even` becomes its `Bg ⧸ D.M` instance; note the pilots are now level-*generic* rather than instantiated at the split level — making them hypothesis-free in the target is a **sixth** Frozen-side corollary, not a Presentation-side one) |
| CB-LV2 | the split instantiations, re-homed to `Frozen.lean` | opus | `GQ2/Dyadic/Count/Frozen.lean` | CB-LV ✓, CB-FR2 ✓ | **done 2026-08-01** (2 commits → merged, 3497 jobs green; 79 → 87 decls, **86 std-3 + the pre-existing `[propext]` subset preserved**, no new axiom, 0 sorries); landed `twoMulExponent_ne_zero_and_even` (`[Group G]`), the four split-target instantiations + the `Vmod` twin (three lines each, CB-FR2's template), and **item 5 turned out reachable and genuinely a corollary** — as TWO corollaries, not one, because CB-LV had made both pilots level-generic so the V-side twin was free; **`splitLevel_ne_zero_and_even` DID become the `Bg ⧸ D.M` instance** (`:= twoMulExponent_ne_zero_and_even _`, one line, statement byte-identical, no consumer disturbed — there are no longer two proofs of the same fact); **both `SourceDataN` field values now hold over Γ_R with NO arithmetic hypothesis about the counting target** (`hN`/`hv`/`hord` all discharged; what remains is branch data, not target arithmetic); **the no-weakening claim was MEASURED, not asserted**: all 79 baseline decls dumped as `#check @X` + `#print axioms X`, the baseline `Frozen.lean` checked out and rebuilt, the same dump re-run and diffed → **empty diff, 79/79 byte-identical signatures AND axiom sets**, re-verified after §11 landed; ⚠ **methodology note worth keeping campaign-wide: the first dump silently dropped the LAST name because the name file lacked a trailing newline and `while read` skips a final unterminated line** — caught only by reconciling 78 axiom lines against 79 names; scratch transferred unchanged apart from campaign renames and `omit`/binder additions forced by §10's section context, no proof term altered; **owed on the resolver/endpoint side: NOTHING on the T-side target** (all five rows + the 4' `.hat` display are matched pairs at the count lane's own split level; surviving hypotheses are conditions on the WORD, not the target) — open: V-side twins for rows 2–5 (three lines each, purely a question of demand), pilot compositions for rows 2–5 (a `Presentation.lean`/CB-4 item, upstream), and `hd : StokesDuality` which is genuine branch data |
| CB-2 | the scalar block — the first clause ticket | opus | `GQ2/Dyadic/Count/Scalar.lean` | CB-1 ✓, CB-S ✓, CB-FR ✓ | **done 2026-08-01** (8743003…9ac744c, 5 commits → merged, import registered; 606 ln vs the 900 estimate — **30% UNDER, not over**; 29 decls, **29/29 exactly std-3**; **`homCard` CLOSED** with the verbatim field goal discharged; **`cardH2` reduced to exactly ONE open argument** (`hcomp`); `lem86` reduced (conclusion verbatim, different hypothesis shape); **CB-S's "risk moved here" was right on location, wrong on which clause, and the ticket was SMALLER not bigger** — the block's real content is a single input `#H²w(𝔽₂) = 2`, one line off WW3b's `card_wordH2`, and what made it cheap is a fact nobody had recorded: **at scalar coefficients the marking CANNOT act** (𝔽₂ has one additive automorphism), so `d⁰ = 0`, both `H⁰w`s are 2 for free, no generation hypothesis is needed (the scalar block is exempt from WW3b's second caution), and CB-S's clause 2 collapses to `2^{n+2}` = `homScalar` on the nose — corollary: **`smulZmod2`/`htriv` carry no information at this coefficient module**; NO count and NO duality re-derived; **new and closing a documented hole**: `homEquivZ1`, the `Hom_c(Γ,𝔽₂) ≃ Z¹(Γ,𝔽₂)` bridge, which existed in no form (the ℚ₂ `lemma_8_2_*` count characters by hand off a `Fin 4` presentation and never meet `Z¹`) — `CertificateMain.lean:264-272` said the candidate side's scalar rows "are not yet a `#Hom` count"; that is now closed; **`hsep`/`hpartial` do NOT belong here** (neither factors through the scalar block — `hsep` has two genuinely disjoint ℚ₂ proofs, `hpartial` forks at one stage) — they stay with CB-4; ⚠ **TWO FINDINGS**: the **missing H² rung** gates three clauses at once → CB-H2, and **CB1's memo schedules `lem86` wrongly** (it puts it in CB-3 as independent of the scalar block, but `half_count` consumes `#H² = 2` directly, as does `stageR136` — **CB-3 must not be scheduled as independent of CB-2**); **revised lane sizing: ≈4,000–4,400 generic lines remain, not the board's ~7,700** — but with a serial dependency the board does not show, and **`hsep` (~1500+, carrying the `hsimp` decision) is where the displaced risk actually landed**) |
| CB-H2 | the count lane's degree-2 rung | opus | `GQ2/Dyadic/Count/HTwo.lean` | CB-2 ✓, CB-1 ✓ | **done 2026-08-01 — CRITICAL PATH CLEARED** (6 commits → merged, registered, 3493 jobs green; 827 ln (CB-2 estimated 800–1200, so low end), 44 decls, **16/16 headlines std-3**, 0 sorries, **zero import delta**; `h2Word : H²(Γ,𝔽₂) ↪ WordH² c w` is degree-generic — no `AbsGalQ2`, no B6, `Γ` any profinite group carrying CB-1's marked presentation — with `h2Word_injective`; **`hcomp` DISCHARGED** (`cardH2_comp` verbatim) and **`cardH2_field_goal_closed` states CB-2's field goal with NO `hcomp` argument**; **route = central extension, NOT five-term, and CB-H2 recommends it over CB-2's sketch**: five-term needs `H²(F̂,A)=0` (cd 1 for free profinite groups) and the repo has no inflation–restriction, no relation module, and **neither does vendored mathlib** (confirmed) — the central-extension route computes the same map without ever constructing `R`; **the decisive find: most of it already existed** — MC-OB's `Word/WordCoh.lean` had level-factoring, `CentExt` and coboundary extraction built generically in the profinite group, and is already in `Count/Scalar.lean`'s closure; CB-2's two constraints hold exactly as stated but sit at the TOP of that stack, not in it: **`IsFrattini` (one relator) is what lands `obs` in 𝔽₂ on the nose — delete it and the error term that appears IS `d¹`, so multi-relator is not a generalisation but the same computation with the error kept, and `WordH2` is the quotient by it** (`pRelZ_coboundary`, the file's one new idea); `IsProP 2 G` is replaced by `isWildTwo_centLift`, the exact twin of CB-1's `isWildTwo_foxLift`; **CB-2's scalar simplification carries to degree 2 and is LOAD-BEARING**: at trivial action `WordLift` is a direct product so the offset coordinate is a hom (`splitU`), `d¹` cannot see the marking, and the correction term needs identifying **only at the two-element target**, which CB-1's standing `hres` already gives — this is what keeps `ResolvesAt` out of the construction, and it matters because **a resolution at every level would have been FALSE (CB-RES)**; two honest extra hypotheses, both flagged in-file: `hwildLevel` (= CB-1's own `hwild2` at every level, part of `GammaR`'s definition) and `Nontrivial (H2 Γ (ZMod 2))`, the latter **unavoidable by ANY comparison theorem** since the rung is an injection not an isomorphism (cokernel = module of identities among relators; `H²=0` is consistent with everything else in sight — a free profinite Γ satisfies it all)) |
| CB-VAR | the nonzero variation class, per branch | opus | `GQ2/Dyadic/Count/Variation.lean` | CB-H2 ✓ | **done 2026-08-01 — THREE CLAUSES UNBLOCKED AT ONCE** (9 commits → merged, registered, 3499 jobs green, census 11; 1412 ln, 60 decls, **25/25 checked headlines exactly std-3**, 0 sorries, no new axiom); **item 2 `hwildLevel` discharged in 12 lines** — CB-H2 was exactly right that it is GR1's `isPGroup_two_wildNormalClosure` read through `image_wildAlphabet`; the pro-2 clause is part of Γ_R's *definition*, so the level form costs one set identity; **item 1**: `exists_nonzero_varCoc` is Γ-generic — from `D.NoDescent` + surjective ρ it produces a `TCocycle` in `lem86N`'s verbatim `hvar` shape — and **all five frozen rows plus the mpc `.hat` display land with their four resolutions discharged at ONE level and NO hypothesis about any target**; **row 5 — the one CB-FR proved has no honest constant resolver at level 6 — costs NOTHING here, because the level is chosen at the target**; **two findings made it cheap**: (a) **§5a, the detection functional — the count half of the duality package is NEVER USED, only clause 3 (the pairing)**: at the scalars `∑_k` kills `im d¹` because the dual `d⁰` vanishes (`Aut(𝔽₂) = 1`, CB-2 §1 one degree up) and a Heisenberg word with zero dual offsets has zero centre, so nonvanishing is detected by the traced sum alone — **no `#H²w = 2`, no `IsSelfDual` count clause, no `h2Word`**; (b) **§2, the level question answers itself**: `A ⋊ C`, `A^∨ ⋊ C` and `𝔽₂ ⋊ C` are all *subgroups* of `H(A) ⋊ C` (three explicit injections), so one level `N = exp(H(A) ⋊ C)` kills all four targets the route reads, and **evenness is free from the Heisenberg CENTRE, needing no hypothesis on `A` at all — a better source than CB-FR §8.2's**; **item 3, the compositions, all landed as theorems**: `cardH2_of_variation`, `lem86_of_variation` (**CB-H2's scratch composition is now a theorem, and its hypothesis block is LITERALLY IDENTICAL to `cardH2`'s — CB-H2's "they share their one non-generic input", made syntactic**), `stageR136_of_variation`, plus an end-to-end pilot from ONE duality payload; **the count is an EXACT EQUALITY, not a bound** (`≤ 2` from CB-H2's rung ∘ WW3b's `#WordH²(𝔽₂) = 2`; §7 supplies the `> 1`) — nothing left as an inequality; bonus `tfg_of_isAdmissibleMarkedPresentation` gives `SourceDataN.tfg` free from clause (i) at any finite alphabet, a generic form the lane lacked; **remaining per row: only TWO things, both branch data not count-lane data** — `hdT`/`hdS` (**not a new payload**: every row's `*_stokesDuality` is `stokesDuality_of_simple` instantiated, so one `hsimp` gives both) and the source-free radical-cover datum (`CardH2GammaA.datum` reused verbatim; `RadicalCoverData` binds no Γ); `HeisLift` gets **local** ⊥ instances (no topology for it exists anywhere in the repo, so no diamond, and locality keeps consumers from seeing them) |
| CB-3 | `liftsOver_card` | opus | `GQ2/Dyadic/Count/Lifts.lean` | CB-2 ✓, CB-H2 ✓ | **done 2026-08-01** (6 commits → merged, registered, 3498 jobs green, census 11; 924 ln / 624 code, 23 decls, **23/23 std-3 or the strict subset `[propext]`**, 0 sorries, no new axiom; **≈600 estimate, 4% over**); `liftsOver_cardN` is **genuinely generic — no split**: one theorem quantified over the alphabet, the relator family `W : κ → PWord ι` (any arity, **including L's single relator**), the degree, and the carrier, with `hdeg` the only per-family number; the verbatim field type closes as `liftsOver_card_field_goal`; **confirmed NOT entangled with CB-VAR** (consumes only CB-S's `IsSelfDualN`, CB-1's presentation interface, and §7-block theory — nothing from `Variation.lean` or `Presentation.lean`), so the board's parallel dispatch was right; ⚠⚠ **THE DESIGN FINDING THAT MATTERS — the `G_ℚ₂` route to nonemptiness is UNAVAILABLE to this lane, and taking it would have blocked the clause**: `liftsOver_card_local` gets its base lift from `#H²(Γ, M_B) = 1` (B6), which needs a degree-2 rung at **module** coefficients, but **CB-H2's rung is scalar-only BY CONSTRUCTION** (`splitU` needs the split group to be a direct product — exactly the trivial-action fact CB-2 found). CB-3 took the **Γ_A route** instead (which `MStageCountGammaA.lean:28` documents precisely because Γ_A had the same problem) and it is degree-≤1 throughout: `#H²w(M_B) = #(M_B^∨)^{Y_C} = 1` puts the relator vector in `im d¹`, a correction kills the family, `hpres.extend` descends it — **no continuous H² at module coefficients anywhere**. **Any future clause scheduled as if CB-H2's rung generalises to modules must be re-checked.**; new mathematics = §2's correction law (the ℚ₂ files do this per relator with two hand-written arity-4 words; for an arbitrary `PWord` that would be Fox calculus with ω₂ exponents, a ticket of its own — **it isn't needed**: one evaluation pushed both ways via `PWord.map_eval` gives the law and the shift is read in the word complex where CB-1 already calls it `d¹x`, **no derivative is ever computed**) and §4's generic nonemptiness (did not exist in any form); §1/§3 re-derived **unavoidably** — the `M_B` module pack and `card_fixedPts_MB_dual` are `private` in **all three** ℚ₂ files and no frozen file may be touched to export them, so this is a fourth copy **but the last**, since everything above it is abstract; **CB1 memo divergence 6 ANSWERED**: `d¹` enters only through `StokesDuality`/`IsSelfDualN`, never through a `FoxCertificate` record, so **L's missing Jacobian does not reach this clause**; **no binder is private to this clause** — beyond `hdeg` the inputs are exactly the two `tcocycle_cardN`/`hZcardN` take, and `hd` at `M_B` is not new either (`M_B` is 2-torsion, so WW3's `stokesDuality_of_simple` produces it from `hsimp`, the memo's Q3 residual all five branches already carry for `hsep`) — **`liftsOver_card` inherits `hsimp` and adds nothing to the census**; ⚠ **merge fix (orchestrator): `collapse` collided with CB-W's `Count/Wild.lean` `collapse` at the root** — genuinely different objects (CB-W's is an 𝔽₃-linear functional, CB-3's the semidirect-product collapse `A ⋊ G →* G`); **CB-3's own module built exit 0 because `Wild.lean` is not in its import closure**, so only the root build caught it. Renamed the newcomer to `wordLiftCollapse` (6 occurrences) at merge; **this is the third such collision in the lane** (after `wildLetters`→`wildAlphabet` and `finite_H2`→`finite_H2_of_presented`) — per-file green is NOT sufficient evidence of no collision; owed: four branch instantiations (M0/Npc/Mpc/L, ~10 ln each once each endpoint certificate is in hand) |
| CB-4 | `hpartial` and `hsep` | opus | `GQ2/Dyadic/Count/Separation.lean` | CB-2 ✓, CB-H2 ✓ | **done 2026-08-01 — BOTH CLAUSES CLOSED, AND CB-2's RISK CALL WAS INVERTED** (5 commits → merged, registered, 3500 jobs green, census 11; 1013 ln vs the **≈2200 combined estimate**, **15/15 decls exactly std-3**, 0 sorries, no new axiom, zero linter warnings); ⚠⚠ **`hsep` is the LIGHTER clause, not the lane's risk**: CB-2 sized it ≈1500+ and named it the real risk because its two ℚ₂ proofs have "no common generic core" — measured, `hsepN` carries **three** binders and needs **none** of `#H²(Γ,𝔽₂)=2`, `htriv`, `hpair` or simplicity, where `hpartialN` needs the first two. **CB-2 was right about the PROOFS (marking route and `cup20` route share nothing) and wrong about what a generic clause factors through, which is the STATEMENT** — spelled cup-free, `hsep`'s body loses its entire `cup20`/`H2mk` layer and stage 4 collapses to the observation that the invariant-dual pushforward of `tDef` **is** `chiDef` at the induced character, *definitionally*; corroborated from the ℚ₂ audit: `sep_word`, the marking route's engine, consumes **only** `IsSelfDual` clause 1 (= `cardH2`), **not** the degree-carrying count clause — **so the `#A^{n+1}` trap is provably OFF `hsep`'s path**; **degree check done for BOTH, three ways** (as briefed after CB-SG's refutation): (1) neither statement contains a cardinality at all — no `Nat.card`, no `SN`, no `n`, so there is no exponent to shift; (2) `SourceDataN`'s own field-delta docstring lists both under **"unchanged — verbatim"**, never "revalued", and every one of `SourceNumerics`' five moving leaves is a count these clauses do not read; (3) machine-checked degree-uniformity — CB-SG's cup lemmas carry `d` implicit and absent from the conclusion while `card_Z1_eqG` has `d` in the exponent, and **§2's `cup11_rightSepG` is sharper still: the right-slot half needs NO Euler hypothesis at all, so `hpartial`'s cohomological input is not merely degree-uniform but degree-FREE**; **genuinely generic across all five families, trivially so once stated correctly** — neither clause mentions the branch word, the marking, or `ι`/`ρ`, so there is nothing per-family to specialize, and **the same theorem serves Γ_R at all five frozen rows AND G_K on the arithmetic side** (CB1 memo Q7's predicted ASK shrinkage realised: §2/§5's `*_of_tateDualityG` suppliers are the whole arithmetic-side obligation); **`hsimp` was NOT needed, not added, not axiomatized** — neither clause carries a simplicity binder, and the ℚ₂ proofs of these two don't use it either (only `hZcard` and the Gauss leaves do), so the owner ruling was respected by not needing to invoke it; both field goals close **verbatim at one frame**, and **Lemma 7.1 `(M^∨)^C = 0` turned out frame-level and source-free**, so it is discharged as a THEOREM at the field goal rather than a binder, shared verbatim by both sides; **deliberate non-port**: the Γ_A *marking* route for `hsep` is `Fin 4`/`Marking`-pinned end to end and porting it would mean rebuilding `Marking.push`/`descend` at arbitrary `ι`/`ρ` — the cup-free binder makes it unnecessary (the marking route's *conclusion* IS the binder), **which is why the clause came in at 400 lines instead of 1500** |
| CB-5 | the word-side (candidate) suppliers of CB-4's forks | opus | `GQ2/Dyadic/Count/Separating.lean` | CB-4 ✓ | **done 2026-08-01 — (b) CLOSED; (a) landed as a lemma but its PREMISE REFUTED** (merged, registered, 3502 jobs green, census 11; 407 ln, **7/7 decls exactly std-3**, 0 sorries, no new axiom, zero linter warnings); **(a) `sepWordN` — the generic `sep_word` — closed, and the ~5–20 line estimate was right (9-line proof body)**, with two departures from the ℚ₂ ancestor, both simplifications: it is a **direct implication, not a contradiction** (`stokesDual_separating` **turned out not to be needed** — the hypothesis descends to `A^∨ ⧸ ker d⁰`, `kerLift`-injects, and `elemDual_extend` extends it; that IS `h2_inj`'s witness, where ℚ₂ instead picks a functional on `H²w` and inverts the trace via a counting surjectivity), and **consequently NO count and no 2-torsion of `A`** — ℚ₂'s `sep_word` takes `IsSelfDual`, `hA₂`, generation and both relator conditions; this takes `StokesDuality c w A` **and nothing else**, so the real input is `h2_inj`, **one step upstream of the clause the ticket named, with no cardinality in sight**; ⚠⚠ **BUT (a) DOES NOT CLOSE `IsTwoSeparating`, and the reason is structural, not a missing lemma**: `IsTwoSeparating Γ A` is about **`H²(Γ, A)` at MODULE coefficients** while `sepWordN` is about the **word** complex `WordH²(A)`; the bridge is a degree-2 comparison at module coefficients, and **CB-1 stops at degree 1 while CB-H2's `h2Word` is scalar-only BY CONSTRUCTION** (`CentExt` is central by 𝔽₂, `splitU` needs the trivial action, `obs` lands in 𝔽₂ on the nose because the target has two elements) — **this is exactly CB-3's standing warning, hit head-on**; corroboration that this is the true shape and not the agent's error: **ℚ₂'s OWN candidate-side `hsep_gammaA` never forms `H²(Γ_A, T)`** — it runs the marking route, so `sepWordN` is genuinely that route's engine; **note this refines CB-4's non-port ruling**: CB-4 was right that the cup-free binder makes the marking route unnecessary *for the clause*, but the **candidate-side supplier still needs it**; **(b) `isRightSeparating_of_selfDualN` closed, and SMALLER than billed** — **the predicted `heisEta1`-for-`mixedB_eq_relZPair` substitution was never needed, because CB-VAR built both missing pieces while this ticket sat queued**, in the *contrapositive* direction of the same ledger identity: `pObsFam_inflation` **is** the degree-generic `obs_inflation`, and `pRelZ_kappaHeisN` **is** the alphabet-generic `PWord`-valued `mixedB_eq_relZPair` (the `Fin 4` traced-sum matching replaced by one `PWord.map_eval`); so §4 is **re-use, not development**, and the only genuinely new code is §2's `pairHom` at an arbitrary module (CB-VAR's `Additive ↥D.T` pin was an instance block, never mathematics) plus §5's assembly; bonus: `hcompatD` is **derived** from `hcompat` where ℚ₂ carries both as hypotheses, and `ContinuousSMul Γ A` turns out unused; **nothing in this file reads a count** — no `Nat.card`, no `SourceNumerics`, no degree in any statement, `IsSelfDualN` consumed through clause 3 (`pairing`) only, so **CB-SG's exponent warning has nothing to bite on here**; **collision check done TWO ways** (the stronger one being a scratch file importing `GQ2` **plus** the new module and `#check`ing every new name — exactly the environment the root build gets after registration, i.e. **the test the three prior collisions would have failed**); consumer note: `hpartialN`'s fork is at **`DD.Vmod` over `C = DD.C0`**, not the `T`-module CB-VAR instantiates, so whoever instantiates owes a `StokesDuality` payload **at `Vmod`, not just at `T`** |
| CB-6 | `hsep`'s candidate-side supplier: the degree-generic marking route | opus | `GQ2/Dyadic/Count/Marking.lean` | CB-5 ✓, CB-4 ✓ | **done 2026-08-01 — ⭐ THE COUNT LANE'S CLAUSE LIST IS COMPLETE** (6 commits → merged, registered, 3503 jobs green, census 11, zero sorries repo-wide; 738 ln, 26 decls, **26/26 std-3** except `exists_kernel_offset` at the strict subset `[propext, Classical.choice]`, zero linter warnings); `hsepN_marking` + `hsep_field_goal_marking` close `SourceDataN.hsep` verbatim at one frame, so **the field is now closed TWICE — arithmetic side via CB-4's `hsep_field_goal` + `isTwoSeparating_of_tateDualityG`, candidate side via the marking route**; ⚠ **route (i) held all the way, but it supplies the `hsep` CLAUSE, not CB-4's `IsTwoSeparating` BINDER — these are not interchangeable**: the binder quantifies over *every* continuous A-valued 2-cocycle of Γ, while the marking route never forms such a class (it works inside `1 → T → B → B/T → 1` whose total space is **finite**, which is exactly what lets clause (iii) `extend` — a statement about finite discrete targets — fire); route (ii) **deliberately not attempted** and priced in the file header instead (it needs level-factoring at module coefficients, and `LevelFactor` is `ZMod 2`/`Fin 4`-pinned, plus the twisted product as a group) — **nothing in the lane needs it now**; ⚠⚠ **CB-4's de-pinning estimate was wrong IN OUR FAVOUR, and the reason is CB-3**: `exists_relatorFree_marking` needed no rebuilding at all because **CB-3's `Count/Lifts.lean` §2 (`wordLiftCollapse`/`baseMap`/`eval_corrected_heisD1`/`isWildTwo_corrected`/`pi_corrected`) IS `Marking.push`/`descend` at arbitrary `ι`/`ρ`**, built for `liftsOver_card` and directly reusable; `mlift_of_relatorFree_marking` also vanished (= `hpres.extend` + `eq_of_eqOn_gen`); what genuinely had to be ported was **a piece the ticket did not name** — `RStageGammaA.redValues_eq_of_coverLift`, the `Fin 4` four-field `corrMark` computation, which became §1's `sum_relatorFib_eq_zero` in **25 lines**: with `ρ` relators, ℚ₂'s "the two reduced values agree" becomes "the traced sum is zero", **i.e. exactly `sepWordN`'s input**; the per-character 𝔽₂-cover layer was already Γ-generic and carrier-abstract, reused verbatim at zero cost; **the six `Vmod` displays landed too** (rows 1–5 + mpc `.hat`) — the ⚠ about the level was right (it is `exp(H(V) ⋊ C₀)`, not `exp(H(T) ⋊ B/M)`) **but cost nothing, because CB-VAR's `orderOf_*_dvd_heisExponent` and `heisLevel_ne_zero_and_even` are already generic in `(A, C)`**; **no exponent constant is written anywhere**, so CB-SG/CB-DET's warning has nothing to bite on; root-collision check done by CB-5's stronger method (scratch importing `GQ2` **plus** the module, `#check`ing all 26 names) — clean; **import-closure delta measured: +3 modules only**; **adds NO new obligation** — every binder is one each branch already carries, and `hd` is `stokesDuality_of_simple` at **`hsimp`**, so the candidate side of `hsep` inherits `hsimp` exactly as `liftsOver_card` does and **`hsimp` stays an explicit hypothesis, never an axiom**; **owed after CB-6: nothing in the lane's own clause list** — two book-keeping items remain, both **branch data rather than count-lane data** (each frozen row hands its `StokesDuality`-at-`Vmod` payload to CB-5 §5 and its `StokesDuality`-at-`T` payload to `hsepN_marking`, both from the one `hsimp` it already carries) ⚠⚠ **AS2 CAVEAT (2026-08-02): the lane's "clause list COMPLETE" is 9-of-11 clause SUPPLIERS** — the two candidate-side `gaussZ_*` clauses have no supplier (CB1's priced "gauss 1900/1100/1300" candidate-side tickets were never opened; CB-DET is K-side only); at instances they surface as the whole-`Prop` binder `hdet`; the missing lane is the Hessian ⇒ `GaussZResidueK` bridge over WN0-c's landed word-side layer — owner-visible queue item, not self-dispatched |
...
| WMP-J | the `mpcW` jet theorem | opus | `GQ2/Dyadic/Certificates/MpcJet.lean` | WMP-d ✓, WW6 ✓ | **done 2026-08-01** (6 commits → merged, registered in `GQ2.lean`, 3492 jobs green; 814 ln, 46 decls, 0 sorries, 0 `decide`, all std-3-or-subset; **the procyclic-M row's last named mathematical gap is CLOSED**: `mpc_cross_operators` at general `(α≥1,r,p,η,h)`, `evalFin_fib_mpcW` at the frozen word, and `mpc_hessRelZTarget` **discharges `mpcHessRelZTarget`**, inhabiting WW6's target exactly as the N row does (no `ResolvedAt` needed — WW6's resolver-immunity holds; only register hypothesis is WMP-c's `ResolverLifts`); **`d₀` is PINNED to `q`, forced not assumed** — the δ-letters do carry the twisted charge `powCharge`, but the plus block uses only `hessSlice_sq`/`hessSlice_commR`, both charge-free, so the twist cancels out of the endpoint and **no `TwistedClass2Domain` input is needed**; **√−10 (gate 9) closed at second order**; ⚠ **two API findings**: (1) **`mpcCopiesCancel` is NOT consumable here and cannot be made so** — it lives in the Heisenberg register, `hessRelZ` in the κ⁰ extension, and no connecting lemma exists *by carrier identification* (the central rules agree only for a Heisenberg-shaped cocycle; `kappa0Cocycle` is f/m-shaped); §6 is the independently-proved κ⁰ analogue and **this is a strict GAIN — it is hypothesis-free, so WMP-c's `CentralReplication` module hypothesis (which the memo's table shows FAILING on three of four ramified simples) does not touch this row's Hessian at all**; (2) **`foxD` and the κ⁰ base coordinate are the same object and nobody had said so** — `evalFin_hessMark_fst`, resolver-free, a **hoist candidate every future "gate-D row consumed at gate E" argument needs**; three new central-factor laws (conj/comm/square blind to the centre), the same gap WW6 filled with its six; WW6's template transferred for §5–§6 as advertised but the **N-row proof shape did NOT** (it is `map_eval` past a finished NC theorem; no M-row theorem exists to transport), so the M row needed a from-scratch three-mechanism argument; row owes only AS-lane instantiation + one optional generalisation (handle tail split off but not evaluated → statements are `h=0`; extending needs the `evalFin` twin of WN0-c's `hess_handlesW_eval`)) |
| AS2 | **pilot instance `ℚ₂(√−2)`** (compact `N₂`) end-to-end | opus | `GQ2/Dyadic/Instances/SqrtNeg2.lean` | AS1 ✓, WN0-c ✓, CB lane ✓ (9-of-11 — see caveat) | **done 2026-08-02** (9 commits `4e2b974…38c8337` → merged, registered `ca01f17`; 1076 ln, 39 decls, 0 sorries; post-merge verification green — build 3509 exit 0, both audits, census 11, twins identical; trap 5 fired a third time at merge — ~30 unqualified citations of CB-DD's deleted trio re-pointed by orchestrator, headline prints re-verified at root) — **THE FIRST `WordCertificate` EVER CONSTRUCTED, AND THE FIRST END-TO-END GENERAL-K INSTANCE**: headline `SqrtNeg2.sqrtNegTwo_candidate_equiv_galK : Nonempty (ContinuousMulEquiv (candidateGroup 2 (qOf K FF) pilotW) (GalK K))` via `candidate_equiv_galK_of_supply`, `q`-generic certificate instantiated at `q_K`; prints **std-3 ∪ {B1,B6,B7,B9,B11a}, B5-K/B10-K NOWHERE**, `sqrtNegTwoWordCertificate` and the entire candidate side **exactly std-3**; `KSupply.determinant` **DISCHARGED** via CB-DET's `affineDeterminant_galK` (no longer carried); both CB-6 fold-in payloads landed **consumer-verified** (at `T` inside `sqrtNegTwo_hsep` via `hsep_field_goal_marking`, at `Vmod` inside `sqrtNegTwo_hrsep`); the marked-core composite written down — and writing it down surfaced a MISSING DATUM (`piAb : G_K(2) →* G_K^{ab}` + `hpiNu`: `toAbK` does NOT factor through G_K(2), the abelianization not being pro-2; now two named G-Lab-pack binders, dischargeable later by a small direct-factor lemma); residual binder surface (all docstringed with provenance): `hsimp` + `hsplit`/`hZcount` (stageR136 recursion side, verbatim `blockStageR136K`'s open inputs) + `hdet` (⚠⚠ see the CB caveat: the candidate-side Gauss clauses have NO supplier — CB1's priced "gauss 1900/1100/1300" tickets were NEVER OPENED, CB-DET is K-side only; the Hessian ⇒ `GaussZResidueK` bridge over WN0-c's word-side layer is the missing lane) + G-Lab pack (`fLab`, `piAb`/`hpiAb`/`hpiNu`, `horient`, `NScalingHypothesis`, `hpair`) + ASK carried leaves 2–3 (`hexact`/`hstokes`) + §12 field inputs + `hdeg`; ⚠ traps for AS3-b/AS5 (broadcast): **pin every `{α h q n}` implicit** — unpinned `2 + 2*?h =?= 2` diverges at **320,878 unfolds** through `GammaR`'s admissible-limit definition (measured, `set_option diagnostics`); state the four stokes conjuncts STANDALONE not packed (packed diverges even pinned); spell the datum quotient `Base ⧸ datum.M` never `Mlayer` (instance keyed at `?D.M`); feed `proTwoWord` from the T2-free `eval_pro2_nCompact_eq_nRelWord` corollary (the natural feed has `[T2Space G]` the record lacks); ⚠ no concrete `ℚ₂(√−2)` field object exists in the repo — headline honestly parametrized over any supplied quadratic K on the branch (KSupply posture, plan-A6 honest) → literal-field instantiation = ticket AS-F (dispatched); positive: the `cardH2` surjection `Γ_R ↠ 𝔽₂²/⟨s̄⟩` built through `D_N`'s universal property, no hand word evaluation — `datum`/`datum_noDescent`/`quotient_cases` reused verbatim as CB-VAR promised |
| AS3 | instances √2, √5 (M0) · √10, √−10 (Mpc, procyclic gate) | opus | `GQ2/Dyadic/Instances/{Sqrt2,Sqrt5,Sqrt10,SqrtNeg10}.lean` | AS1 ✓, WM0-c ✓, WM0-d ✓, WMP-c ✓, WMP-d ✓, CB lane ✓ | **done 2026-08-02** (5 commits `16fee1d`/`e7c3047`/`a72f06e`/`8595b0f`/`3eef962` → merged `39565dd`, 4 modules registered `c2df453`; 1595 ln, 104 decls, 0 sorries, 0 new axioms; post-merge verification green — build 3507 jobs exit 0, `check_dyadic` + `check_axioms`, census 11, twins identical; **all four headlines re-printed at the root post-merge: std-3 + exactly {B1, B6, B7}, identical across fields, no B5-K/B10-K** — the first non-ℚ₂ instances of packet Thm 1.1, surface strictly smaller than the frozen capstone's; collision scratch test clean; trap 5 fired at merge exactly as planned — AS3 branched pre-CB-DD and cited the deleted `Count.trivial*ZmodTwo` trio at 33 sites → orchestrator re-pointed to canonical `scalarActionZmodTwo*`, prints unchanged) ⚠⚠ **THE ROW WAS NOT INSTANTIATION-ONLY — its premise was wrong**: `Count/ProTwo.lean` §6's "one `CoreReindex` per branch" existed only for compact `N` (`Count.PilotN.nReindex`), so `WordCertificate.pro2/ker_pro2/hpro2/compat` were unreachable at ALL FOUR fields; **both missing dictionaries built here** — `MCompactCore` (Sqrt2 §1–5: compact `M`'s letter table IS compact `N`'s plus a triangular core-side twist `mTwist`/`mUntwist`, so it factors through `nReindex`; generic in (α,h,q), far under the memo's 80–150 ln/branch) and `MProcyclicCore` (Sqrt10 §1–5: does NOT factor — σ at core slot 2 compact vs 3 procyclic, the σ-offset convention at dictionary level; built at h=0, η=1). 13 of 17 `WordCertificate` fields proved per field; the four analytic clauses remain binders (AS1 divergence 4, unchanged — same posture as n=1). **Eight `hsimp` fold-ins landed** (2/field: at `T` for `hsepN_marking`'s `hd`, at `Vmod` for `Count/Marking` §5's `hsd`; branch-generic forms + named row instances; NO Count file edited). **`r ≥ 1` discharged at both procyclic rows as THEOREMS** (`Sqrt10.r_pos`/`SqrtNeg10.r_pos` — frozen `level = 1`, so WNP-c's r=0 degeneracy never arises). Freeze/F5 regression pins recorded per field (digests + (S₃,D₈,A₄) triples), no proof depends on them. ⚠ findings: **the procyclic dictionary needs η = 1** (recovering σ from the slot-3 `σ^{η̂}` inverts a ℤ̂-power — free at `.one`, unavailable at a general `.hat` display; a `.hat` instance needs a new construction or an η̂-invertibility hypothesis); **`Mpc.hlinrow` is closed only at √−10** (no √10 twin; not a `WordCertificate` field so the inventory stands, but the two procyclic fields sit at different depths — AS1's docstring predicted this); **`q_K` identifies no field** (√2 and √10 both `q_K=2` — the WORD pins the field; and `q_K=4` at √5 is NOT the ramified-i condition — √5 sits in FD2's `κ_K ≠ 0` branch like every row, `kappaK_ne_zero_*` recorded per field); **no concrete quadratic field exists in the repo** (no `ℚ₂(√2)` `IntermediateField`, only `⊥`) so the headlines are honestly parametrized over `K` with the row's arithmetic entering via `hqK`/`KS`/`params` binders; `hnuSigma`/`hnuWild` satisfiability HAND-verified but not Lean-proved — blocked on a `maxProPQuotient` commutativity lemma in files AS3 did not own (the agent stopped and reported, correctly). ⚠ traps 1/2 never fired (the K-side is reached only through `KSupply`, which absorbs both; `local_gauss_K` is not consumed — the LG5 ⇒ `GaussZResidueK` bridge still has no owner, so `determinant` stays a binder). → AS3-b dispatched (ν-witness + core hoist + QTwo:69 one-liner); general-h procyclic dictionary NOT queued (no frozen instance needs it) |
| AS3-b | ν-satisfiability witness · core hoist · stale-table sweep | opus | `Instances/Cores.lean` (new) + the four instance files + `QTwo.lean`/`CertificateMain.lean` docstrings | AS3 ✓ | **done 2026-08-02** (5 commits `60622d9`…`6a17d26` → merged `d2ecef9`, Cores registered; root build 3510 exit 0, both audits, census 11, 0 sorries; nonvacuous prints re-verified at root — B5-K/B10-K nowhere). **All four AS3 headlines gained a ν-free corollary** `candidate_equiv_galK_*_nonvacuous`, printing std-3 + {B1,B6,B7} — identical to their parents, which are byte-identical and untouched; the abstract slot `(P, ν_P)` is now demonstrably inhabited at every AS3 row (G3 exhibit). ⚠⚠ **The authorized freeze exception was NOT exercised — the ticket's premise was wrong in a good way**: `maxProPQuotient` commutativity is unnecessary; marking `D_M` by powers of a SINGLE element makes the M_α commutators die by `Commute.zpow_zpow`, and the relator collapses to one integer condition `2e₀+2^α e₂ = 0` (compact `e=(−m,0,1,0)`, procyclic `e=(−ms,p,s,1)`, both by `2m=2^α` — exactly AS3's hand-verification). `MaxProP.lean`/`BoundaryFrame.lean` untouched; ⚠ two homing debts recorded in-file (`mRelWord_of_commute` → beside `MarkedCore.mRelWord_comm`, folded into MC-CoV-split as optional; the two `Z₂`-procyclicity facts → `BoundaryFrame.lean`, needs a frozen-tree ticket — owner-visible minor). **Hoist verified by hash, not by reading**: both blocks sha256-identical before/after (396 + 361 ln), 85 moved fully-qualified names re-checked from a root scratch, `Sqrt5`/`SqrtNeg10` needed no change; per-section `open` scoping solved the only obstruction (a merged open list would collide `Words.MCompact`/`Words.Mpc`). QTwo's inventory went **all-green — 3 rows, not 1** (the scalar trio was never unbuilt, only unnamed; `pro2` &c. by AS4-b's `comap_proPKernel`/`gammaR_hom_ext`; the analytic four at n=1 by CB-TRN) + `CertificateMain`'s matching rows + AS2's `sqrtNegTwoWordCertificate` recorded as the first complete branch-row certificate; all docstring commits proved comment-only by stripped diff; ⚠ `surjective_of_ztwoOne_mem_range` came out sharper than needed (only `Group + TopologicalSpace + CompactSpace` on the source) |
| AS4 | `n = 1` wrapper: the dyadic machinery recovers the ℚ₂ theorem | opus | `GQ2/Dyadic/Instances/QTwo.lean` | AS1 ✓, WL-c ✓ | **done 2026-08-01 — RECOVERY IS UNCONDITIONAL, AND PROVED TWICE** (6 commits → merged, registered, 3495 jobs green, census 11; 547 ln, 53 decls, 0 sorries, no new axioms); `candidateGroup_lSq_equiv_absGalQ2 : Nonempty (ContinuousMulEquiv (candidateGroup 1 2 (lSqW 0)) AbsGalQ2)` with **no hypothesis binders and no instance binders**; **Route 2 is the stronger statement and was beyond the brief — `…_via_sourcesN` goes through the DYADIC RECURSION, not the ℚ₂ capstone** (SD3's `nonempty_continuousMulEquiv_of_sourcesN` on SD-R1's `sourceR_N`/`sourceF_N` over the shared n=1 slot), also fully unconditional: **the dyadic pipeline itself closes at n = 1**; `q2_capstones_agree` puts Γ_A, Γ_R and Γ_{L_sq,1} side by side (**merge gate 8**; the frozen ℚ₂ capstone names still compile); **the real mathematics is §4's marked identification `gammaR_lSq_equiv_roe`, which had to be PROVED** — the two `Γ_R`s are independent constructions over different alphabets with different `IsAdmissibleU` predicates and different relator trees, so the chain is alphabet transport `F(σ,τ,x₀,x₁) ⇄ F₄` hand-classified through `homEquiv` (**no `FreeProfiniteGroup` functoriality exists**) → `isAdmissibleU_iff_isAdmissibleUR` → `NR_eq_comap`; **nothing was re-derived** (only non-formal input cited is WL-a's `eval_lSqW_zero`); axioms: **49/53 std-3 or strict subset — the whole §1–§4 core identification is std-3** — and the 4 recovery-level theorems print a list **BYTE-IDENTICAL** to `main_presentation_literal_roe_unconditional`'s, **B5-K/B10-K consumed nowhere** (the n=1 route never enters the K-layer); ⚠ **finding: WL-c was NOT used at all** — the n=1 recovery is a *presentation* statement, while the certificate layer (Fox rows, Stokes endpoints, Grams, Hessians) feeds only the four count clauses, so **WL-c is not on AS4's path**; ⚠⚠ **SHARPEST CONSEQUENCE FOR AS5/G3: at n = 1 the missing `WordCertificate` costs only record-level plumbing, not the theorem (route 2 proves it without one) — at general K it costs THE THEOREM. That is AS1-divergence-4's real content, and it is the AS lane's TRUE BLOCKER, not AS4.**) |
| AS4-b | AS4's three named ownerless pieces | opus | `GQ2/Dyadic/GammaRHom.lean` (new) | AS4 ✓ | **done 2026-08-01** (2 commits → merged, registered, 3497 jobs green; 328 ln, **14/14 decls std-3**, census 11); **the uniqueness principle came out SHARPER than specified — the target needs only `Group` + `TopologicalSpace` + `T2Space`**: not `IsTopologicalGroup`, not `CompactSpace`, not `TotallyDisconnectedSpace`, not `ProfiniteGrp`, because it is a density argument (`gammaGen` generates Γ_R only *topologically*) so the only demand on the target is that the equalizer of two continuous maps be closed; **that is strictly weaker than BOTH ext principles already in the tree** (`Count.CorePresentation.hom_ext` demands a full profinite target; `QTwo.free_hom_ext` demands a `ProfiniteGrp` object and is about the FREE group), and **the gap matters for general-K consumers landing in `Tq q`, `Ztwo`, and subgroups of products**; piece 1 was stated via a strictly more general primitive extracted first — `proPKernel_le_comap` is naturality for **any** continuous hom (no surjectivity, no compactness, no profiniteness), and both directions of the equality are instances of it; **all four of `pro2`/`ker_pro2`/`hpro2`/`compat` discharge at n=1, VERIFIED end to end** (a scratch file importing `QTwo` + `GammaRHom` compiles them sorry-free, printing exactly what `pro2R` already carries — nothing new); ⚠ **finding: piece 3 was NOT actually unwritten — AS4's inventory row and `CertificateMain.lean`'s obligation table are STALE**: CB-0 closed it generically in `Count/Routine.lean` §4 over an arbitrary monoid; only the `(n,q,R)`-generic naming at Γ_R was missing (**correct the table**); ⚠ **the trivial `ZMod 2`-action now has FIVE copies** and `topGen_gammaR` **FOUR** — `Count/Scalar.lean`'s docstring rules the merge belongs to whichever ticket first has both in scope, and **`GammaRHom.lean` is the first file that could be that home** (it sits directly above `AdmissibleR.lean`; all others are plain-import downstream) → dedup ticket; two spelling frictions recorded in-file for the QTwo transcription (`pro2R`/`phiR` live at `FreeProfiniteGroup (Fin 4) ⧸ NR` while `toRoe` lands at `↑GQ2.GammaR` — defeq but **not at `instances` transparency**, so `rw` fails with a misleading "target not type-correct"; and `match x with | .wild ⟨0,_⟩` needs a `show` to normalise the `Fin` literal) |
| CB-DD | dedup ticket named by AS4-b: **five** copies of the trivial `ZMod 2`-action (`Count.trivialSMulZmodTwo`, `Count.scalarAction`, inline in `HalfTorsorGammaA.lean` + `HalfTorsorGammaR.lean`, and `GammaRHom`'s) and **four** of `topGen_gammaR` (`Count/Routine.lean` §3, `Count/ProTwo.lean` §2, `Count/Presentation.lean`, `GammaRHom`). `Count/Scalar.lean`'s docstring rules the merge belongs to whichever ticket first has both in scope — **`GammaRHom.lean` is that home**. Also correct `CertificateMain.lean`'s stale obligation-table row (piece 3 was closed by CB-0, not unwritten) | opus | `GQ2/Dyadic/GammaRHom.lean` + the duplicate sites | AS4-b ✓ | **done 2026-08-02** (3 commits `e9b06f3`/`3d2d3ac`/`4e2759e` → merged ff `4e2759e`; **8 duplicate declarations deleted, 9 consumers re-pointed, ZERO statement changes and ZERO renames** — the WAH/WWH standard held; net −24 ln; root build 3503 jobs green under `pipefail`, `check_dyadic` + `check_axioms` green, census 11; **all 162 decls of the 5 touched files axiom-audited before/after — the diff is exactly the 8 deletions, 0 surviving prints changed**; `GammaRHom.lean` is now the canonical home for both pieces per `Count/Scalar.lean`'s ruling; `CertificateMain.lean` stale obligation row corrected) ⚠ **AS4-b's fourth `topGen_gammaR` copy was under a DIFFERENT NAME** (`Count.gammaGen_topGen`) — why the name-grep saw three; ⚠ **`GQ2.Roe.topGen_gammaR` is NOT a copy** — the ℚ₂ four-element-set statement; same name, different theorem, left alone; ⚠ **two copies deliberately KEPT** (`HalfTorsorGammaA/R.lean` inline `letI`s): the obstruction is **POLICY not Lake** — a probe compiled green with the home imported, but the files are frozen-ℚ₂ (A6), the copies are anonymous instances inside proof bodies, and redirecting would create the tree's only ℚ₂→Dyadic import edge; ⚠ **the trivial-action SHAPE has 12 sites repo-wide, not 5** — the 7 uncounted are deliberately different (frozen-tree global `instance`s at concrete groups vs the canonical `def` that must NOT shadow the `(140)`-layer; one at `ZMod p`; one inline `letI` at `RadicalEdge/Local.lean:541`) — NOT dedup candidates; ⚠ **the same stale row survives at `Instances/QTwo.lean:69`** (lane off-limits mid-flight) → orchestrator one-liner after the AS wave merges |
| CB-TRN | **`SourceDataN` transport across an iso** — AS4 established that `exactLifting`/`stokes`/`scalar`/`determinant` are **not reachable by citation** at n=1: the ℚ₂ proofs exist and are packaged as `sourceR_N`, but every clause is a statement about *that carrier*, quantified over `BoundaryLiftsK`/`RecursionFrame`/`TCocycle`/`VCocycle`/`GaussZResidueK` data depending on Γ through its boundary map. **Scope note before dispatching: this buys the n=1 `WordCertificate` only — AS4's route 2 already proves the n=1 theorem without it, and at general K there is no iso to transport across, so the clauses must be PROVED (that is the CB lane's job). Low priority; do not schedule ahead of CB-VAR/CB-3/CB-4** | opus | new | AS4 ✓ | **done 2026-08-02 — THE TRANSPORT WENT THROUGH; the n=1 `WordCertificate` EXISTS** (9 commits → merged `0cc4828`, registered; `GQ2/Dyadic/SourceTransport.lean` new, 755 ln, 41 decls, 0 sorries, 0 open binders, no new axioms; post-merge verification green — build 3508 exit 0, both audits, census 11, twins identical); the ticket's bounded scope note is now UNDERSTATED: besides `wordCertificateLSq` (unconditional; B-Lab = `Labute.bLab`) the file lands **`SourceDataN.transport` — a fully generic record-level transport of all 21 fields along any `ContinuousMulEquiv`, exactly std-3, no ℚ₂/`L_sq` anywhere** — and a **THIRD unconditional proof of `Γ_{L_sq,1} ≅ G_ℚ₂`** (`candidateGroup_lSq_equiv_absGalQ2_via_wordCertificate`, the first whose conclusion never mentions `GQ2.GammaR`; certificate → `WordCertificate.toSource` → SD3): **AS5's uniform story is now executable, not just asserted**; the structural reason it worked (reusable): every object the recursion counts is a set of continuous maps OUT of the source cut out pointwise, and every extracted scalar is pointwise, so precomposition moves everything — mostly `rfl`; the two cohomological invariants move too (`iotaB` = `B²`-membership, `H²` via `ContCoh.H2comap`); **the single real hypothesis is triviality of both `ZMod 2`-actions (`htriv`)**, which is what makes `ι_Γ`/`betaChi`/`QZero`/#H² transportable; the certificate consumes 18 of 21 fields (the scalar trio is supplied for Γ′, not transported); axioms VERIFIED AT ROOT POST-MERGE: §§1–7 all exactly std-3, `wordCertificateLSq` prints exactly `sourceR_N`'s list (std-3 ∪ {B3c,B5,B8} — packaging costs ZERO axioms), the recovery theorem prints **byte-identical to `main_presentation_literal_roe_unconditional`**; B5-K/B10-K nowhere; nothing reads a count; ⚠ **AS4's inventory table (`QTwo.lean`) and `CertificateMain.lean`'s obligation table are now STALE in two rows** (the four analytic clauses AND `pro2`/`ker_pro2`/`hpro2`/`compat`) — folded into AS3-b's doc remit; ⚠ two new frictions recorded in-file: `iotaB`'s `Decidable` is `Classical.dec` under a `letI` (`if_pos`/`if_neg` do NOT apply — use `iotaB_of_mem_B2`/`iotaB_eq_zero_iff`), and `[TopologicalSpace (ZMod 2)]` as a section variable SHADOWS the global instance and breaks bridging lemmas; ⚠ confirmed for the census sign-off: there is NO global `DistribMulAction _ (ZMod 2)` — the clause `def`s get theirs from the record's own `smulZmod2` binder used as a local instance; optional follow-on noted (no ticket): `SourceDataN.transport` is the tool if ASK's source ever needs re-reading on `GalK K` itself |
| AS5 | final ramified-i theorem + axiom report + acceptance appendix + trust-boundary doc | fable | `GQ2/Dyadic/Main.lean`, `docs/dyadic/literature-axioms-dyadic.md` | AS2–AS4, all WC lanes, F4 | **done 2026-08-02 — G3 IS NOW REACHABLE** (3 commits `482f89b`/`e4fc5a5`/`9b11835` → merged `82a2e6c`, Main registered; post-merge verification green — build 3512 exit 0, `check_dyadic` all-OK **with merge-gate-7's D4 hash gate ACTIVE for the first time**, `check_axioms` green **with the new 5b block: 13 dyadic capstones at frozen per-declaration sets, B5-K/B10-K in none, 4 n=1 twin pairs identical to the ℚ₂ capstone**; census 11, 0 sorries). `Main.lean` (711 ln): **`ramifiedI_candidate_equiv_galK`** = the packet completion criterion in landed form over `FrozenQuadRow` (the five freeze rows), branch-by-branch, given `FieldInputs` (§12) + `RowResidual` — **re-verified at root post-merge: prints exactly std-3 ∪ {B1,B6,B7,B9,B11a} = the union of the instance headlines, nothing new**; per-row `rowEquiv_*` (pilot 5-B set; four M-rows std-3∪{B1,B6,B7} — §A.1 flags the union so G3 doesn't misread it as per-row); **uniformity theorem `candidate_equiv_galK_of_frozenRow_certificates`** — row-uniform WordCertificate/KSupply gate, NO case split, std-3∪{B1,B6,B7}; certificate stock: `wordCertificate_nonempty_lSq` (unconditional) + `_sqrtNegTwo` (exactly std-3), M-rows cited at their true 13-of-17 state; `lRow_candidate_equiv_absGalQ2` + all three n=1 routes **byte-identical to `main_presentation_literal_roe_unconditional`**; ⚠⚠ **`no_frozen_Npc` is a THEOREM (std-3)** — the packet's fully-general "every branch" statement is NOT provable from the landed pieces (Npc has word + certificate layers but no CoreReindex dictionary, no WordCertificate constructor, no frozen field); the final theorem is honestly the frozen-table form, delta recorded in module docstring + trust doc §C.6. Trust doc `literature-axioms-dyadic.md`: §A measured print tables, §B census-11 rows (K-side citations UNVERIFIED per Q7), §C complete residual surface (C.0 bundle binders ↔ B5-K/B10-K; C.1 `FieldInputs` — the `hqK` pins are GONE, assembly consumed headlines at `q := qOf K FF`; C.2 pilot's 14 binders incl. `hdet` ⚠⚠ 9-of-11; C.3 M-rows; C.4 clause ledger; C.5 n=1 EMPTY; C.6 Npc), §D the G3 checklist. WW5 appendices emitted for all five words (envelopes vendored `docs/dyadic/words/`, hash manifest, ledger-§7 skeletons); check-5b growth-FAIL exercised (deliberate shrink → FAIL with exact diff → restored). ⚠ AS3-b/MC-CoV-split trap-5 sweep at merge: CLEAN (fully-qualified core names — the hoist preserved them; parent headline signatures untouched). ⚠ `pilotW` lives in `GQ2.Dyadic.Count` (Count/Routine.lean:421), not `SqrtNeg2` — G3-reader note |
| AS-F | literal-field instantiation, feasibility-first | opus | `GQ2/Dyadic/Instances/Fields.lean` (new) | AS2 ✓, AS3 ✓ | **done 2026-08-02 — FEASIBLE, AND THE PILOT IS NOW LITERAL** (6 commits `6ae32d8…0db8aaa` → merged, registered; 724 ln, 66 decls, 0 sorries; verification green — build 3514 exit 0, both audits incl. check-5b, census 11). **The repo now has concrete quadratic extensions of ℚ₂** — closing the finding AS2 and AS3 both recorded: `quadField a = ℚ_[2]⟮√a⟯` (K-type `IntermediateField ℚ_[2] (AlgebraicClosure ℚ_[2])`, no exotic API needed) with unconditional `FiniteDimensional`; **`hdeg` discharged at all five rows, `params_qK` (q_K = 2) + the ramified-i binder at all four ramified rows**, and **`Fields.sqrtNegTwo_candidate_equiv_galK_literal` = packet Thm 1.1 at the LITERAL ℚ₂(√−2)** — print re-verified at root post-merge, **byte-identical to AS2's headline** (std-3 ∪ {B1,B6,B7,B9,B11a}); no B5-K/B10-K anywhere — literal instantiation is census-free; everything else exactly std-3 (56/58). Two proofs beat expected cost: `q_K` WITHOUT the residue field and WITHOUT `e·f = n` (odd `v(a)` parity ⇒ filtration collapses at depth 1 ⇒ `card_gr_zero` reads `2^f − 1 = 1`), and a UNIFORM ramified-i witness (`z = 1 + √a + i`, `‖z‖⁴ = ‖2‖³` odd vs the even powers Kˣ supplies — an earlier ζ₈ witness was ±2-only and is RETRACTED in-file). ⚠ still blocked, named precisely: √5's `q_K = 4` + ramified-i (the unramified-flavor row — parity says nothing; needs the norm form + the residue field, whose bridge `UnitFiltrationCounts.card_gradeZero`/`card_gradeI` is **`private`** — a one-word owner-call fix in a frozen-tree file); `ramifiedData` (Galois-theoretic, routed through AX3/AX4); and the bundles themselves, which ARE B5-K/B10-K = local class field theory — **mathlib has none of it; this is the campaign's owner-approved endstate, not a gap** (AS-F §8 is now the concrete evidence that nothing else in the pilot headline is field-arithmetic). ⚠ NEW TRAPS, broadcast: **`lake env lean` does NOT apply the lakefile's `leanOptions`** (`autoImplicit false`) — a file can pass it and still fail `lake build`; **`lake build <module>` is the only valid per-file gate**; bash heredocs CORRUPT the `ℚ̄₂` notation bytes (use Write/Edit tools); trap 3 fired at §8 — slow not divergent, `maxHeartbeats 800000` (SqrtNeg2's own convention) elaborates in ~5 s. → AS-F2 dispatched (the three cheap literal specializations √2/√10/√−10 + trust-doc addendum); √5 = a real ticket (250–400 ln, owner queue); de-private `card_gradeZero`/`card_gradeI` = owner call |
| AS-F2 | the three literal specializations + trust-doc addendum | opus | `Instances/Fields.lean` §9 + trust doc §C.7 | AS-F ✓, AS3-b ✓ | **done 2026-08-02** (4 commits → merged ff `716672b`; verification green post-rebuild — build 3514 exit 0, both audits, census 11; 0 sorries). **`Fields.candidate_equiv_galK_{sqrtTwo,sqrtTen,sqrtNegTen}_literal`** — packet Thm 1.1 at the literal ℚ₂(√2)/ℚ₂(√10)/ℚ₂(√−10), specializing AS3-b's `_nonvacuous` corollaries (strictly dominant choice: ν-binders already discharged, same print as the plain parents — verified identical); all three print std-3 ∪ {B1,B6,B7} **byte-identical to their parents**, no B5-K/B10-K; the `hqK` substitution makes the CONCLUSIONS literal too (`candidateGroup 2 2 R` — the §8 pilot cannot, its parent reads `qOf K FF`). Trust doc §C.7 added (4-row measured table + the "nothing else is field-arithmetic" finding, now measured rather than argued). ⚠ finding: **the M-row headlines consume NO `hdeg`** — the degree lives inside the unbuilt M-row `KSupply` (the pilot builds its supply, the M-rows carry it as a binder); `finrank_KSqrt*` proved and waiting for the future M-row `KSupply` producer. Orchestrator hardening at merge: the four literal theorems added to check-5b's `AUDIT_DYADIC` (**17 capstones now gate-frozen**, count auto-derived), probe retargeted `import GQ2.Dyadic.Main` → `import GQ2` (Main does not import Fields — root import makes any registered decl auditable; skip-guard now on `GQ2.olean`), and §C.0's stale "no concrete field object" row rewritten to point at Fields.lean (§C.7 flag kept). ⚠ orchestrator process note: the diagnosis run ATE the handoff's own `\| tail` exit-masking trap once (text showed FAIL, echoed exit was tail's 0) — re-run with file-redirect + honest `$?`; and the first probe failure had TWO stacked causes (narrow probe import + un-rebuilt post-merge olean tree) — rebuild-before-audit is part of the merge cadence for a reason |
