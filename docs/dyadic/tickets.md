...
| CB-1 | **the lane's load-bearing ticket** (post-spike): the `z1Equiv` comparison isomorphism carrying CB-S's word-complex counts into the recursion's cocycle vocabulary — the arithmetic already matches exactly, only the vocabulary transport is missing | opus | `GQ2/Dyadic/Count/Compare.lean` | CB-S ✓ | **dispatched 2026-08-01** (ww worktree, `dyadic-cb1t`; scope reduced 1400 → ~1100 by the spike) || CB-P | the candidate-side pro-2 bridge (generalize `maxPro2Bridge`, NOT `exists_pro2R`; Labute-free is this ticket's defining property) | opus | `GQ2/Dyadic/Count/ProTwo.lean` | CB1 ✓ | **dispatched 2026-08-01** (mc worktree, `dyadic-cbp`) — on AS2's shortest path |
