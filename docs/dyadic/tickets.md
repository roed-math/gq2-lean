...
| CB-SG | **ASK's design finding, promoted to a ticket**: make `prop_5_16` **Γ-generic** over a `TateDualityG Γ 2` bundle + an Euler-characteristic hypothesis, instead of cloning `Phase140/Local.lean`'s four residues (~1.3k ℚ₂ lines) to K. Both ℚ₂ inputs are available at K; LG2's `PairingK.lean` already generalized the neighbouring layer this way; **the generic version discharges ASK's `stokes` binder AND serves the candidate side** | opus | `GQ2/Dyadic/Phase/StokesGeneric.lean` (new, name provisional) | ASK ✓, LG2 ✓ | **dispatched 2026-08-01** → `~/claude/gq2-dyadic-lg`, branch `dyadic-cbsg`. Briefed with the three verified traps (ASK's instance-path trap, LG2's `smul` trap + LG4c's `cup11Fun` variant, LG4a's anchoring convention) and told to **measure** the ambient-free fraction before committing to a size — LG4c's comparable retype needed only ~35 decls. Told explicitly that a generic statement turning out FALSE, or needing a hypothesis true only at `G_K`, is the finding that matters, since ASK's whole recommendation rests on the generic version also serving the candidate side. |
...
| CB-LV2 | **CB-LV's correctly-refused items, re-homed to the right file**: the four split-target instantiations (`mCompactFam`, `lSqFam`, `mpcFam`, `mpcFamOf_hat`) + the `Vmod` twin + the generalisation `twoMulExponent_ne_zero_and_even` (`[Group G]`) that the twin needs and CB-FR2 did not anticipate; optionally the sixth corollary making CB-LV's now-generic pilots hypothesis-free in the target. **Spec is copy-paste: CB-LV already wrote and compiled all five sorry-free at std-3** in `/private/tmp/claude-501/-Users-roed-claude-lmfdb/cc4971ba-897e-47c0-8465-f948478632b2/scratchpad/CblvSplit.lean` | opus | `GQ2/Dyadic/Count/Frozen.lean` §10 | CB-LV ✓, CB-FR2 ✓ | **dispatched 2026-08-01** → `~/claude/gq2-dyadic-ob`, branch `dyadic-cblv2` |
...
| AS4-b | **AS4's named unowned pieces, all generic and all currently ownerless**: (i) `Subgroup.comap e (proPKernel p B) = proPKernel p A` for a `ContinuousMulEquiv` (AS4 says it belongs in `GQ2/MaxProP.lean`); (ii) **the uniqueness principle "two continuous homs out of `GammaR n q R` agreeing on `gammaGen` are equal"** (belongs in `AdmissibleR.lean`; broadly useful beyond this row); (iii) the routine `smulZmod2`/`contSMulZmod2`/`htriv` instances for `GammaR`. With (i)+(ii), `exists_pro2R ∘ toRoe` discharges `pro2`/`ker_pro2`/`hpro2`/`compat` at n=1 (AS1 divergence 3). **Put them in ONE new leaf rather than editing the two existing files** | opus | `GQ2/Dyadic/GammaRHom.lean` (new) | AS4 ✓ | **dispatched 2026-08-01** → `~/claude/gq2-dyadic-as`, branch `dyadic-as4b` |
