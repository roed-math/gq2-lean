...
| AS2 | **pilot instance `ℚ₂(√−2)`** (compact `N₂`) end-to-end | opus | `GQ2/Dyadic/Instances/SqrtNeg2.lean` | AS1 ✓, WN0-c ✓, **CB lane** | **UNBLOCKED 2026-08-02 — the CB lane's clause list is COMPLETE (CB-6, `4d7ea23`); this row is now on the critical path to G3.** [was: BLOCKED on the CB lane] (CB-TR then the clause tickets); CB-S measured the shortest path as CB-P + CB-1, both now done, so AS2 unblocks as soon as the clause values exist ⚠ **Brief the agent with the two live instantiation traps**: ASK's instance-path trap (`GalK K` vs `↥K.fixingSubgroup` — `rw` fails where `exact` succeeds) and **CB-DET's `ContinuousSMul ↥U (MuN 2)` trap, which every consumer of `local_gauss_K` at a concrete K will hit** (defeq but not *reducibly* so; fix = name the two binders and pass them explicitly). Also fold in the row's two book-keeping hand-offs, both from the single `hsimp` it already carries: a `StokesDuality`-at-`Vmod` payload to `Count/Separating.lean` §5, and a `StokesDuality`-at-`T` payload to `hsepN_marking`. |
