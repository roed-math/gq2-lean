...
| SD-R2 | spine clone II: §9 Terminal/Induction + MStageCount **+ SEAM B** (generalize `ActsThroughTame`/`kappa0_exists`/`kappa0_exists_tame` to `t^q` with `Even q ∧ q ≠ 0`, then REMOVE SD-R1's threaded `hrel2`/`hrel2HV` binders — no other edit needed; consumers via `tame_odd_order_pow`/`tame_zpowers_normal_pow` + `lemma_6_11_of_odd_normal`, NOT `lemma_6_11_of_tame_pair_pow` (pinned at q = 2^f); one expected omega seam at Induction.lean:539) | opus | `GQ2/Dyadic/Recursion/` (part II) | SD-R1 ✓ | **dispatched 2026-07-31** (wl worktree, branch `dyadic-sdr2`) |
| SD-R3 | spine clone III: Phase140 + RStage + bridges + generic-Prop89 (**+ Block/RStage's three b-typed theorems** (moved from part I — reduce to `stageR136_ofRSepData`, already this part's file) **+ the Q1-sub-ruling VLiftCount vH-wrapper** (its sole consumer is Phase140/Assembly:191); one expected omega seam at ThmFourTwo:285; measured b-densities in SD-R1's report) | opus | `GQ2/Dyadic/Recursion/` (part III) | SD-R2 | queued |
