...
| CB-MP | build the **RESTRICTED** `IsMarkedPresentation` for `GammaR n q R` (GR1 settled the form: the plain clause is equivalent to being the bare presentation, so only the restricted one — extension over markings with 2-group wild normal closure — holds at the corrected group; machine = `gammaLift` + `NR_le_ker_of_isOpen`), **and adapt CB-1's `z1Equiv` consumer to it** | opus | `GQ2/Dyadic/Count/Presentation.lean` (new leaf) + `Count/Compare.lean` (CB-1's, reopened for the hypothesis swap) | CB-1 ✓, GR1 ✓ | **dispatched 2026-08-01** |
