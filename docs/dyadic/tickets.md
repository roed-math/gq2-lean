...
| MC5-swap | mechanical follow-up to MC-CoV: at the compact-`M` consumer sites swap `mMarkedMatching`/`marked_matching_certificate_M`/`prop_MC_M_correction` for the `_of_chiKer` versions and drop `hpivot` in favour of `hker` (MC-CoV's recommended edit — keeps the general-`h` statements intact; the alternative textual removal costs them) | opus | `GQ2/Dyadic/MarkedCore/{Certificate,M}.lean` (reopened for the swap) | MC-CoV ✓ | **queued behind GR1** (avoiding a concurrent edit while the `GammaR` correction ripples) |
...
| GR1 | **the definitional correction CB-W's refutation forces** — and it is a BUG, not a G-AX matter: the campaign doc §3 says verbatim *"A candidate presentation always means an **admissible** presentation … A bare two-relator profinite quotient is not interchangeable with this definition"*, and plan.md §1 states the target as `⟨σ,τ,x₀…xₙ ∣ τ^σ=τ^{q_K}, R=1, ⟪x₀…xₙ⟫-closure pro-2⟩`, but `TameBoundary.lean:333` defines `GammaR` as the bare two-relator `profinitePresentation`. Redefine as the admissible limit (precedent: `GQ2/GammaA.lean:211`'s `N_A`), discharge `hwild` via CB-W's §7 bridge, keep the whole Prop 3.4/Thm 3.5 chain + AS1 + CB-P + CB-0 compiling | opus | `GQ2/Dyadic/TameBoundary.lean` (in-place, approved) ± a new leaf | CB-W ✓ | **dispatched 2026-08-01** (wn0 worktree, `dyadic-gr1`) — **critical path: AS2 and CB fields 4–7/12–15 are moot until this lands** |
