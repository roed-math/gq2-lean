...
(handle-only, core-only, σ/x₀-only, x₀-only) it returns **INFEASIBLE at 60 of the 63**
markings at class 3, and it reproduces the committed `D₄` / `D₈` refutations of the
restricted `U`/`V`-word families.  So "solvable" is a measurement, not an artefact.
...
* An **infeasible** verdict would need the greedy to be complete.  It is **not** guaranteed
  ...
  level `j+1` could in principle be repaired by a different choice inside `ker δ_j`.  No
  infeasible verdict arose in the main sweep, so this never mattered there.  Where the
  restricted-regime probe (§8) reports INFEASIBLE, 48 of the 60 cases are exact — the
  class-two gate itself is *empty*, and Stage A is exhaustive.
...
scripts/w50_depth_sweep/
  padic_c0.py               the 2-adic constants; emits c0.m
  c0.m                      C0 := sqPivotExp mod 2^64
  ...
  w50_regime.m              discrete-shadow probe        magma -b cls:=3 out:=reg_c3.txt w50_regime.m
  survivors.m               survivor structure + W48 forced-alone run
  indep_class2.py           independent class-two model (no group-theory library)
