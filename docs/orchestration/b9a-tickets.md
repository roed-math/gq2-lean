...
| W2c | ⌣ notation sweep at remaining consumer sites (`SectionSix.lemma_6_16`, `HilbertLedger`, `Shapiro/Deepness`) | opus | those files, display-only rewrites | — (independent) | **done 2026-07-24** — surface-only `a ⌣[h] b`: `HilbertLedger` 12 sites (incl. `trivialCupPairing_comm`, ledger lemmas, deep/mid), `LocalKummer` 1, `DeepDualityK` 2, `StiefelWhitney` 1 (`show`), `Demushkin` 6 (API + `IsDemushkin` nondegen fields + CyclicTwo example). `SectionSix`/`Shapiro/Deepness`: no convertible sites (only prose `⌣`/comments; SectionSix has no `trivialCupPairing`). Skipped: bundled-hom `set C :=` (partial app) + `_comm` lemma-name refs. Build green 3308 jobs; elaborated terms unchanged |
