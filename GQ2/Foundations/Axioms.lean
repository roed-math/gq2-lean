...
Lemma 10.1, …) are **not** here — they are the paper's contribution and are proved in the
section files culminating in `main_surjection_count'`.

**How to read this for review.**  Each `axiom` below packages published input; direct leaves
match a cited theorem, while composite leaves explicitly record the additional normalization
and dictionary steps.  Each docstring gives the precise Lean statement, citations, and the
...
the unused B4 was deleted on 2026-07-10, census 10 → 9), faithfully stated against
...
* **B10** `tameQuotient` — the tame quotient of `G_ℚ₂` (Iwasawa, NSW (7.5.3)), **oriented
  ...
  `ker ν_t`; `rec(2)` ↦ `ztwoOne⁻¹`; Serre, *Local Fields*, Ch. XIII §4, Prop. 13 and its
  corollary, plus Neukirch ANT V (1.2)) (defs + convention/deviation
...
(the multiplicative-root-of-unity form); over `ℚ_p` also CiA [CiA] Ch. III §1.1 Prop. 1.
For unramified norms use Ch. V §2.  Paper: §6.3, displays (93)/(94) and Lemma 6.16's proof. -/
