...
/-!
# The crossed `V`-cocycle layer  (P-16d6c1a)
...
* the **`B¹`-translation facts** — conjugating an `M`-lift `f` by `m ∈ M` keeps it over the same
  ...
  the Bug-1 recalibration (`docs/p16d6c-handoff.md` §⚠): the central `red_T`-image is a free
...
@[simp] theorem vCob_c (DD : DescData D) (ρ : ContinuousMonoidHom Γ (Bg ⧸ D.M)) (v : DD.Vmod) :
...
@[simp] theorem vCob_zero : vCob DD ρ (0 : DD.Vmod) = 0 := by
...
@[simp] theorem mConj_apply (m : Bg) (hm : m ∈ D.M) (f : MLifts D ρ) (γ : Γ) :
...
include hσ in
/-- **The semidirect bijection** `Z¹_{Γ,ρ}(V) ≃ {Γ →ₜ Q over ρ'}` (P-16d6c1a): via a splitting
...
include hσ in
/-- **The `B¹`-translation fact** (P-16d6c1a, the algebraic core of Bug 1): conjugating an
