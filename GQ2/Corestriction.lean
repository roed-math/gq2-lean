...
* `H2ofFun`/`H1ofFun` are **junk-total class formers**: they send a raw function to its
  cohomology class when it is a (continuous) cocycle and to `0` otherwise.  Statement tickets used
  them to *define* classes while their cocycle properties were proved downstream (the
  paper's Lemma 6.1/6.15 content), keeping all `def`s total.

Cocycle membership, Mackey restriction, and transversal-independence are proved in the P-15
downstream layer.  This file is `sorry`-free.
-/
...
otherwise.  These let statement files *define* classes before importing their downstream
cocycle proofs, without any partial definition. -/
