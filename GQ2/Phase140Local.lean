...
omit [CompactSpace AbsGalQ2] [TotallyDisconnectedSpace AbsGalQ2] [IsTopologicalGroup AbsGalQ2]
  [DistribMulAction AbsGalQ2 (ZMod 2)] [ContinuousSMul AbsGalQ2 (ZMod 2)] in
/-- **No nonzero conjugation-invariant character of `M_B`** — the paper's `Hom_C(M, 𝔽₂) = 0`
(Lemma 7.1 through Prop 7.4's proof, p. 38), in the raw-conjugation form `hpartial_local`
consumes.  Private copy of the `hfix` bridge inside `Half139Local.hMcountM_local`
(promotion to a shared statement deferred, per the leaf-copy convention): a nonzero invariant
additive character's kernel pulls back to an index-2 `Y`-normal subgroup `X` with
`R ≤ X ≤ K`, refuted by `SectionSeven.lemma_7_1_dual`.  The raw `∀ bb : RF.YB` invariance
hypothesis makes the normality step a one-line application (no coset-rep bookkeeping). -/
private theorem mchar_invariant_eq_zero
    (lam : ↥(En.radData l h).M → ZMod 2)
    (hadd : ∀ m m' : ↥(En.radData l h).M, lam (m * m') = lam m + lam m')
    (hconj : ∀ (bb : RF.YB) (m : ↥(En.radData l h).M),
      lam ⟨bb * m.1 * bb⁻¹, (En.radData l h).hM.conj_mem m.1 m.2 bb⟩ = lam m)
    (m : ↥(En.radData l h).M) : lam m = 0 := by
  classical
  by_contra hlamne
  ...
  have hlam1 : lam 1 = 0 := by
    have hone := hadd 1 1
    rw [mul_one] at hone
    exact add_left_cancel (hone.symm.trans (add_zero _).symm)
  let φ : ↥Blk.K →* Multiplicative (ZMod 2) :=
    { toFun := fun k => Multiplicative.ofAdd (lam (s k))
      map_one' := by rw [map_one, hlam1]; rfl
      map_mul' := fun a b => by rw [map_mul, hadd, ofAdd_add] }
  have hφ_apply : ∀ k, φ k = Multiplicative.ofAdd (lam (s k)) := fun _ => rfl
  have hφne : φ ≠ 1 := by
    intro hφ1
    obtain ⟨k, hk⟩ := hs_surj m
    apply hlamne
    have hk1 : φ k = 1 := by rw [hφ1]; rfl
    have h0 := congrArg Multiplicative.toAdd hk1
    rw [hφ_apply] at h0
    simpa [hk] using h0
  ...
  have hXnormal : X.Normal := by
    ...
    have hsconj : s ⟨y * x * y⁻¹, hyk⟩
        = ⟨RF.piB y * (s ⟨x, hxK⟩).1 * (RF.piB y)⁻¹,
            (En.radData l h).hM.conj_mem _ (s ⟨x, hxK⟩).2 _⟩ := by
      apply Subtype.ext
      rw [hs]
      show RF.piB (y * x * y⁻¹) = RF.piB y * (s ⟨x, hxK⟩).1 * (RF.piB y)⁻¹
      rw [hs, map_mul, map_mul, map_inv]
    rw [hsconj]
    have hinvx := hconj (RF.piB y) (s ⟨x, hxK⟩)
    rw [hinvx]
