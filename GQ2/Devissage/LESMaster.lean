...
include hf hg hinj hsurj hexact in
/-- **Direction (mid)** of `selfdualW_two_of_three`: if the sub `A'` and the quotient `A''` are
self-dual, so is the middle `A`.  Three `four_lemma_inj` windows across the duality ladder. -/
private theorem selfdualW_two_of_three_mid (hA₂ : ∀ a : A, a + a = 0) (t : Marking C)
    (ht : t.TameRel) (hw : t.WildRel) (hsd' : IsSelfDualW t A') (hsd'' : IsSelfDualW t A'') :
    IsSelfDualW t A := by
...
include hf hg hinj hsurj hexact in
/-- **Direction (quot)** of `selfdualW_two_of_three`: if the sub `A'` and the middle `A` are
self-dual, so is the quotient `A''`.  Three `four_lemma_inj` windows across the duality ladder. -/
private theorem selfdualW_two_of_three_quot (hA₂ : ∀ a : A, a + a = 0) (t : Marking C)
    (ht : t.TameRel) (hw : t.WildRel) (hsd' : IsSelfDualW t A') (hsdA : IsSelfDualW t A) :
    IsSelfDualW t A'' := by
  -- Torsion on the outer modules and the duals.
  have hA'₂ : ∀ a' : A', a' + a' = 0 := two_torsion_of_injective f hinj hA₂
  have hA''₂ : ∀ a'' : A'', a'' + a'' = 0 := two_torsion_of_surjective g hsurj hA₂
...
include hf hg hinj hsurj hexact in
/-- **Direction (sub)** of `selfdualW_two_of_three`: if the middle `A` and the quotient `A''` are
self-dual, so is the sub `A'`.  Three `four_lemma_inj` windows across the duality ladder. -/
private theorem selfdualW_two_of_three_sub (hA₂ : ∀ a : A, a + a = 0) (t : Marking C)
    (ht : t.TameRel) (hw : t.WildRel) (hsdA : IsSelfDualW t A) (hsd'' : IsSelfDualW t A'') :
    IsSelfDualW t A' := by
  -- Torsion on the outer modules and the duals.
  have hA'₂ : ∀ a' : A', a' + a' = 0 := two_torsion_of_injective f hinj hA₂
  have hA''₂ : ∀ a'' : A'', a'' + a'' = 0 := two_torsion_of_surjective g hsurj hA₂
...
theorem selfdualW_two_of_three (hA₂ : ∀ a : A, a + a = 0) (t : Marking C) (ht : t.TameRel)
  ...
  refine ⟨fun ⟨hsd', hsd''⟩ =>
      selfdualW_two_of_three_mid f g hf hg hinj hsurj hexact hA₂ t ht hw hsd' hsd'',
    fun ⟨hsd', hsdA⟩ =>
      selfdualW_two_of_three_quot f g hf hg hinj hsurj hexact hA₂ t ht hw hsd' hsdA,
    fun ⟨hsdA, hsd''⟩ =>
      selfdualW_two_of_three_sub f g hf hg hinj hsurj hexact hA₂ t ht hw hsdA hsd''⟩
