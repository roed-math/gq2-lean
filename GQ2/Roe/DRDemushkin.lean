...
The three named generators topologically generate `D_R`: the free profinite group on `Fin 3` is
topologically generated by `of 0, of 1, of 2` (`topGen_freeProfiniteGroup`), and `D_R` is a
continuous quotient of it via `drPi` (compose the presentation and pro-2 projections).  Two
continuous homs out of `D_R` therefore agree once they agree on `drS, drX, drY` (`dr_hom_ext`) —
the "characters are determined by generator values" input of `drH1_bijective`-surjectivity. -/

/-- The composite continuous surjection `F₃ ↠ D_R`, sending `of 0, of 1, of 2` to `drS, drX, drY`
(the presentation quotient followed by the maximal pro-2 quotient). -/
private noncomputable def drPi : FreeProfiniteGroup (Fin 3) →* (DR : Type) :=
  (maxProPMk 2 DRFull).toMonoidHom.comp (quotientMk (relatorSubgroup {drRelator})).toMonoidHom

private theorem drPi_continuous : Continuous drPi :=
  (maxProPMk 2 DRFull).continuous_toFun.comp
    (quotientMk (relatorSubgroup {drRelator})).continuous_toFun

private theorem drPi_surjective : Function.Surjective drPi :=
  (quotientMk_surjective (proPKernel 2 DRFull)).comp
    (quotientMk_surjective (relatorSubgroup {drRelator}))

@[simp] private theorem drPi_of0 : drPi (FreeProfiniteGroup.of 0) = drS := rfl
@[simp] private theorem drPi_of1 : drPi (FreeProfiniteGroup.of 1) = drX := rfl
@[simp] private theorem drPi_of2 : drPi (FreeProfiniteGroup.of 2) = drY := rfl

open CategoryTheory in
/-- The free profinite group on `Fin 3` is topologically generated by its generators (local clone
of `GQ2.topGen_freeProfiniteGroup`/`isTopologicallyFinGen_freeProfiniteGroup`, kept inline to avoid
their heavier import chains). -/
private theorem drFreeGen :
  ...
  have hrange : Subgroup.closure (Set.range (FreeProfiniteGroup.of (X := Fin 3))) = g.range := by
    rw [h1, ← MonoidHom.map_closure, FreeGroup.closure_range_of (Fin 3), ← MonoidHom.range_eq_map]
...
/-- **Continuous homs out of `D_R` are determined by their values on `drS, drX, drY`.**  They
agree on the (algebraic) subgroup generated by the three generators, which is dense because the
free generators of `F₃` topologically generate and `drPi` is a continuous surjection. -/
private theorem dr_hom_ext {H : Type*} [Group H] [TopologicalSpace H] [T2Space H]
    {φ ψ : (DR : Type) →* H} (hφ : Continuous φ) (hψ : Continuous ψ)
    (hs : φ drS = ψ drS) (hx : φ drX = ψ drX) (hy : φ drY = ψ drY) : φ = ψ := by
  have hcl : Set.EqOn (⇑(φ.comp drPi)) (⇑(ψ.comp drPi))
      (↑(Subgroup.closure (Set.range (FreeProfiniteGroup.of (X := Fin 3)))) :
        Set (FreeProfiniteGroup (Fin 3))) :=
    MonoidHom.eqOn_closure (by
      rintro _ ⟨i, rfl⟩
      fin_cases i
      · exact hs
      · exact hx
      · exact hy)
  have hdense : Dense (↑(Subgroup.closure (Set.range (FreeProfiniteGroup.of (X := Fin 3)))) :
      Set (FreeProfiniteGroup (Fin 3))) := by
    rw [dense_iff_closure_eq, ← Subgroup.topologicalClosure_coe, drFreeGen,
      Subgroup.coe_top]
  have hcomp : φ.comp drPi = ψ.comp drPi :=
    DFunLike.coe_injective
      (Continuous.ext_on hdense (hφ.comp drPi_continuous) (hψ.comp drPi_continuous) hcl)
  ext g
  obtain ⟨w, rfl⟩ := drPi_surjective g
...
theorem drH1_bijective : Function.Bijective drH1 := by
  ...
  · -- surjectivity: read coordinates off any representing cocycle and rebuild it via `drCharM`
    ...
    have hchar : (drCharM ![z.1 drS, z.1 drX, z.1 drY]).toMonoidHom = φz := by
      refine dr_hom_ext (drCharM _).continuous_toFun hφzcont ?_ ?_ ?_
