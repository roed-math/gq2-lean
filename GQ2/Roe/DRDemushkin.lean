...
noncomputable def drPi : FreeProfiniteGroup (Fin 3) →* (DR : Type) :=
  (maxProPMk 2 DRFull).toMonoidHom.comp (quotientMk (relatorSubgroup {drRelator})).toMonoidHom

theorem drPi_continuous : Continuous drPi :=
  (maxProPMk 2 DRFull).continuous_toFun.comp
    (quotientMk (relatorSubgroup {drRelator})).continuous_toFun

theorem drPi_surjective : Function.Surjective drPi :=
  (quotientMk_surjective (proPKernel 2 DRFull)).comp
    (quotientMk_surjective (relatorSubgroup {drRelator}))

@[simp] theorem drPi_of0 : drPi (FreeProfiniteGroup.of 0) = drS := rfl
@[simp] theorem drPi_of1 : drPi (FreeProfiniteGroup.of 1) = drX := rfl
@[simp] theorem drPi_of2 : drPi (FreeProfiniteGroup.of 2) = drY := rfl
...
theorem dr_hom_ext {H : Type*} [Group H] [TopologicalSpace H] [T2Space H]
