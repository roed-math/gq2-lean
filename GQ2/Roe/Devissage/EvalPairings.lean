...
Mechanical R-spine clone of `GQ2/Devissage/EvalPairings.lean` (campaign decision,
...
`IsSelfDual(W) → IsSelfDual(W)_R`, with R-suffixed public names.  The `(A)` helper `H0w_two_torsion`
and the whole elementary-dual pack are reused from `GQ2.Devissage.*`; `mixedB_zero_left/right ⇒
mixedB_R_zero_left/right` and `prop_5_8_left/right ⇒ prop_5_8_left/right_R`
...
`mixedB_R` pairing — comes separately).  Well-definedness against `im d¹`/`ker d⁰` is exactly
Prop 5.8 (left/right).  Four maps: `chi0_R`/`chi2_R` (primal `A` against dual classes) and their
transposes `chi0T_R`/`chi2T_R` (primal `A^∨` against `A`-classes).  Two are *always* injective
...
theorem H1w_two_torsion_R (t : Marking C) (hA₂ : ∀ a : A, a + a = 0) (h : H1wR (A := A) t) :
    h + h = 0 := by
  obtain ⟨x, rfl⟩ := QuotientAddGroup.mk_surjective h
  show (QuotientAddGroup.mk (x + x) : H1wR (A := A) t) = 0
...
theorem H2w_two_torsion_R (t : Marking C) (hA₂ : ∀ a : A, a + a = 0) (h : H2wR (A := A) t) :
    h + h = 0 := by
  obtain ⟨p, rfl⟩ := QuotientAddGroup.mk_surjective h
  show (QuotientAddGroup.mk (p + p) : H2wR (A := A) t) = 0
...
`B(d⁰a, y) = B(0, y) = 0`. -/
noncomputable def chi0_R (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR) :
    H0w (A := A) t →+ ElemDual (H2wR (A := ElemDual A) t) where
  toFun a := QuotientAddGroup.lift _
    ...
    show (d1FunR (A := ElemDual A) t y).1 a.1 + (d1FunR (A := ElemDual A) t y).2 a.1 = 0
    have h1 := prop_5_8_left_R t ht hw a.1 y
    rw [AddMonoidHom.mem_ker.mp a.2, mixedB_R_zero_left] at h1
...
noncomputable def chi2_R (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR) :
    H2wR (A := A) t →+ ElemDual (H0w (A := ElemDual A) t) :=
  QuotientAddGroup.lift _
    ...
    apply ElemDual.ext
    intro lam
    show lam.1 ((d1FunR t x).1 + (d1FunR t x).2) = 0
    have h1 := prop_5_8_right_R t ht hw x lam.1
    rw [AddMonoidHom.mem_ker.mp lam.2, mixedB_R_zero_right] at h1
    exact h1.symm

/-- **`χ⁰` transposed**: `H⁰w(A^∨) →+ (H²w(A))^∨`, `λ ↦ ([(u,v)] ↦ λ(u+v))`.  Well-defined by
Prop 5.8 (right), like `chi2_R` with the roles of the arguments exchanged. -/
noncomputable def chi0T_R (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR) :
    H0w (A := ElemDual A) t →+ ElemDual (H2wR (A := A) t) where
...
by Prop 5.8 (left), like `chi0_R` with the roles exchanged. -/
noncomputable def chi2T_R (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR) :
    H2wR (A := ElemDual A) t →+ ElemDual (H0w (A := A) t) :=
  QuotientAddGroup.lift _
    ({ toFun := fun q =>
...
theorem chi0_injective_R (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR)
    (hA₂ : ∀ a : A, a + a = 0) : Function.Injective (chi0_R (A := A) t ht hw) := by
  intro a b hab
  apply Subtype.ext
  by_contra hne
...
theorem chi0T_injective_R (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR) :
    Function.Injective (chi0T_R (A := A) t ht hw) := by
  intro lam mu h
  apply Subtype.ext
  apply ElemDual.ext
  intro u
...
theorem chi2_surjective_R (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR)
    (hA₂ : ∀ a : A, a + a = 0) : Function.Surjective (chi2_R (A := A) t ht hw) := by
  intro psi
...
theorem chi2T_surjective_R (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR)
    (hA₂ : ∀ a : A, a + a = 0) : Function.Surjective (chi2T_R (A := A) t ht hw) := by
  intro psi
...
theorem chi2_square_R (φ : A →+ B) (hφ : ∀ (c : C) (a : A), φ (c • a) = c • φ a)
    (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR) (h : H2wR (A := A) t) :
    chi2_R (A := B) t ht hw (H2wMap_R t φ hφ h)
      = dualMap (H0wMap t (dualMap φ) (dualMap_equivariant φ hφ))
          (chi2_R (A := A) t ht hw h) := by
...
/-- The `χ⁰` square: `χ⁰_B ∘ H⁰wMap φ = (H²wMap φ^∨)^∨ ∘ χ⁰_A`. -/
theorem chi0_square_R (φ : A →+ B) (hφ : ∀ (c : C) (a : A), φ (c • a) = c • φ a)
    (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR) (a : H0w (A := A) t) :
    chi0_R (A := B) t ht hw (H0wMap t φ hφ a)
      = dualMap (H2wMap_R t (dualMap φ) (dualMap_equivariant φ hφ))
...
theorem chi0T_square_R (φ : A →+ B) (hφ : ∀ (c : C) (a : A), φ (c • a) = c • φ a)
    (t : Marking C) (ht : t.TameRel) (hw : t.WildRelR) (lam : H0w (A := ElemDual B) t) :
    chi0T_R (A := A) t ht hw (H0wMap t (dualMap φ) (dualMap_equivariant φ hφ) lam)
      = dualMap (H2wMap_R t φ hφ) (chi0T_R (A := B) t ht hw lam) := by
  apply ElemDual.ext
  intro h
  obtain ⟨z, rfl⟩ := QuotientAddGroup.mk_surjective h
