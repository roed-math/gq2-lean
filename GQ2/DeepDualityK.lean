...
/-- `SectionSix.IsDeepUnit` is antitone in the subgroup: only the two fixedness fields
mention `N`. -/
private theorem isDeepUnit_of_le {N N' : Subgroup (Kummer.GaloisGroup ℚ_[2])}
    (hle : ∀ x ∈ N', x ∈ N) {A : ℚ̄₂} (hA : SectionSix.IsDeepUnit N A) :
    SectionSix.IsDeepUnit N' A := by
  obtain ⟨hA0, hAfix, b, hbfix, hAeq, hb⟩ := hA
  exact ⟨hA0, fun g hg => hAfix g (hle g hg), b, fun g hg => hbfix g (hle g hg), hAeq, hb⟩

/-- `IsMidUnit` is antitone in the subgroup (the `≤`-mirror of `isDeepUnit_of_le`). -/
private theorem isMidUnit_of_le {N N' : Subgroup (Kummer.GaloisGroup ℚ_[2])}
    (hle : ∀ x ∈ N', x ∈ N) {A : ℚ̄₂} (hA : IsMidUnit N A) : IsMidUnit N' A := by
  obtain ⟨hA0, hAfix, b, hbfix, hAeq, hb⟩ := hA
  exact ⟨hA0, fun g hg => hAfix g (hle g hg), b, fun g hg => hbfix g (hle g hg), hAeq, hb⟩
...
classes of `N`-fixed squares vanishes, the corresponding `pairingK`-value over `ker ρ`
vanishes — the `k`-side coboundary witness transports through `kerToFixing` and the
`𝔽₂ ≃+ μ₂` bridge, and the injective `inv_K` kills the class. -/
private theorem pairingK_kummer_eq_zero (k : IntermediateField ℚ_[2] ℚ̄₂)
    [FiniteDimensional ℚ_[2] k]
    (htriv : ∀ (g : k.fixingSubgroup) (m : ZMod 2), g • m = m)
    (hker : ∀ x : Kummer.GaloisGroup ℚ_[2],
      x ∈ (ρ.toMonoidHom.ker : Subgroup AbsGalQ2) ↔ x ∈ k.fixingSubgroup)
    {A B β δ : ℚ̄₂} (hsqA : β ^ 2 = A) (hβ0 : β ≠ 0)
    (hAfix : ∀ g : Kummer.GaloisGroup ℚ_[2],
      g ∈ (ρ.toMonoidHom.ker : Subgroup AbsGalQ2) → g • A = A)
    (hsqB : δ ^ 2 = B) (hδ0 : δ ≠ 0)
    ...
    pairingK ρ
      ...
      (H1ofFun ↥(ρ.toMonoidHom.ker : Subgroup AbsGalQ2) fun n =>
        Kummer.kummerCocycleFun δ (n : AbsGalQ2)) = 0 := by
...
theorem pairingK_deep_deep (k : IntermediateField ℚ_[2] ℚ̄₂) [FiniteDimensional ℚ_[2] k]
  ...
  -- the Tier-5 vanishing over `k` (unit data transported to the `k`-side view), then the
  -- shared witness-transport tail
  exact pairingK_kummer_eq_zero ρ k htriv hker hsqA hβ0 hdA.2.1 hsqB hδ0 hdB.2.1
    (cup_deepClasses k htriv
