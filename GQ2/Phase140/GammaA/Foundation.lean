...
private theorem charKer_le (χ : ↥(TCharC D)) : charKer χ ≤ D.T :=
...
omit [TopologicalSpace Bg] [DiscreteTopology Bg] in
private theorem exists_val_one (χ : ↥(TCharC D)) (hχ : χ ≠ 0) : ∃ t : ↥D.T, χ.1 t = 1 := by
  by_contra! hall
...
omit [TopologicalSpace Bg] [DiscreteTopology Bg] in
private theorem charWitness_spec (χ : ↥(TCharC D)) (hχ : χ ≠ 0) : χ.1 (charWitness χ hχ) = 1 :=
