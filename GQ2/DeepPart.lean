...
/-- **Self-perpendicular half-count** (converse counting to `selfperp_of_card_sq`, the
Lemma 6.17(dim) skeleton): for a biadditive **nondegenerate** pairing `P` on a finite exp-2
group, a subgroup that *equals* its own perp (`∀ a, (∀ x ∈ X, P x a = 0) ↔ a ∈ X`) has
`#X² = #A`.  Two counting halves: `θ : A →+ Hom(X, 𝔽₂)` has kernel exactly `X`, so
`#A = #im θ · #X ≤ #X²`; and `X` injects into `Hom(A/X, 𝔽₂)` via the descended pairing
(injective by nondegeneracy), so `#X ≤ #(A/X) = #A/#X`. -/
theorem card_sq_of_selfperp {A : Type*} [AddCommGroup A] [Finite A]
    (h2 : ∀ a : A, a + a = 0) (P : A → A → ZMod 2)
    (hPl : ∀ a b c : A, P (a + b) c = P a c + P b c)
    (hPr : ∀ a b c : A, P a (b + c) = P a b + P a c)
    (hnd : ∀ a : A, a ≠ 0 → ∃ b : A, P a b ≠ 0)
    (X : AddSubgroup A)
    (hself : ∀ a : A, (∀ x ∈ X, P x a = 0) ↔ a ∈ X) :
    Nat.card ↥X ^ 2 = Nat.card A := by
  classical
  haveI : Fintype A := Fintype.ofFinite A
  haveI : Fintype ↥X := Fintype.ofFinite _
  -- exponent 2 passes to `X` and `A/X`
  ...
  have h2Q : ∀ y : A ⧸ X, y + y = 0 := by
    ...
    rw [← QuotientAddGroup.mk_add, h2]; rfl
  -- `P` vanishes on `X × X`
  have hXX : ∀ x ∈ X, ∀ y ∈ X, P x y = 0 := fun x hx y hy => (hself y).mpr hy x hx
  -- (i) `θ : A →+ Hom(X, 𝔽₂)`, `ker θ = X` ⟹ `#A ≤ #X²`
  set θ : A →+ (↥X →+ ZMod 2) := AddMonoidHom.mk'
    (fun a => AddMonoidHom.mk' (fun x : ↥X => P ↑x a)
      (fun x y => by rw [AddSubgroup.coe_add, hPl]))
    (fun a b => by
      ext x
      exact hPr ↑x a b) with hθ
  have hkX : θ.ker = X := by
    ext a
    ...
    · intro ha
      rw [hθ] at ha
      refine (hself a).mp fun x hx => ?_
      exact DFunLike.congr_fun ha (⟨x, hx⟩ : ↥X)
    · intro ha
      rw [hθ]
      ext x
      exact (hself a).mpr ha ↑x x.2
  haveI : Finite (↥X →+ ZMod 2) :=
    Finite.of_injective (fun f => (f : ↥X → ZMod 2)) DFunLike.coe_injective
  have hrange : Nat.card ↥θ.range ≤ Nat.card ↥X := by
    calc Nat.card ↥θ.range ≤ Nat.card (↥X →+ ZMod 2) :=
          Nat.card_le_card_of_injective _ Subtype.val_injective
      _ = Nat.card ↥X := card_addHom_zmod2 ↥X h2X
  have hA_le : Nat.card A ≤ Nat.card ↥X * Nat.card ↥X := by
    have hlag : Nat.card A = Nat.card (A ⧸ θ.ker) * Nat.card ↥θ.ker :=
      AddSubgroup.card_eq_card_quotient_mul_card_addSubgroup θ.ker
    have hiso : Nat.card (A ⧸ θ.ker) = Nat.card ↥θ.range :=
      Nat.card_congr (QuotientAddGroup.quotientKerEquivRange θ).toEquiv
    calc Nat.card A = Nat.card ↥θ.range * Nat.card ↥θ.ker := by rw [← hiso, ← hlag]
      _ = Nat.card ↥θ.range * Nat.card ↥X := by rw [hkX]
      _ ≤ Nat.card ↥X * Nat.card ↥X := Nat.mul_le_mul_right _ hrange
  -- (ii) `X ↪ Hom(A/X, 𝔽₂)` via the descended pairing ⟹ `#X² ≤ #A`
  ...
  have hX_le : Nat.card ↥X ≤ Nat.card (A ⧸ X) := by
    ...
    calc Nat.card ↥X ≤ Nat.card (A ⧸ X →+ ZMod 2) :=
      ...
      _ = Nat.card (A ⧸ X) := card_addHom_zmod2 (A ⧸ X) h2Q
  have hsq_le : Nat.card ↥X * Nat.card ↥X ≤ Nat.card A := by
    ...
    calc Nat.card ↥X * Nat.card ↥X ≤ Nat.card (A ⧸ X) * Nat.card ↥X :=
      ...
      _ = Nat.card A := hquot
  rw [sq]
  exact le_antisymm hsq_le hA_le
...
@[simp] theorem polarMuDual_apply (q : V → ZMod 2) (hq : IsQuadraticFp2 q) (v w : V) :
    polarMuDual q hq v w = SectionSix.muTwoOfF2 (polar q v w) := rfl
...
@[simp] theorem mem_deepPartSubgroup (ρ : ContinuousMonoidHom AbsGalQ2 C)
    (hρ : ∀ (g : AbsGalQ2) (v : V), g • v = ρ g • v) (hV2 : ∀ v : V, v + v = 0)
    (x : H1 AbsGalQ2 V) :
    x ∈ deepPartSubgroup ρ hρ hV2 ↔ x ∈ deepPart (V := V) ρ := Iff.rfl

/-! ### The dim clause and Prop 6.18 (ramified), reduced to the two Kummer cores -/

/-- **Lemma 6.17(dim) modulo the deep-half self-perpendicularity** — the (94) Kummer core: if
`X₊` equals its own perp under the polar form of `Q⁰_loc`, then `#X₊² = #H¹`
(`card_sq_of_selfperp` with the (93)-quadratic structure and B6-nonsingularity).
Ax: **B6** (via `D`), **B7** (`finite_H1`). -/
theorem card_deepPart_sq_of_selfperp (D : TateDuality 2)
    (q : V → ZMod 2) (hq : IsQuadraticFp2 q) (hns : Nonsingular q)
    (dat : FactorSet C V) (hdat : IsEquivariantFactorSet q dat)
    (ρ : ContinuousMonoidHom AbsGalQ2 C) (hρ : ∀ (g : AbsGalQ2) (v : V), g • v = ρ g • v)
    (hqG : ∀ (g : AbsGalQ2) (v : V), q (g • v) = q v)
    (hV2 : ∀ v : V, v + v = 0)
    (hself : ∀ x : H1 AbsGalQ2 V,
      (∀ y ∈ deepPartSubgroup ρ hρ hV2, polar (Q0loc D dat ρ) y x = 0)
        ↔ x ∈ deepPartSubgroup ρ hρ hV2) :
  ...
  exact card_sq_of_selfperp (h1_add_self hV2) (fun a b => polar (Q0loc D dat ρ) a b)
    hq'.polar_add_left hq'.polar_add_right hns' (deepPartSubgroup ρ hρ hV2) hself
...
/-- **The trace pairing does not degenerate on the fixed field**: if `c` is not
Frobenius^m-fixed, some fixed `y` has `Tr(c·y) ≠ 0`.  (The perp of `𝔽_{2^m}` has its size by
counting and contains it, so it *is* `𝔽_{2^m}`.) -/
theorem exists_frobFixed_trace_ne_zero {m : ℕ} (hm : 1 ≤ m)
    (hcard : Fintype.card D = 2 ^ (2 * m)) [Algebra (ZMod (ringChar D)) D]
    {c : D} (hc : c ^ 2 ^ m ≠ c) :
    ∃ y : D, y ^ 2 ^ m = y ∧ Algebra.trace (ZMod (ringChar D)) D (c * y) ≠ 0 := by
  have hchar : ringChar D = 2 := ringChar_eq_two_of_card hm hcard
  haveI : CharP D 2 := hchar ▸ ringChar.charP D
  have e2 : ZMod (ringChar D) ≃+ ZMod 2 := by rw [hchar]
  by_contra hall
  push_neg at hall
  apply hc
  -- the `𝔽₂`-valued trace pairing
  set P : D →+ D →+ ZMod 2 := AddMonoidHom.mk'
    (fun z => AddMonoidHom.mk'
      (fun w => e2 (Algebra.trace (ZMod (ringChar D)) D (z * w)))
      (fun w w' => by rw [mul_add, map_add, map_add]))
    (fun z z' => by
      ext w
      show e2 (Algebra.trace _ D ((z + z') * w)) = _
      rw [add_mul, map_add, map_add]
      rfl) with hP
  have h2D : ∀ a : D, a + a = 0 := fun a => CharTwo.add_self_eq_zero a
  have hnd : ∀ z : D, z ≠ 0 → ∃ w : D, P z w ≠ 0 := by
    intro z hz
    obtain ⟨b, hb⟩ := FiniteField.trace_to_zmod_nondegenerate D hz
    refine ⟨b, ?_⟩
    show e2 (Algebra.trace _ D (z * b)) ≠ 0
    intro h0
    exact hb (by simpa using e2.injective (by rw [h0, map_zero]))
  have hX : ∀ x ∈ frobFixed D m, ∀ x' ∈ frobFixed D m, P x x' = 0 := by
    intro x hx x' hx'
    show e2 (Algebra.trace _ D (x * x')) = 0
    have hmul : (x * x') ^ 2 ^ m = x * x' := by
      rw [mul_pow, hx, hx']
    rw [trace_eq_zero_of_frobenius_fixed hm hcard hmul, map_zero]
  have hcardX : Nat.card ↥(frobFixed D m) * Nat.card ↥(frobFixed D m) = Nat.card D := by
    rw [card_frobFixed hm hcard, Nat.card_eq_fintype_card, hcard, two_mul, pow_add]
  have hmem : c ∈ frobFixed D m := by
    refine mem_of_pairing_eq_zero h2D P hnd (frobFixed D m) hX hcardX ?_
    intro y hy
    show e2 (Algebra.trace _ D (c * y)) = 0
    rw [hall y hy, map_zero]
  exact hmem
...
/-- **The Hermitian-line count** (paper Prop 6.18, unramified computation): on
`D = 𝔽_{2^{2m}}` the trace form `x ↦ Tr(c·x^{2^m+1})` with `c` outside the fixed field has
exactly `2^{2m−1} − 2^{m−1}` zeros — the minus-type count
`1 + (2^m+1)(2^{m−1}−1)`. -/
theorem card_hermitian_trace_form_zero {m : ℕ} (hm : 1 ≤ m)
    (hcard : Fintype.card D = 2 ^ (2 * m)) [Algebra (ZMod (ringChar D)) D]
    {c : D} (hc : c ^ 2 ^ m ≠ c) :
    Nat.card {x : D // Algebra.trace (ZMod (ringChar D)) D (c * x ^ (2 ^ m + 1)) = 0}
      = 2 ^ (2 * m - 1) - 2 ^ (m - 1) := by
  classical
  have hchar : ringChar D = 2 := ringChar_eq_two_of_card hm hcard
  haveI : CharP D 2 := hchar ▸ ringChar.charP D
  have hq1 : 1 ≤ 2 ^ m := Nat.one_le_two_pow
  set q : ℕ := 2 ^ m with hqdef
  set Tr : D → ZMod (ringChar D) := fun z => Algebra.trace (ZMod (ringChar D)) D z with hTr
  -- ψ : the norm map on units
  set ψ : Dˣ →* Dˣ := powMonoidHom (q + 1) with hψ
  -- kernel size q+1
  have hcardU : Nat.card Dˣ = 2 ^ (2 * m) - 1 := by
    rw [Nat.card_eq_fintype_card, Fintype.card_units, hcard]
  have hsq : 2 ^ (2 * m) = q ^ 2 := by rw [hqdef, ← pow_mul, mul_comm]
  have hdvd1 : (q + 1) ∣ (2 ^ (2 * m) - 1) := by
    refine ⟨q - 1, ?_⟩
    have hsub := Nat.sq_sub_sq q 1
    rw [hsq]
    simpa using hsub
  have hkerψ : Nat.card ↥ψ.ker = q + 1 := by
    rw [hψ, IsCyclic.card_powMonoidHom_ker, hcardU, Nat.gcd_eq_right hdvd1]
  -- range ψ = the Frobenius-fixed units (by counting)
  have hrangesub : ∀ y : Dˣ, y ∈ ψ.range → (↑y : D) ^ q = ↑y := by
    rintro _ ⟨x, rfl⟩
    show ((↑(x ^ (q + 1)) : D)) ^ q = ↑(x ^ (q + 1))
    rw [Units.val_pow_eq_pow_val, ← pow_mul]
    have hxq : (↑x : D) ^ (q * q) = ↑x := by
      have := FiniteField.pow_card (↑x : D)
      rwa [hcard, hsq, sq] at this
    have hstep : (↑x : D) ^ ((q + 1) * q) = ((↑x : D) ^ (q * q)) * (↑x : D) ^ q := by
      rw [← pow_add]
      congr 1
      ring
    rw [hstep, hxq, ← pow_succ']
  have hdvd2 : (q - 1) ∣ (2 ^ (2 * m) - 1) := by
    refine ⟨q + 1, ?_⟩
    have hsub := Nat.sq_sub_sq q 1
    rw [hsq]
    simpa [mul_comm] using hsub
  have hrangecard : Nat.card ↥ψ.range = q - 1 := by
    rw [hψ, IsCyclic.card_powMonoidHom_range, hcardU, Nat.gcd_eq_right hdvd1]
    have h1q : 1 ≤ q := Nat.one_le_two_pow
    have : 2 ^ (2 * m) - 1 = (q + 1) * (q - 1) := by
      have hsub := Nat.sq_sub_sq q 1
      rw [hsq]
      simpa using hsub
    rw [this, Nat.mul_div_cancel_left _ (by omega)]
  have hfixcard : Nat.card ↥((powMonoidHom (q - 1) : Dˣ →* Dˣ).ker) = q - 1 := by
    rw [IsCyclic.card_powMonoidHom_ker, hcardU, Nat.gcd_eq_right hdvd2]
  have hrangele : ψ.range ≤ (powMonoidHom (q - 1) : Dˣ →* Dˣ).ker := by
    intro y hy
    rw [MonoidHom.mem_ker, powMonoidHom_apply]
    apply Units.ext
    rw [Units.val_pow_eq_pow_val, Units.val_one]
    have h1q : 1 ≤ q := Nat.one_le_two_pow
    have hfix := hrangesub y hy
    have hcancel : (↑y : D) ^ (q - 1) * ↑y = 1 * ↑y := by
      rw [one_mul, ← pow_succ, show q - 1 + 1 = q from by omega]
      exact hfix
    exact mul_right_cancel₀ y.ne_zero hcancel
  have hrangeeq : (ψ.range : Set Dˣ) = ((powMonoidHom (q - 1) : Dˣ →* Dˣ).ker : Set Dˣ) := by
    refine Set.eq_of_subset_of_ncard_le (fun y hy => hrangele hy) ?_ (Set.toFinite _)
    rw [← Nat.card_coe_set_eq, ← Nat.card_coe_set_eq]
    show Nat.card ↥((powMonoidHom (q - 1) : Dˣ →* Dˣ).ker) ≤ Nat.card ↥ψ.range
    rw [hfixcard, hrangecard]
  have hrange_iff : ∀ y : Dˣ, y ∈ ψ.range ↔ (↑y : D) ^ q = ↑y := by
    intro y
    constructor
    · exact hrangesub y
    · intro hfix
      have hmem : y ∈ (powMonoidHom (q - 1) : Dˣ →* Dˣ).ker := by
        rw [MonoidHom.mem_ker, powMonoidHom_apply]
        apply Units.ext
        rw [Units.val_pow_eq_pow_val, Units.val_one]
        have h1q : 1 ≤ q := Nat.one_le_two_pow
        have hcancel : (↑y : D) ^ (q - 1) * ↑y = 1 * ↑y := by
          ...
          exact hfix
        exact mul_right_cancel₀ y.ne_zero hcancel
      have : y ∈ (ψ.range : Set Dˣ) := hrangeeq ▸ hmem
      exact this
  -- the trace-kernel of the fixed field: `2^{m−1}` elements, `2^{m−1} − 1` nonzero
  have e2 : ZMod (ringChar D) ≃+ ZMod 2 := by rw [hchar]
  set ℓ : ↥(frobFixed D m) →+ ZMod 2 := AddMonoidHom.mk'
    (fun y => e2 (Tr (c * ↑y)))
    (fun y y' => by
      show e2 (Tr (c * ↑(y + y'))) = _
      rw [show ((↑(y + y') : D)) = ↑y + ↑y' from rfl, mul_add]
      rw [hTr]
      show e2 (Algebra.trace _ D (c * ↑y + c * ↑y')) = _
      rw [map_add, map_add]) with hℓ
  have hℓne : ∃ y₀ : ↥(frobFixed D m), ℓ y₀ ≠ 0 := by
    obtain ⟨y, hyfix, hyne⟩ := exists_frobFixed_trace_ne_zero hm hcard hc
    refine ⟨⟨y, hyfix⟩, ?_⟩
    show e2 (Tr (c * y)) ≠ 0
    intro h0
    exact hyne (by simpa using e2.injective (by rw [h0, map_zero]))
  have hkerℓ : Nat.card ↥ℓ.ker = 2 ^ (m - 1) := by
    have hrangeℓ : ℓ.range = ⊤ := by
      rw [AddSubgroup.eq_top_iff']
      intro z
      rcases (show z = 0 ∨ z = 1 from by revert z; decide) with rfl | rfl
      · exact zero_mem _
      · obtain ⟨y₀, hy₀⟩ := hℓne
        have : ℓ y₀ = 1 := by
          rcases (show ℓ y₀ = 0 ∨ ℓ y₀ = 1 from by revert hy₀; generalize ℓ y₀ = w; revert w; decide) with h | h
          · exact absurd h hy₀
          · exact h
        exact ⟨y₀, this⟩
    have hrn : Nat.card ↥ℓ.range * Nat.card ↥ℓ.ker = Nat.card ↥(frobFixed D m) := by
      rw [← Nat.card_congr (QuotientAddGroup.quotientKerEquivRange ℓ).toEquiv]
      exact (AddSubgroup.card_eq_card_quotient_mul_card_addSubgroup ℓ.ker).symm
    have hr2 : Nat.card ↥ℓ.range = 2 := by
      rw [hrangeℓ, Nat.card_congr AddSubgroup.topEquiv.toEquiv, Nat.card_zmod]
    rw [hr2, card_frobFixed hm hcard] at hrn
    have hpow : 2 ^ m = 2 * 2 ^ (m - 1) := by
      rw [← pow_succ', show m - 1 + 1 = m from by omega]
    rw [hpow] at hrn
    exact Nat.eq_of_mul_eq_mul_left (by norm_num) hrn
  -- the final count
  have hcond0 : Tr (c * (0 : D) ^ (q + 1)) = 0 := by
    rw [zero_pow (by omega : q + 1 ≠ 0), mul_zero, hTr]
    show Algebra.trace _ D 0 = 0
    rw [map_zero]
  rw [Nat.card_eq_fintype_card, Fintype.card_subtype]
  have hzero_mem : (0 : D) ∈ Finset.univ.filter (fun x : D => Tr (c * x ^ (q + 1)) = 0) := by
    simp only [Finset.mem_filter]
    exact ⟨Finset.mem_univ _, hcond0⟩
  have hsplit : Finset.univ.filter (fun x : D => Tr (c * x ^ (q + 1)) = 0)
      = insert (0 : D)
          ((Finset.univ.filter (fun x : D => Tr (c * x ^ (q + 1)) = 0)).erase 0) := by
    rw [Finset.insert_erase hzero_mem]
  rw [hsplit, Finset.card_insert_of_notMem (Finset.notMem_erase 0 _)]
  -- nonzero part ↔ units with the ψ-condition
  have hunits : ((Finset.univ.filter (fun x : D => Tr (c * x ^ (q + 1)) = 0)).erase 0).card
      = (Finset.univ.filter (fun u : Dˣ => Tr (c * ↑(ψ u)) = 0)).card := by
    symm
    refine Finset.card_bij (fun u _ => (↑u : D)) ?_ ?_ ?_
    · intro u hu
      rw [Finset.mem_filter] at hu
      rw [Finset.mem_erase, Finset.mem_filter]
      refine ⟨u.ne_zero, Finset.mem_univ _, ?_⟩
      have : (↑(ψ u) : D) = (↑u : D) ^ (q + 1) := by
        rw [hψ]
        exact Units.val_pow_eq_pow_val u (q + 1)
      rw [← this]
      exact hu.2
    · intro u _ v _ huv
      exact Units.ext huv
    · intro x hx
      rw [Finset.mem_erase, Finset.mem_filter] at hx
      obtain ⟨hx0, _, hxc⟩ := hx
      refine ⟨Units.mk0 x hx0, ?_, rfl⟩
      rw [Finset.mem_filter]
      refine ⟨Finset.mem_univ _, ?_⟩
      have : (↑(ψ (Units.mk0 x hx0)) : D) = x ^ (q + 1) := by
        rw [hψ]
        show (↑((Units.mk0 x hx0) ^ (q + 1)) : D) = x ^ (q + 1)
        rw [Units.val_pow_eq_pow_val, Units.val_mk0]
      rw [this]
      exact hxc
  rw [hunits]
  -- fibre the units count over the range of ψ
  have hfib : (Finset.univ.filter (fun u : Dˣ => Tr (c * ↑(ψ u)) = 0)).card
      = (q + 1) * ((Finset.univ.filter
          (fun y : Dˣ => Tr (c * ↑y) = 0 ∧ y ∈ ψ.range)).card) := by
    have hkerfilter : (Finset.univ.filter (fun u : Dˣ => u ∈ ψ.ker)).card = q + 1 := by
      rw [← hkerψ, Nat.card_eq_fintype_card, Fintype.card_subtype]
    have hmapsto : ∀ u ∈ Finset.univ.filter (fun u : Dˣ => Tr (c * ↑(ψ u)) = 0),
        ψ u ∈ Finset.univ.filter (fun y : Dˣ => Tr (c * ↑y) = 0 ∧ y ∈ ψ.range) := by
      intro u hu
      exact Finset.mem_filter.mpr
        ⟨Finset.mem_univ _, (Finset.mem_filter.mp hu).2, ⟨u, rfl⟩⟩
    rw [Finset.card_eq_sum_card_fiberwise hmapsto]
    have hpointwise : ∀ y ∈ Finset.univ.filter
        (fun y : Dˣ => Tr (c * ↑y) = 0 ∧ y ∈ ψ.range),
        ((Finset.univ.filter (fun u : Dˣ => Tr (c * ↑(ψ u)) = 0)).filter
          (fun u => ψ u = y)).card = q + 1 := by
      intro y hy
      have hy' := (Finset.mem_filter.mp hy).2
      have hfiber : (Finset.univ.filter (fun u : Dˣ => Tr (c * ↑(ψ u)) = 0)).filter
            (fun u => ψ u = y) = Finset.univ.filter (fun u : Dˣ => ψ u = y) := by
        ext u
        ...
        · rintro ⟨_, h⟩
          exact h
        · intro h
          exact ⟨by rw [h]; exact hy'.1, h⟩
      rw [hfiber, card_filter_eq_of_mem_range ψ hy'.2, hkerfilter]
    rw [Finset.sum_congr rfl hpointwise, Finset.sum_const, smul_eq_mul, mul_comm]
  rw [hfib]
  -- identify the base count with the nonzero trace-kernel of the fixed field
  have hbase : (Finset.univ.filter
        (fun y : Dˣ => Tr (c * ↑y) = 0 ∧ y ∈ ψ.range)).card
      = 2 ^ (m - 1) - 1 := by
    have hbij : (Finset.univ.filter
          (fun y : Dˣ => Tr (c * ↑y) = 0 ∧ y ∈ ψ.range)).card
        = (Finset.univ.filter
            (fun s : ↥(frobFixed D m) => ℓ s = 0 ∧ (↑s : D) ≠ 0)).card := by
      refine Finset.card_bij (fun y hy => ⟨(↑y : D), ?_⟩) ?_ ?_ ?_
      · rw [Finset.mem_filter] at hy
        exact (hrange_iff y).mp hy.2.2
      · intro y hy
        rw [Finset.mem_filter] at hy ⊢
        refine ⟨Finset.mem_univ _, ?_, y.ne_zero⟩
        show e2 (Tr (c * ↑y)) = 0
        rw [hy.2.1, map_zero]
      · intro y _ v _ hyv
        have : (↑y : D) = ↑v := congrArg Subtype.val hyv
        exact Units.ext this
      · intro s hs
        rw [Finset.mem_filter] at hs
        obtain ⟨_, hs0, hsne⟩ := hs
        refine ⟨Units.mk0 (↑s : D) hsne, ?_, ?_⟩
        · rw [Finset.mem_filter]
          refine ⟨Finset.mem_univ _, ?_, ?_⟩
          · have : e2 (Tr (c * ↑s)) = 0 := hs0
            have h0 : Tr (c * (↑s : D)) = 0 := by
              apply e2.injective
              rw [this, map_zero]
            exact h0
          · rw [hrange_iff]
        ...
        · apply Subtype.ext
          rfl
    rw [hbij]
    -- the zero element accounts for the −1
    have hker_split : (Finset.univ.filter
          (fun s : ↥(frobFixed D m) => ℓ s = 0)).card = 2 ^ (m - 1) := by
      rw [← Fintype.card_subtype, ← Nat.card_eq_fintype_card]
      have : Nat.card {s : ↥(frobFixed D m) // ℓ s = 0} = Nat.card ↥ℓ.ker := by
        apply Nat.card_congr
        exact Equiv.subtypeEquivRight (fun s => by rw [AddMonoidHom.mem_ker])
      rw [this, hkerℓ]
    have hzero_in : (⟨0, (frobFixed D m).zero_mem⟩ : ↥(frobFixed D m))
        ∈ Finset.univ.filter (fun s : ↥(frobFixed D m) => ℓ s = 0) := by
      rw [Finset.mem_filter]
      refine ⟨Finset.mem_univ _, ?_⟩
      show e2 (Tr (c * ↑(⟨0, _⟩ : ↥(frobFixed D m)))) = 0
      rw [show ((↑(⟨0, (frobFixed D m).zero_mem⟩ : ↥(frobFixed D m))) : D) = 0 from rfl,
        mul_zero, hTr]
      show e2 (Algebra.trace _ D 0) = 0
      rw [map_zero, map_zero]
    have hfilter_eq : Finset.univ.filter
          (fun s : ↥(frobFixed D m) => ℓ s = 0 ∧ (↑s : D) ≠ 0)
        = (Finset.univ.filter (fun s : ↥(frobFixed D m) => ℓ s = 0)).erase
            ⟨0, (frobFixed D m).zero_mem⟩ := by
      ext s
    ...
    rw [hfilter_eq, Finset.card_erase_of_mem hzero_in, hker_split]
  rw [hbase]
  -- arithmetic: `1 + (q+1)(2^{m−1}−1) = 2^{2m−1} − 2^{m−1}` — wait: `card_insert` put the 1 last
  have hx1 : 1 ≤ 2 ^ (m - 1) := Nat.one_le_two_pow
  obtain ⟨k, hk⟩ : ∃ k, 2 ^ (m - 1) = k + 1 := ⟨2 ^ (m - 1) - 1, by omega⟩
  have hqk : q = 2 * (k + 1) := by
    rw [hqdef, ← hk, ← pow_succ', show m - 1 + 1 = m from by omega]
  have h2m1 : 2 ^ (2 * m - 1) = q * (k + 1) := by
    rw [hqdef, ← hk, ← pow_add, show m + (m - 1) = 2 * m - 1 from by omega]
  rw [hk, hqk, h2m1, hqk]
  have hexpand : (2 * (k + 1) + 1) * (k + 1 - 1) + 1
      = 2 * (k + 1) * (k + 1) - (k + 1) := by
    have hsub1 : k + 1 - 1 = k := by omega
    rw [hsub1]
    have hgoal : (2 * (k + 1) + 1) * k + 1 + (k + 1) = 2 * (k + 1) * (k + 1) := by ring
    omega
  omega
...
/-- **The unramified count for any norm-one-invariant nonsingular form** (Lemma 6.7 + the
Hermitian count): `#Q⁻¹(0) = 2^{2m−1} − 2^{m−1}` — the paper's Prop 6.18 minus-type count,
model-free. -/
theorem card_normOne_invariant_form_zero {m : ℕ} (hm : 1 ≤ m)
    (hcard : Fintype.card D = 2 ^ (2 * m)) [Algebra (ZMod (ringChar D)) D]
    (Q : D → ZMod 2) (hQ : IsQuadraticFp2 Q) (hns : Nonsingular Q)
    (hU : ∀ u : Dˣ, u ^ (2 ^ m + 1) = 1 → ∀ x : D, Q (↑u * x) = Q x) :
    Nat.card {x : D // Q x = 0} = 2 ^ (2 * m - 1) - 2 ^ (m - 1) := by
  have hchar : ringChar D = 2 := ringChar_eq_two_of_card hm hcard
  have e2 : ZMod (ringChar D) ≃+ ZMod 2 := by rw [hchar]
  obtain ⟨c, hcne, hQeq⟩ := hermitian_form_eq_trace_form hm hcard e2 Q hQ hns hU
  have hcong : Nat.card {x : D // Q x = 0}
      = Nat.card {x : D //
          Algebra.trace (ZMod (ringChar D)) D (c * x ^ (2 ^ m + 1)) = 0} := by
    apply Nat.card_congr
    refine Equiv.subtypeEquivRight fun x => ?_
    rw [hQeq x]
    constructor
    · intro h
      exact e2.injective (h.trans (map_zero e2).symm)
    · intro h
      rw [h, map_zero]
  rw [hcong]
  exact card_hermitian_trace_form_zero hm hcard hcne
