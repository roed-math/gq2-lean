...
The final section isolates a second route: if scalar `H²` is generated by degree-one cup
products, then scalar `H¹`-vanishing forces scalar `H²`-vanishing.  The cup-generation
hypothesis is an explicit interface; this file does not import a norm-residue theorem.
-/
...
section ScalarCupGeneration

variable {G : Type}
  [Group G] [TopologicalSpace G] [IsTopologicalGroup G]
  [DistribMulAction G (ZMod 2)] [ContinuousSMul G (ZMod 2)]

/-- Degree-two mod-two cohomology is generated additively by symbols (cup products of
degree-one classes).  This is the precise cohomological conclusion supplied by the degree-two
norm-residue theorem for absolute Galois groups; no norm-residue theorem is assumed here. -/
def ScalarH2GeneratedByCupProducts
    (htriv : ∀ (g : G) (a : ZMod 2), g • a = a) : Prop :=
  ∀ z : H2 G (ZMod 2),
    ∃ (n : ℕ) (x y : Fin n → H1 G (ZMod 2)),
      z = ∑ i, GQ2.trivialCupPairing 2 G htriv (x i) (y i)

/-- If scalar `H¹` vanishes and scalar `H²` is generated by cup products, scalar `H²`
vanishes. -/
theorem continuousH2Vanishes_of_h1Vanishes_of_cupGenerated
    (htriv : ∀ (g : G) (a : ZMod 2), g • a = a)
    (hH1 : ∀ x : H1 G (ZMod 2), x = 0)
    (hgen : ScalarH2GeneratedByCupProducts (G := G) htriv) :
    ContinuousH2Vanishes G (ZMod 2) := by
  intro z
...
/-- Cochain-level `H¹`-vanishing is enough for the preceding cup-generation criterion. -/
theorem continuousH2Vanishes_of_h1CocyclesVanish_of_cupGenerated
    (htriv : ∀ (g : G) (a : ZMod 2), g • a = a)
    (hH1 : ∀ z : Z1 G (ZMod 2), z = 0)
    (hgen : ScalarH2GeneratedByCupProducts (G := G) htriv) :
    ContinuousH2Vanishes G (ZMod 2) := by
  apply continuousH2Vanishes_of_h1Vanishes_of_cupGenerated htriv ?_ hgen
  intro x
  obtain ⟨z, rfl⟩ := H1mk_surjective (G := G) (M := ZMod 2) x
  simpa using congrArg (H1mk G (ZMod 2)) (hH1 z)

end ScalarCupGeneration
