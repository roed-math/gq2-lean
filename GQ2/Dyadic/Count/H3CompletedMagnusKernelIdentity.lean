...
/-! ## The exact degree-two relation-ideal comparison -/

/-- Replace every completed coefficient by the scalar given by its augmentation.  In degree two
this is the only part of the coefficient visible modulo `J³`. -/
def sqCompletedQuadraticScalarizedCoefficient (h : ℕ)
    (c : FiniteGeneratorWord (Fin (sqRank h)) 2 →
      ModTwoCompletedGroupAlgebra (DSq h : Type)) :
    FiniteGeneratorWord (Fin (sqRank h)) 2 →
      ModTwoCompletedGroupAlgebra (DSq h : Type) :=
  fun w => modTwoCompletedAugmentationCoordinate (DSq h : Type)
    (sqMagnusOneKernel h) (c w) • 1

@[simp] theorem sqCompletedQuadraticScalarizedCoefficient_augmentation
    (h : ℕ)
    (c : FiniteGeneratorWord (Fin (sqRank h)) 2 →
      ModTwoCompletedGroupAlgebra (DSq h : Type))
    (w : FiniteGeneratorWord (Fin (sqRank h)) 2) :
    modTwoCompletedAugmentationCoordinate (DSq h : Type)
        (sqMagnusOneKernel h)
        (sqCompletedQuadraticScalarizedCoefficient h c w) =
      modTwoCompletedAugmentationCoordinate (DSq h : Type)
        (sqMagnusOneKernel h) (c w) := by
  simp [sqCompletedQuadraticScalarizedCoefficient]

/-- Removing the augmentation scalar from a completed coefficient leaves an augmentation-ideal
element. -/
theorem sqCompletedCoefficient_sub_augmentationScalar_mem (h : ℕ)
    (x : ModTwoCompletedGroupAlgebra (DSq h : Type)) :
    x - modTwoCompletedAugmentationCoordinate (DSq h : Type)
        (sqMagnusOneKernel h) x • 1 ∈
      modTwoCompletedAugmentationIdeal (DSq h : Type) := by
  apply sqCompletedDegreeZero_reverse h
  simp

/-- In a quadratic monomial sum, replacing every completed coefficient by its augmentation
scalar changes the result only in `J³`. -/
theorem sqCompletedQuadraticMonomial_sub_scalarized_mem_cube (h : ℕ)
    (c : FiniteGeneratorWord (Fin (sqRank h)) 2 →
      ModTwoCompletedGroupAlgebra (DSq h : Type)) :
    finiteGeneratorMonomialMap (sqCompletedGeneratorDifference h) 2 c -
        finiteGeneratorMonomialMap (sqCompletedGeneratorDifference h) 2
          (sqCompletedQuadraticScalarizedCoefficient h c) ∈
  ...
  apply Ideal.sum_mem
  intro w hw
  change (c w - sqCompletedQuadraticScalarizedCoefficient h c w) *
      finiteGeneratorWordProduct (sqCompletedGeneratorDifference h) 2 w ∈ J ^ 3
  rw [show 3 = 1 + 2 by omega, Submodule.pow_add J (by omega : 2 ≠ 0),
    Submodule.pow_one]
  exact Ideal.mul_mem_mul
    (sqCompletedCoefficient_sub_augmentationScalar_mem h (c w))
    (finiteGeneratorWordProduct_mem_idealPower J
      (sqCompletedGeneratorDifference h)
      (sqCompletedGeneratorDifference_mem h) 2 w)

/-- The scalar quadratic word combination in the abstract one-relation PBW algebra. -/
def sqQuadraticScalarWordCombination (h : ℕ)
    (a : FiniteGeneratorWord (Fin (sqRank h)) 2 → ZMod 2) :
    SqQuadraticAlgebra h :=
  ∑ w, a w • quadraticWordEval (SqQuadraticAlgebra h)
    (sqQuadraticQuotientLetter h) (finiteGeneratorWordList 2 w)

/-- The corresponding scalar-coefficient quadratic monomial in the completed group algebra. -/
def sqCompletedQuadraticScalarMonomial (h : ℕ)
    (a : FiniteGeneratorWord (Fin (sqRank h)) 2 → ZMod 2) :
    ModTwoCompletedGroupAlgebra (DSq h : Type) :=
  finiteGeneratorMonomialMap (sqCompletedGeneratorDifference h) 2
    (fun w => a w • 1)

/-- The exact missing degree-two comparison.

The right side is computed in the abstract quadratic algebra defined by the single improved
relation `YY + SX + XS + Σ(UV+VU)`.  Thus this proposition says precisely that the scalar
quadratic relations among the actual completed generator differences modulo `J³` are exactly
the relations generated by that one PBW relation.  The proved quadratic initial-relator
calculation identifies which relation must occur; this statement is the remaining assertion
that no additional completed quadratic relations occur. -/
def SqCompletedQuadraticRelationIdealExact (h : ℕ) : Prop :=
  ∀ a : FiniteGeneratorWord (Fin (sqRank h)) 2 → ZMod 2,
    sqQuadraticScalarWordCombination h a = 0 ↔
      sqCompletedQuadraticScalarMonomial h a ∈
        modTwoCompletedAugmentationIdeal (DSq h : Type) ^ 3

/-- PBW normalization of a scalarized quadratic coefficient family evaluates to the abstract
quadratic word combination of its scalar coefficients. -/
theorem sqQuadraticHomogeneousEval_scalarizedPBWNormal (h : ℕ)
    (a : FiniteGeneratorWord (Fin (sqRank h)) 2 → ZMod 2) :
    sqQuadraticHomogeneousEval h 2
        (sqCompletedMonomialPBWNormalMap h 2 (fun w => a w • 1)) =
      sqQuadraticScalarWordCombination h a := by
  rw [sqQuadraticHomogeneousEval_monomialPBWNormal]
  apply Finset.sum_congr rfl
  intro w hw
  simp

/-- The degree-two completed kernel identity implies the exact scalar relation-ideal
comparison. -/
theorem sqCompletedQuadraticRelationIdealExact_of_kernelIdentity
    (h : ℕ) (H : SqCompletedMonomialPBWKernelIdentity h 2) :
    SqCompletedQuadraticRelationIdealExact h := by
  intro a
  let c : FiniteGeneratorWord (Fin (sqRank h)) 2 →
      ModTwoCompletedGroupAlgebra (DSq h : Type) := fun w => a w • 1
  have heval : sqCompletedMonomialPBWNormalMap h 2 c = 0 ↔
      sqQuadraticScalarWordCombination h a = 0 := by
    constructor
    · intro hz
      rw [← sqQuadraticHomogeneousEval_scalarizedPBWNormal h a]
      simp [c, hz]
    · intro hz
      apply sqQuadraticHomogeneousEval_injective h 2
      rw [map_zero, sqQuadraticHomogeneousEval_scalarizedPBWNormal h a, hz]
  rw [← heval, H c]
  rfl

/-- The scalar relation-ideal comparison implies the full degree-two kernel identity.  The
only extra input needed to pass from arbitrary completed coefficients to scalars is the
unconditional degree-zero exactness already proved above. -/
theorem sqCompletedMonomialPBWKernelIdentity_two_of_relationIdealExact
    (h : ℕ) (H : SqCompletedQuadraticRelationIdealExact h) :
    SqCompletedMonomialPBWKernelIdentity h 2 := by
  intro c
  let a : FiniteGeneratorWord (Fin (sqRank h)) 2 → ZMod 2 := fun w =>
    modTwoCompletedAugmentationCoordinate (DSq h : Type)
      (sqMagnusOneKernel h) (c w)
  let c₀ := sqCompletedQuadraticScalarizedCoefficient h c
  have heval : sqCompletedMonomialPBWNormalMap h 2 c = 0 ↔
      sqQuadraticScalarWordCombination h a = 0 := by
    constructor
    · intro hz
      have := congrArg (sqQuadraticHomogeneousEval h 2) hz
      rw [map_zero, sqQuadraticHomogeneousEval_monomialPBWNormal] at this
      exact this
    · intro hz
      apply sqQuadraticHomogeneousEval_injective h 2
      rw [map_zero, sqQuadraticHomogeneousEval_monomialPBWNormal]
      exact hz
  have hscalar : sqCompletedQuadraticScalarMonomial h a =
      finiteGeneratorMonomialMap (sqCompletedGeneratorDifference h) 2 c₀ := by
    rfl
  have hdiff := sqCompletedQuadraticMonomial_sub_scalarized_mem_cube h c
  rw [heval, H a, hscalar]
  change finiteGeneratorMonomialMap (sqCompletedGeneratorDifference h) 2 c₀ ∈
      modTwoCompletedAugmentationIdeal (DSq h : Type) ^ 3 ↔
    finiteGeneratorMonomialMap (sqCompletedGeneratorDifference h) 2 c ∈
      modTwoCompletedAugmentationIdeal (DSq h : Type) ^ 3
  constructor
  · intro hc₀
    have := (modTwoCompletedAugmentationIdeal (DSq h : Type) ^ 3).add_mem hdiff hc₀
    simpa only [c₀, sub_add_cancel] using this
  · intro hc
    have := (modTwoCompletedAugmentationIdeal (DSq h : Type) ^ 3).sub_mem hc hdiff
    simpa only [c₀, sub_sub_cancel] using this

/-- In degree two, the completed Magnus--Labute kernel identity is *equivalent* to the single
finite/completed relation-ideal comparison above. -/
theorem sqCompletedMonomialPBWKernelIdentity_two_iff (h : ℕ) :
    SqCompletedMonomialPBWKernelIdentity h 2 ↔
      SqCompletedQuadraticRelationIdealExact h := by
  exact ⟨sqCompletedQuadraticRelationIdealExact_of_kernelIdentity h,
