...
⚠ **De-duplicated (ticket CB-DD).**  The topological-generation input used to be a local
`Count.topGen_gammaR`; it is now `GQ2.Dyadic.topGen_gammaR` (`GQ2/Dyadic/GammaRHom.lean` §2),
which this file imports.  The statement is verbatim what stood here. -/

section Tfg

variable (n q : ℕ) (R : PWord (Generator n))

/-- **`WordCertificate.tfg`, generically.**  `Generator n` is a `Fintype`, so the topological
generating set of `GQ2.Dyadic.topGen_gammaR` is finite on the nose. -/
