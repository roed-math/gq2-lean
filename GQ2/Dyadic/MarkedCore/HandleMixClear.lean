...
/-!
# Handle mixing, step 4: the ν-clearing, and the restated obligation as a theorem

**Ticket HM4** of the dyadic campaign (lane MC) — the payoff ticket of the `HandleMixLift`
discharge, implementing the spike memo `docs/dyadic/handlemixlift-spike.md` §5.3 (the ν-clearing
recipe), V5 (the restated obligation) and §7's fourth row.  The three upstream files landed the
ingredients: HM1 (`HandleMix.lean`) the handle-block splitting, the commutator expansions and the
**exact transvections with 2-adic exponents**; HM2 (`HandleMixEquiv.lean`) the mixing substitution
as honest continuous automorphisms `dmMixEquiv`, `dnMixEquiv` of the presented cores; HM3
(`HandleMixFrame.lean`) the frame calculus — the Eichler elements, `N² = 0`, the `θ_w`-conjugation
that reaches **every** 2-adic coefficient, the 2×2 local-ring `SL₂ = E₂` argument, and the ν-frame
dictionary.  This file spends them:

1. **the ν-clearing theorem** (§2, memo §5.3): for every ν-frame vector whose `c̄`-slot is a 2-adic
   unit there is a move in `A(P,h)` annihilating the whole handle plane;
2. **the restated obligation as a THEOREM** (§5, memo V5): `ν_P ∈ ν'·A(P,h)` on the handle plane,
   for **both** rank-four cores at general `(α, h)`;
3. **MC5's `hLift` split** (§6, memo §7's Consequences): three strata, of which the handle one is
   no longer a hypothesis.

Repo conventions as upstream: `x ^ g = g⁻¹xg` (`GQ2.conjP`), `[x,y] = x⁻¹y⁻¹xy` (`GQ2.commP`), and
HM1's naming rule for the `τ` family (**a Lean name's suffix is the letter that MOVES**, so
`tauUMark j k` moves `u_j` by a power of `v_j` and is the memo's `τ_{v_j}(k)`).

## `A(P,h)`, twice

Memo §5.3's `A(P,h)` is "the subgroup generated by the realized elements".  It appears here in two
guises, and the file's spine is the bridge between them:

* **at the frame level** — `Submonoid.closure (frameClearGens h)` inside
  `Function.End (Fin (coreRank h) → ℤ_[2])`, i.e. HM3's `mem_closure` vocabulary, whose four
  generating families are `frameTauU j k`, `frameTauV j k`, `frameTauD k` and `frameMixAdd j`;
* **on the cores** — `Submonoid.closure (dmClearAuts α h)` / `(dnClearAuts α h)` inside
  `Function.End (D_P)`, generated by the *same* four families read as automorphisms:
  `dmTauUEquiv`, `dmTauVEquiv`, `dmTauDEquiv` (§3) and HM2's `dmMixEquiv`.

`exists_dmRealizes`/`exists_dnRealizes` (§4) say the second surjects onto the first: **every**
frame move in `A(P,h)` is the ν-frame action of a continuous automorphism of the core which is
itself in `A(P,h)`.  Composites are the only subtle point: substitutions act on markings, so
`Ψ₁.trans Ψ₂` realizes `F₁ * F₂` — the assignment is an *anti*-homomorphism, exactly as HM3's
docstring records for `frameMat`.

## Contents, and the memo section each part serves

* **§1 `A(P,h)` at the frame level** (memo §5.3's generating set).  `frameEnd` (the one-line
  barrier that keeps `Function.End`'s composition monoid from collapsing to `Pi.mulOneClass`),
  `frameClearGens` and the membership ladder: `frameMatEnd_mem_clearGens` transports HM3's
  `SL₂ = E₂` theorem, giving `frameTheta`, `frameS`, `frameSinv`; then
  `frameEichlerU_one_mem_clearGens` (HM3's normalisation `E_j = τ_c(−1) ∘ τ_{v_j}(1) ∘ Φ_j` read as
  a three-factor product) and finally `frameEichlerU_mem_clearGens`,
  `frameEichlerV_mem_clearGens` — **every 2-adic Eichler coefficient is in `A(P,h)`**, which is
  memo §5.2's conclusion in submonoid clothing.

* **§2 The ν-clearing** (memo §5.3).  `exists_frameClear_one` is the memo's two steps at one
  handle (`E_j^x` with `x = −ν'(ū_j)/ν'(c̄)`, then `(E'_j)^y` with `y = −ν'(v̄_j)/ν'(c̄)`, which
  does not undo step 1 because `E'_j` fixes `ū_j`); `exists_frameClear_lt` is the induction over the
  handles, whose invariant is that each step is invisible to the other handles and to `c̄`; and
  **`exists_frameClear`** is memo §5.3's conclusion at `n = h`.

* **§3 The `τ` families as automorphisms** (memo §5.1; HM3's docstring's "HM4's first step").
  `tauUMark`, `tauVMark`, `tauDMark` with their one-parameter-group laws
  (`tau*Mark_tau*Mark`, `tau*Mark_zero`) and naturality (`map_tau*Mark`, via `map_zpowZtwo`); the
  two relator rows HM1 left at the *core-word* level are completed here
  (`mRelWord_update_three`, `mRelWord_tauDMark`, and the `N`-mirrors — HM1 has
  `mRelWord_tau_handleU/V` but no `mRelWord_tau_three`); then HM2 §3's `thetaEquiv` assembly,
  isolated as `dmParamEquiv`/`dnParamEquiv` — *a one-parameter family whose inverse at `k` is its
  member at `−k`* — used six times to produce `dmTauU/V/DEquiv` and `dnTauU/V/DEquiv`.

* **§4 The realization bridge.**  `autEnd`, `dmClearAuts`, `dnClearAuts`, the relation
  `DmRealizes`/`DnRealizes`, its four generator rows per core (each one `rw [funext …]` plus the
  matching HM3 §6 theorem — the one-`rw` end-to-end probe HM3 recorded, now committed), and the
  closure induction `exists_dmRealizes`/`exists_dnRealizes`.

* **§5 The restated obligation** (memo V5).  `exists_dmClear_nu`, `exists_dnClear_nu` and the two
  corollaries `exists_dmClear_nu_eq_nuM`, `exists_dnClear_nu_eq_nuN` that phrase the conclusion as
  the memo does: `ν'∘Ψ` **is** the standard marking `ν_P` on every handle letter (`ν_M`, `ν_N` are
  `0` there, `nuM_handleU`/`nuN_handleV`).

* **§6 MC5's `hLift`, split three ways** (memo §7).  `DmRealizesAll`/`DnRealizesAll`, the two
  hypothesis `def`s per core (`MNielsenScalingHypothesis` = MC1 §5.1–§5.2's S1 ∪ S2,
  `MCoreMixHypothesis` = MC1 §5.3's S3 = MC1 §8 Decision 2's `MMixHypothesis`), the bundles
  `MLiftSplit`/`NLiftSplit`, the theorems `mLiftSplit_handle`/`nLiftSplit_handle` discharging the
  handle field, and `mLiftSplit_iff`/`nLiftSplit_iff`: **MC5 supplies two of the three fields, not
  three.**

## What HM5 and MC5 consume

* **MC5** (`mc-design.md` §6.3) consumes §6: `MLiftSplit α h S12 S3` in place of the single
  `hLift` antecedent, with `mLiftSplit_iff` showing that only `S12` and `S3` have to be provided,
  and `MLiftSplit.nu_clear_handles` delivering the handle half of the ν-correction with its
  correction certified to lie in `A(P,h)` (so it composes with the core solve, which is what
  `correction`/`correction_nu` of `MarkedCoreCertificate` need).  `MarkedCoreCertificate` itself is
  untouched — the certificate's `correction` field is the *output*.
* **HM5** (memo §7's fifth row, "the `M`/`N` instantiations") is **thinner than the memo
  budgeted**: HM2's family swap made one definition serve both rank-four cores, so HM2, HM3 and
  this file are already `M`- *and* `N`-uniform at general `(α, h)`, and §5 states the obligation
  twice.  What is left for HM5 is polish: the unit rows (§5's `hc` hypothesis — memo §6.4's
  residue 2 on the `M` side, memo §5.3's `ν'(σ̄) ∈ ℤ₂ˣ` on the `N` side; both become one-liners
  once F4/MC5 have the `M` ν-row), the `e`-row restatements once MC3/MC4's `MDecomposition` exists
  (take `f = (coordinate of e) ∘ abMk`, HM3's recorded delta), and the L-family instantiation,
  whose core lives in `GQ2/Dyadic/SqCore/`.

## What is now a theorem, and what is not

**The HANDLE stratum of `HandleMixLift` is a theorem for the `M` and `N` families.**  Precisely:
memo §1's `MHandleMixHypothesis`/`NHandleMixHypothesis` binder is *not needed* for the statement
MC5 consumes — `exists_dmClear_nu`/`exists_dnClear_nu` prove it outright, at general `α` and
general handle count `h`, with **no new axiom, no B8** (`PeripheralCyclotomicAction`) and no appeal
to compactness of `Aut(D_P)`.  Memo V4 and V5 are thereby measured rather than asserted, and
MC1 §4.2's "MC5's handle stability is **not** a formality" together with MC1 §7.5's risk **R3** are
resolved in the affirmative direction: not a formality, but not a binder either.

Three things are explicitly **out of scope here** and remain open:

* **the `L_sq` frame case.**  Memo §6.3 (residue 1) leaves the `L_sq` prefix undecided, and SQ1
  sharpened this into a correction: `docs/dyadic/sq-design.md` §7 **R1** shows S2.4 §1.1's
  "`χ(σ) = 1` for type L" is **false** for `L_sq` (`χ_sq(σ) = S`, of infinite order), so the
  χ-trivial subspace of the `L_sq` frame is a rank-1 free `ℤ₂`-module transverse to `σ̄` rather than
  `⟨σ̄⟩ ⊕ P_han`, and memo §6.4's identification of the reachable stabilizer block has to be redone
  in that frame.  Nothing in this file speaks to `L_sq`: its cores are `DM`/`DN`, and the `L_sq`
  redo is MC5's to absorb (sq-design.md §7 R1's own verdict).
* **the S3 core-mixing stratum.**  MC1 §5.3's families M4–M7 and N5–N6 are core↔core mixing at
  rank four, are *not* reachable from the constructions here, and stay hypotheses
  (`MCoreMixHypothesis`, `NCoreMixHypothesis` — MC1 §8 Decision 2, G-Lab territory).  Memo §6.5's
  bonus (HM6, owner-held) would turn the `N`-instance into a theorem; that spike is not run here.
* **the S1 ∪ S2 core stratum.**  MC1 §5.1's Nielsen lifts are axiom-free and MC1 §5.2's unit
  scalings go through the *existing* axiom B8; both are MC3/MC4's to construct, and §6 records
  them as `MNielsenScalingHypothesis`/`NNielsenScalingHypothesis` in the meantime.

## Axiom hygiene

Every declaration prints within **std-3** (`propext`, `Classical.choice`, `Quot.sound`), and the
`autEnd` group within `{Quot.sound}` alone.  **No census axiom is reachable, in particular no B8** —
the whole file runs on HM1's `zpowZtwo` transvections, HM2's universal-property assembly, HM3's
frame calculus and `Submonoid.closure` bookkeeping.  The census stays at 11.

## Deviations from the memo (recorded)

* **Slots.**  As in HM2/HM3, the realized family is memo §4.4's `Φ^M_j`, so the memo's `σ̄` is the
  slot `2` (`c = C₀` for `M`, `σ` for `N`) and the absorbing core slot is `3` (`d = D` for `M`,
  `x₂` for `N`).  The unit hypothesis is therefore `IsUnit (toAdd (ν' (dmC α h)))` resp.
  `IsUnit (toAdd (ν' (dnSigma α h)))`; for `N` that *is* memo §5.3's `ν'(σ̄) ∈ ℤ₂ˣ` verbatim, and
  for `M` it is memo §6.4's residue 2.
* **`A(P,h)` is a `Submonoid`, not a `Subgroup`.**  Memo §5.3 says "subgroup"; the clearing needs
  only products, and each generator's inverse is another generator (`τ`'s at `−k`, `E_j^x` at
  `−x`), so the submonoid already contains all of them and the weaker structure is the honest one.
  No statement below would change if the closure were taken in a group.
* **The obligation is the handle part, not the whole ν-correction.**  Memo V5 writes
  `ν_P ∈ ν'·A(P,h)`; what §4–§5 of the memo prove — and what §5 here states — is that equality
  *on the handle plane*, together with `ν'(c̄)` being preserved.  The core coordinates are finished
  by memo §5.3's "block (a)", i.e. the rank-three/rank-four solve, which is not this ticket's.
  Stating more would overclaim.
* **Schematic stratum sets in §6.**  MC3/MC4's `MDecomposition`/`IsMStabilizer` do not exist, so
  the two hypothesis fields quantify over a *given* set of frame moves rather than over a
  concretely enumerated family.  This is HM3's characters-not-frames delta applied to the split:
  the shape MC5 consumes is fixed now, and MC3/MC4 pin the sets later without reshaping it.
-/
...
/-- Monoid powers in `Function.End` **are** iterates — the identification that turns HM3's
`(frameEichlerU j 1)^[n]` into a factor of a `Submonoid` product (`frameEichlerU_mem_clearGens`). -/
theorem frameEnd_pow_apply (F : (Fin (coreRank h) → ℤ_[2]) → Fin (coreRank h) → ℤ_[2]) (n : ℕ)
    (m : Fin (coreRank h) → ℤ_[2]) : (frameEnd F ^ n) m = F^[n] m := rfl

/-- The unit of the frame endomorphism monoid is the identity substitution. -/
...
@[simp] theorem tauUMark_handleV (i : Fin h) :
    tauUMark hP j k m (handleIdxV i) = m (handleIdxV i) :=
...
@[simp] theorem tauVMark_handleU (i : Fin h) :
    tauVMark hP j k m (handleIdxU i) = m (handleIdxU i) :=
...
/-! ## §6 MC5's `hLift`, split three ways  (memo §7's Consequences)

`mc-design.md` §6.3 gives `marked_matching_reduction` a single antecedent
`hLift : ∀ φ, IsMStabilizer α n B φ → ∃ Ψ, …`.  Memo §7 splits it into MC1 §5.3's three strata,
and this file turns one of the three into a theorem.  Since MC3/MC4's frame vocabulary
(`MDecomposition`'s isomorphism `e`, `IsMStabilizer`) is not landed, the split is stated
**schematically over each stratum's set of frame moves** and in the character vocabulary HM3
adopted — the concrete stratum sets are MC3/MC4's to pin down, and pinning them down does not
change the shape below.

Two of the three fields stay hypotheses and are **`def`s, never axioms** (the `BLabHypothesis`
convention, `GQ2/Roe/MarkedPro2.lean:133–141`); the third is discharged here. -/

section LiftSplit

variable (α h : ℕ)

/-- **A stratum of MC5's `hLift`, for `D_M`**: every frame move in `S` is realized by a continuous
automorphism of the core lying in `A(P,h)`.  All three fields of `MLiftSplit` have this shape;
`DmRealizesAll α h (Submonoid.closure (frameClearGens h))` is the one that is a theorem. -/
def DmRealizesAll (α h : ℕ) (S : Set (Function.End (Fin (coreRank h) → ℤ_[2]))) : Prop :=
  ∀ F ∈ S, ∃ Ψ : ContinuousMulEquiv (DM α h : Type) (DM α h : Type), DmRealizes α h Ψ F

/-- The `N`-mirror of `DmRealizesAll`. -/
def DnRealizesAll (α h : ℕ) (S : Set (Function.End (Fin (coreRank h) → ℤ_[2]))) : Prop :=
  ∀ F ∈ S, ∃ Ψ : ContinuousMulEquiv (DN α h : Type) (DN α h : Type), DnRealizes α h Ψ F

/-- **The S1 ∪ S2 stratum for `D_M`** (MC1 §5.1–§5.2), hypothesis form — **never an axiom**.
`S12` is MC3/MC4's set of elementary Nielsen moves and unit scalings on the rank-four core: `S1`
is axiom-free (MC1 §5.1's matched-conjugator recipe) and `S2` runs through the *existing* axiom
B8 (MC1 §5.2).  Nothing in this file consumes it, and nothing in this file discharges it. -/
def MNielsenScalingHypothesis (α h : ℕ) (S12 : Set (Function.End (Fin (coreRank h) → ℤ_[2]))) :
    Prop := DmRealizesAll α h S12

/-- **The S3 core-mixing stratum for `D_M`** (MC1 §5.3's families M4–M7), hypothesis form —
**never an axiom**.  This is MC1 §8 Decision 2's `MMixHypothesis`, the core↔core mixing at rank
four, whose home is MC3/MC4 (`M.lean`/`N.lean`) once `MDecomposition` exists; it is **not** this
ticket's to discharge and it is **not** implied by anything here.  Memo §6.5 reports a spike
(HM6, owner-held) that would turn the `N`-instance into a theorem. -/
def MCoreMixHypothesis (α h : ℕ) (S3 : Set (Function.End (Fin (coreRank h) → ℤ_[2]))) : Prop :=
  DmRealizesAll α h S3

/-- The `N`-mirror of `MNielsenScalingHypothesis` (MC1 §5.1–§5.2, families N1–N4). -/
def NNielsenScalingHypothesis (α h : ℕ) (S12 : Set (Function.End (Fin (coreRank h) → ℤ_[2]))) :
    Prop := DnRealizesAll α h S12

/-- The `N`-mirror of `MCoreMixHypothesis` (MC1 §5.3's families N5–N6, the `p,q`-directions —
MC1 §8 Decision 2's `NMixHypothesis`). -/
def NCoreMixHypothesis (α h : ℕ) (S3 : Set (Function.End (Fin (coreRank h) → ℤ_[2]))) : Prop :=
  DnRealizesAll α h S3

/-- **MC5's `hLift` for the `M` family, split three ways** (memo §7): the handle↔core mixing
block, MC1 §5.1–§5.2's Nielsen moves and unit scalings, and MC1 §5.3's core↔core mixing.  The
first field is **not** a hypothesis — `mLiftSplit_handle` proves it — so MC5 supplies only the
other two, which is exactly memo §7's "MC5 stops being conditional on the handle stratum". -/
structure MLiftSplit (α h : ℕ) (S12 S3 : Set (Function.End (Fin (coreRank h) → ℤ_[2]))) :
    Prop where
  /-- The handle↔core mixing stratum — a **theorem** (`mLiftSplit_handle`), this file's payoff. -/
  handle : DmRealizesAll α h (Submonoid.closure (frameClearGens h) : Set _)
  /-- MC1 §5.1–§5.2's S1 ∪ S2 stratum — MC3/MC4's, S1 axiom-free and S2 through B8. -/
  nielsenScaling : MNielsenScalingHypothesis α h S12
  /-- MC1 §5.3's S3 core-mixing stratum — `MMixHypothesis`, MC1 §8 Decision 2. -/
  coreMix : MCoreMixHypothesis α h S3

/-- The `N`-mirror of `MLiftSplit`. -/
structure NLiftSplit (α h : ℕ) (S12 S3 : Set (Function.End (Fin (coreRank h) → ℤ_[2]))) :
    Prop where
  /-- The handle↔core mixing stratum — a **theorem** (`nLiftSplit_handle`). -/
  handle : DnRealizesAll α h (Submonoid.closure (frameClearGens h) : Set _)
  /-- MC1 §5.1–§5.2's S1 ∪ S2 stratum. -/
  nielsenScaling : NNielsenScalingHypothesis α h S12
  /-- MC1 §5.3's S3 core-mixing stratum — `NMixHypothesis`. -/
  coreMix : NCoreMixHypothesis α h S3

/-- **The handle stratum of MC5's `hLift` is a THEOREM for the `M` family** (memo V5, V6): every
element of `A(P,h)` — the whole monoid generated by HM2's mixing automorphisms and §3's exact
transvections — is realized on `D_M`, with no new axiom, no B8, and no appeal to compactness of
`Aut(D_P)`. -/
theorem mLiftSplit_handle : DmRealizesAll α h (Submonoid.closure (frameClearGens h) : Set _) :=
  fun _ hF => exists_dmRealizes α h hF

/-- **The handle stratum is a THEOREM for the `N` family**. -/
theorem nLiftSplit_handle : DnRealizesAll α h (Submonoid.closure (frameClearGens h) : Set _) :=
  fun _ hF => exists_dnRealizes α h hF

/-- **MC5 needs only two of the three fields**: `MLiftSplit` is *equivalent* to its two hypothesis
fields, because the handle field is unconditional.  This is the precise form of memo §7's
consequence for MC5, and the reason the `HandleMixLiftHypothesis` binder of memo §1 can be
retired for the `M`/`N` families. -/
theorem mLiftSplit_iff (S12 S3 : Set (Function.End (Fin (coreRank h) → ℤ_[2]))) :
    MLiftSplit α h S12 S3
      ↔ MNielsenScalingHypothesis α h S12 ∧ MCoreMixHypothesis α h S3 :=
  ⟨fun hs => ⟨hs.nielsenScaling, hs.coreMix⟩,
    fun hs => ⟨mLiftSplit_handle α h, hs.1, hs.2⟩⟩

/-- The `N`-mirror of `mLiftSplit_iff`. -/
theorem nLiftSplit_iff (S12 S3 : Set (Function.End (Fin (coreRank h) → ℤ_[2]))) :
    NLiftSplit α h S12 S3
      ↔ NNielsenScalingHypothesis α h S12 ∧ NCoreMixHypothesis α h S3 :=
  ⟨fun hs => ⟨hs.nielsenScaling, hs.coreMix⟩,
    fun hs => ⟨nLiftSplit_handle α h, hs.1, hs.2⟩⟩

/-- **What MC5 gets from the split, on the `M` side**: the ν-correction on the handle plane, with
its correction inside `A(P,h)`, from the `handle` field alone.  The remaining work — memo §5.3's
"block (a) finishes on `P_core`" — is the rank-four core solve, and it consumes the other two
fields, not this one. -/
theorem MLiftSplit.nu_clear_handles {S12 S3 : Set (Function.End (Fin (coreRank h) → ℤ_[2]))}
    ...
    ∃ Ψ : ContinuousMulEquiv (DM α h : Type) (DM α h : Type),
      autEnd Ψ ∈ Submonoid.closure (dmClearAuts α h)
        ∧ (∀ j : Fin h, nu' (Ψ (dmGen α h (handleIdxU j))) = 1)
        ∧ (∀ j : Fin h, nu' (Ψ (dmGen α h (handleIdxV j))) = 1)
        ∧ nu' (Ψ (dmC α h)) = nu' (dmC α h) :=
  exists_dmClear_nu α h nu' hc

/-- The `N`-mirror of `MLiftSplit.nu_clear_handles`. -/
theorem NLiftSplit.nu_clear_handles {S12 S3 : Set (Function.End (Fin (coreRank h) → ℤ_[2]))}
    ...
    ∃ Ψ : ContinuousMulEquiv (DN α h : Type) (DN α h : Type),
      autEnd Ψ ∈ Submonoid.closure (dnClearAuts α h)
        ∧ (∀ j : Fin h, nu' (Ψ (dnGen α h (handleIdxU j))) = 1)
        ∧ (∀ j : Fin h, nu' (Ψ (dnGen α h (handleIdxV j))) = 1)
        ∧ nu' (Ψ (dnSigma α h)) = nu' (dnSigma α h) :=
  exists_dnClear_nu α h nu' hc
