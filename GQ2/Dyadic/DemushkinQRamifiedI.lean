...
/-! ## Splitting the remaining premise into completion and reciprocity halves -/

/-- The finite-order elements of the abstract pro-2 completion of `K×`. -/
abbrev ProTwoCompletionTorsion (K : IntermediateField ℚ_[2] ℚbar2) :=
  {x : proPCompletion 2 ((↥K)ˣ) // IsOfFinOrder x}

/-- The canonical map from 2-primary field roots into the torsion of the pro-2 completion of
`K×`. -/
def twoPowerRootToProTwoCompletionTorsion :
    TwoPowerRoots K → ProTwoCompletionTorsion K := fun x ↦
  ⟨proPCompletionMk 2 ((↥K)ˣ) (twoPowerRootUnit x), by
    rw [isOfFinOrder_iff_pow_eq_one]
    obtain ⟨n, hn⟩ := x.2
    refine ⟨2 ^ n, by positivity, ?_⟩
...
/-- The source-side completion theorem still needed after deep-unit torsion-freeness: all
torsion in `(K×)^(2)` comes from an actual 2-primary root of unity in `K`. -/
def ProTwoCompletionTorsionGeneratedByFieldRoots
    (K : IntermediateField ℚ_[2] ℚbar2) : Prop :=
  Function.Surjective (twoPowerRootToProTwoCompletionTorsion (K := K))

/-- The two classical completion statements imply the exact target-side torsion premise:

* completed reciprocity is injective (it is already proved surjective); and
* torsion in the source completion `(K×)^(2)` is generated by field roots.

Injectivity is essential here: a quotient of a torsion-free pro-2 group can acquire torsion. -/
theorem maxProTwoAbTorsionGeneratedByFieldRoots_of_completion
    (B : MarkedRecip R K)
    (hrecip : Function.Injective (proTwoReciprocityToTopAb B))
    (hsource : ProTwoCompletionTorsionGeneratedByFieldRoots K) :
    MaxProTwoAbTorsionGeneratedByFieldRoots B := by
  intro y
  obtain ⟨x, hx⟩ := proTwoReciprocityToTopAb_surjective B y.1
  have hxfin : IsOfFinOrder x := by
    have hyfin : IsOfFinOrder y.1 := y.2
    rw [isOfFinOrder_iff_pow_eq_one] at hyfin ⊢
    obtain ⟨n, hnpos, hyn⟩ := hyfin
    refine ⟨n, hnpos, hrecip ?_⟩
    rw [map_pow, map_one, hx, hyn]
  obtain ⟨z, hz⟩ := hsource ⟨x, hxfin⟩
  refine ⟨z, Subtype.ext ?_⟩
  change proTwoReciprocityToTopAb B
      (proPCompletionMk 2 ((↥K)ˣ) (twoPowerRootUnit z)) = y.1
  have hzval : proPCompletionMk 2 ((↥K)ˣ) (twoPowerRootUnit z) = x :=
...
/-- Literature-shaped sufficient conditions for the odd ramified-`i` field theorem.  The
deep-unit completion is already torsion-free in `DepthPower.lean`; the source premise here is
the remaining unit/uniformizer decomposition transfer, while `hrecip` is the missing completed
local-reciprocity kernel identification. -/
theorem demushkinQ_maxProTwoGalK_eq_two_of_completion
    (B : MarkedRecip R K) (hodd : Odd (Module.finrank ℚ_[2] K))
    (RI : RamifiedIData K)
    (hrecip : Function.Injective (proTwoReciprocityToTopAb B))
    (hsource : ProTwoCompletionTorsionGeneratedByFieldRoots K) :
    demushkinQ (maxProPQuotient 2 (GalK K)) = 2 :=
  demushkinQ_maxProTwoGalK_eq_two_of_odd_ramifiedI B hodd RI
    (maxProTwoAbTorsionGeneratedByFieldRoots_of_completion B hrecip hsource)
