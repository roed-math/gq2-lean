...
| frozen `stageR136` | `blockStageR136K` + the uniform variation witness | legacy hom-lift separation and `R`-cocycle count |
| corrected `stageR136N` | `blockStageR136NK` + resolver/Stokes count | none beyond per-simple Stokes duality |
...
/-- The improved compact-N presentation has deficiency `2h+2`. -/
theorem degree (h : ℕ) :
...
/-- Corrected exact lifting for the improved compact-N presentation.  The resolver and Stokes
data now prove both former `R`-stage residues, and the R-cocycle coefficient is the
degree-indexed `zRN`. -/
      ...
      (fun g hg => by
        ...
        have hd : StokesDuality
          ...
          simpa only [hrho_apply] using
            (stokesDuality hsimp hα hqe rho he hres2 (Additive ↑Blk.frattiniK)
      ...
      (fun f₀ => by
        ...
        have hd : StokesDuality
          ...
          simpa only [htheta_apply] using
            (stokesDuality hsimp hα hqe theta he hres2 (Additive ↑Blk.frattiniK)
              (RStageLocal.frattiniK_add_self hRK hR₂))
        exact rCocycle_card_standard_zRN hE₂ hRK hR₂ f₀.1.1 f₀.1.2 hpres
          hresR
          (isWildTwo_of_gammaGen theta htheta_surj (fun _ => rfl))
          (degree h) hd hb.2)

/-- Constructor-table regression: compact `N` uses the improved `nCompactW`, not the initial
draft word. -/
theorem gamma_eq_improved (α h q : ℕ) :
    gamma α h q = GammaR (2 + 2 * h) q (nCompactW α h) := rfl
...
/-- The corresponding harmless degree normalization for corrected `RN` semantics. -/
...
/-- A selected compact-N row literally uses the improved `nCompactW` presentation. -/
theorem gamma_eq_fieldSelection
    ...
    GammaR S.semantic.degree q S.semantic.word =
      gamma α (handleCount FP (.N0 α)) q := by
  cases S with
...
/-- The selected compact-N presentation receives corrected exact-lifting semantics directly;
...
theorem exactLiftingRN_of_fieldSelection
  ...
  | mk branch valid compatible level_eq family_eq arithmetic_matches display degree_params
      ...
      have hn : 2 * handleCount FP (.N0 α) + 2 =
          2 + 2 * handleCount FP (.N0 α) := by omega
      exact exactLiftingRN_standard_congr hn (exactLiftingRN hsimp hα hq0 hqe nuP)
