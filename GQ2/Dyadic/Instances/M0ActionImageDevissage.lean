...
/-!
# Action-image devissage for the compact-`M` presentation
...
local instance mCompactActionImageHeisTopology
    {C A : Type} [Group C] [AddCommGroup A] : TopologicalSpace (HeisLift A C) := ⊥

local instance mCompactActionImageHeisDiscrete
    {C A : Type} [Group C] [AddCommGroup A] : DiscreteTopology (HeisLift A C) := ⟨rfl⟩

/-! ## The compact-`M` action image -/

/-- The finite action image of a coefficient of the compact-`M` candidate. -/
noncomputable abbrev ActionImage (α h q : ℕ) (M : Type)
  ...
  RowActionImage.ActionImage (2 + 2 * h) q (mCompactW α h) M

/-- The marked generators of the compact-`M` presentation, read in that image. -/
noncomputable abbrev actionGenerators (α h q : ℕ) (M : Type)
  ...
  actionImageGenerators (2 + 2 * h) q (mCompactW α h) M

/-- The intrinsic compact-`M` relator death agrees with the uniform integer resolver used by the
...
theorem actionImage_mCompact_relator_death_resolved
  ...
  have hresolved : PWord.ResolvedAt (actionGenerators α h q M)
      (fun _ ↦ (omega2Exp N : ℤ)) (fun _ ↦ (omega2Exp N : ℤ)) (mCompactW α h) :=
    PWord.resolvedAt_of_isOmega2Only _ _ _
      (fun c ↦ PWord.zpowHat_omega2_zpow hN (hord c)) _
      (Words.MCompact.isOmega2Only_mCompact α h)
  have hrel := actionImageGenerators_relator_death
    (n := 2 + 2 * h) (q := q) (R := mCompactW α h) (M := M) (1 : Fin 2)
  change PWord.eval (actionGenerators α h q M) (mCompactW α h) = 1 at hrel
  rw [PWord.eval_eq_evalZ _ _ _ _ hresolved] at hrel
  simp [C, N] at hrel
  exact hrel

/-- The intrinsic tame relation on the compact-`M` action image. -/
theorem actionImage_mCompact_tameRelAt
    {α h q : ℕ} {M : Type} [AddCommGroup M] [TopologicalSpace M] [DiscreteTopology M]
    [DistribMulAction ((gamma α h q : Type)) M]
    [ContinuousSMul ((gamma α h q : Type)) M] [Finite M] :
    (actionImageMarking (2 + 2 * h) q (mCompactW α h) M).TameRelAt q :=
  actionImage_tameRelAt

/-! ## The two word-specific simple branches

These are the only compact-`M` inputs the devissage still needs.  Both are statements about a
...
def UnramifiedSimpleStokes (α h q : ℕ) : Prop :=
  ∀ (M : Type) [AddCommGroup M] [TopologicalSpace M] [DiscreteTopology M]
    ...
    (∀ m : M, gammaGen (2 + 2 * h) q (mCompactW α h) .tau • m = m) →
...
theorem actionImage_simpleStokes_of_hsimp {α h q : ℕ} (hsimp : Hsimp α h q)
  ...
  have hrt : PWord.evalZ ⇑(actionImageMarking (2 + 2 * h) q (mCompactW α h) M)
      (fun _ => ((omega2Exp (4 * Monoid.exponent (ActionImage α h q M)) : ℕ) : ℤ))
      (fun _ => ((omega2Exp (4 * Monoid.exponent (ActionImage α h q M)) : ℕ) : ℤ))
      (tameRelW (2 + 2 * h) q) = 1 :=
    evalZ_tameRelW_eq_one_of_tameRelAt _ _ _
      (actionImage_mCompact_tameRelAt (α := α) (h := h) (q := q) (M := M))
  exact hsimp (ActionImage α h q M)
    (actionImageMarking (2 + 2 * h) q (mCompactW α h) M) _ he hrt
    actionImage_mCompact_relator_death_resolved M hM₂
...
theorem actionImage_stokesDuality_simple {α h q : ℕ}
  ...
  rcases actionImage_tau_split_or_ramified_simple
    (n := 2 + 2 * h) (q := q) (R := mCompactW α h) hM₂ hsimple with hτ | hτfpf
...
theorem actionImage_stokesDuality {α h q : ℕ}
  ...
  let w₀ := mCompactFam α h q (omega2Exp (4 * Monoid.exponent C₀))
  have hb := resolvesAt_and_endpoint_mCompactFam_uniformHeis
    (C := C₀) (A := M) hM₂ (α := α) (h := h) (q := q) hq
  letI : TopologicalSpace (WordLift M C₀) := ⊥
  letI : DiscreteTopology (WordLift M C₀) := ⟨rfl⟩
  have hresWord : ResolvesAt (gammaFam (2 + 2 * h) q (mCompactW α h)) w₀
      (WordLift M C₀) := by
  ...
  have hr : ∀ k, FreeGroup.lift c₀ (w₀ k) = 1 := fun k ↦
    lower_rel (A := M) (actionImageHom (2 + 2 * h) q (mCompactW α h) M) (fun _ ↦ rfl)
      (isAdmissibleMarkedPresentation_gammaR (2 + 2 * h) q (mCompactW α h)) hresWord k
  ...
  · intro V _ _ _ hV₂ hsimple
    ...
    letI : DistribMulAction ((gamma α h q : Type)) V :=
      DistribMulAction.compHom V (actionImageHom (2 + 2 * h) q (mCompactW α h) M).toMonoidHom
    letI : ContinuousSMul ((gamma α h q : Type)) V :=
      continuousSMul_of_comp_finite
        (actionImageHom (2 + 2 * h) q (mCompactW α h) M) (fun _ _ ↦ rfl)
    ...
    have hc : ∀ i, piV (actionGenerators α h q V i) = pi₀ (c₀ i) := by
      ...
      ext v
      rfl
    have hresV : ResolvesAt (gammaFam (2 + 2 * h) q (mCompactW α h))
        (mCompactFam α h q
          (omega2Exp (4 * Monoid.exponent (ActionImage α h q V))))
        (HeisLift V (ActionImage α h q V)) :=
      resolvesAt_mCompactFam_uniformHeis hV₂ α h q
    have hres₀ : ResolvesAt (gammaFam (2 + 2 * h) q (mCompactW α h)) w₀
        (HeisLift V C₀) := resolvesAt_mCompactFam_uniformHeis hV₂ α h q
...
consumed by the compact-`M` exact-lifting assembly. -/
theorem uniformPushedHsimp_of_actionImage {α h q : ℕ}
    (hunram : UnramifiedSimpleStokes α h q) (hram : RamifiedSimpleStokes α h q)
    (hq : Even q) : UniformPushedHsimp α h q := by
  ...
  have hc : ∀ i, piA (actionGenerators α h q A i) =
      piC (rho (gammaGen (2 + 2 * h) q (mCompactW α h) i)) := by
    intro i
    apply Multiplicative.toAdd.injective
    ext a
    rfl
  have hresA : ResolvesAt (gammaFam (2 + 2 * h) q (mCompactW α h))
      (mCompactFam α h q
        (omega2Exp (4 * Monoid.exponent (ActionImage α h q A))))
      (HeisLift A (ActionImage α h q A)) :=
    resolvesAt_mCompactFam_uniformHeis hA₂ α h q
  have hresC : ResolvesAt (gammaFam (2 + 2 * h) q (mCompactW α h))
      (mCompactFam α h q (omega2Exp (4 * Monoid.exponent C))) (HeisLift A C) :=
    resolvesAt_mCompactFam_uniformHeis hA₂ α h q
...
/-- Corrected exact lifting for the compact-`M` presentation, reduced to the two word-specific
...
theorem exactLiftingRN_of_actionImage {α h q : ℕ}
  ...
  exactLiftingRN_of_uniformPushed
    (uniformPushedHsimp_of_actionImage hunram hram hqe) hα hq0 hqe nuP
