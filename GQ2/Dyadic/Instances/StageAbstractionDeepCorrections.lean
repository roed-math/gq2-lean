...
* **What happens at `α = 2`?**  The depth is then `s = 1`, which is the committed seam, so
  ...
  `evenRow_rowSupply_imChiN_two` (where it is **not** surjective, since `-1 ∉ imChiN 2`).
  `α ≥ 3` is precisely the regime that needs this file.
...
6. the image identification discharging that reduction on the `M` branch, and the `α = 2`
   `M` verdict;
7. the `N` branch through powers of the generator, giving the uniform depth verdict and the
   `α = 2` `N` verdict.
-/
...
/-! ## §7 The `N` branch, and the uniform depth verdict

The `N` image is procyclic on `nUnit α ≡ -1 mod 2^α`, which has no exact level in the sense
`η - 1 = 2^L·(unit)`, so §5's congruence-form roots are unavailable there: the roots wanted on
the `N` branch are taken up to sign.  Tracking that sign through a valuation bookkeeping on
`2`-adic exponents turns out to be avoidable.  Apply the ported pivot lemma not to the
generator but to the **power** `η^(2^t)`, whose exact level the committed telescoping lemma
computes; procyclic surjectivity at that level then produces the `2^t`-th root together with
its base in one step, with no valuation argument at all.

Two consequences.  The `N` branch lands at the *same* depth `s = α - 1` as `M`, and the
...
open GQ2.Dyadic.MarkedCore (nUnit imChiN nUnit_sq_sub_one mUnit imChiM mUnit_sub_one)

/-- **Roots from the pivot, one level up.**  If `η` has exact level `L ≥ 2` then every
`ξ ≡ 1 mod 2^(L+t)` is a `2^t`-th power of an element of the procyclic group generated by
`η`: the element `η^(2^t)` has exact level `L + t` by telescoping, so the pivot lemma hits
`ξ` with it, and `(η^(2^t))^c = (η^c)^(2^t)`. -/
theorem pivot_pow_surjective (η b : ℤ_[2]ˣ) (L : ℕ) (hL : 2 ≤ L)
    (hη : ((η : ℤ_[2])) - 1 = 2 ^ L * (b : ℤ_[2])) (t : ℕ) {ξ : ℤ_[2]ˣ}
    (hξ : (2 : ℤ_[2]) ^ (L + t) ∣ ((ξ : ℤ_[2])) - 1) :
    ∃ c : ℤ_[2], (zpowZtwo isProP_two_unitsPadicInt η c) ^ 2 ^ t = ξ := by
  obtain ⟨b', hb'⟩ := GQ2.Dyadic.MarkedCore.mExists_unit_pow_two_pow_sub_one η b L hL hη t
  obtain ⟨c, hc⟩ := pivot_exists_zpowZtwo_eq (η ^ 2 ^ t) b' (L + t) (by omega)
    (by rw [show L + t = t + L by omega]; exact hb') hξ
  refine ⟨c, ?_⟩
  rw [← hc, ← zpowZtwo_natCast isProP_two_unitsPadicInt η (2 ^ t),
    ← zpowZtwo_natCast isProP_two_unitsPadicInt
      (zpowZtwo isProP_two_unitsPadicInt η c) (2 ^ t),
    zpowZtwo_zpowZtwo, zpowZtwo_zpowZtwo]
  congr 1
  ring

/-- The `N` generator squared has exact level `α + 1`, so `nUnit α ^ 2^(m-1)` has exact level
`α + m - 1`, and every `ξ ≡ 1 mod 2^(m+α-1)` is a `2^(m-1)`-th power inside `imChiN α`.  The
factor `2` supplied by squaring is exactly what makes the `N` exponent come out at `2^(m-1)`
rather than `2^(m-2)`: that is the sign, accounted for. -/
theorem evenRow_pow_root_imChiN {α m : ℕ} (hα : 2 ≤ α) (hm : 2 ≤ m) {ξ : ℤ_[2]ˣ}
    (hξ : (2 : ℤ_[2]) ^ (m + (α - 1)) ∣ ((ξ : ℤ_[2])) - 1) :
    ∃ z : ℤ_[2]ˣ, z ∈ imChiN α ∧ z ^ 2 ^ (m - 1) = ξ := by
  obtain ⟨b, hb⟩ := nUnit_sq_sub_one (α := α) hα
  obtain ⟨c, hc⟩ := pivot_pow_surjective (nUnit α ^ 2) b (α + 1) (by omega) hb (m - 2)
    (by rw [show α + 1 + (m - 2) = m + (α - 1) by omega]; exact hξ)
  refine ⟨zpowZtwo isProP_two_unitsPadicInt (nUnit α) c,
    mem_topologicalClosure_of_zpowZtwo (nUnit α) c, ?_⟩
  rw [← hc, ← zpowZtwo_natCast isProP_two_unitsPadicInt (nUnit α) 2,
    zpowZtwo_zpowZtwo,
    ← zpowZtwo_natCast isProP_two_unitsPadicInt _ (2 ^ (m - 2)),
    ← zpowZtwo_natCast isProP_two_unitsPadicInt _ (2 ^ (m - 1)),
    zpowZtwo_zpowZtwo, zpowZtwo_zpowZtwo]
  congr 1
...
/-- **Sufficiency on the `N` branch**, at the same depth `s = α - 1` as the `M` branch, and
for every `α ≥ 2`. -/
theorem evenRow_deepSupply_imChiN {α : ℕ} (hα : 2 ≤ α)
    (hrange : MonoidHom.range chi.toMonoidHom = imChiN α)
    (hv : ∀ i, v i ∈ Set.range chi) :
    EvenRowDeepFibreLiftSupply (α - 1) v G chi := by
  refine evenRow_deepSupply_of_powRoots (fun m hm y hyker ↦ ?_) hv
  obtain ⟨z, hzmem, hzy⟩ := evenRow_pow_root_imChiN hα hm (evenSharp_mem_ker_iff.mp hyker)
  obtain ⟨w, hw⟩ := evenRow_mem_range_of_mem_of_range_eq hrange hzmem
  exact ⟨w, by rw [hw, hzy]⟩

/-- The `M` analogue by the same route: `mUnit α` has exact level `α`, so `mUnit α ^ 2^(m-1)`
has exact level `α + m - 1`. -/
theorem evenRow_pow_root_imChiM {α m : ℕ} (hα : 2 ≤ α) (hm : 2 ≤ m) {ξ : ℤ_[2]ˣ}
    (hξ : (2 : ℤ_[2]) ^ (m + (α - 1)) ∣ ((ξ : ℤ_[2])) - 1) :
    ∃ z : ℤ_[2]ˣ, z ∈ imChiM α ∧ z ^ 2 ^ (m - 1) = ξ := by
  obtain ⟨c, hc⟩ := pivot_pow_surjective (mUnit α) (mUnit α) α hα
    (mUnit_sub_one (by omega)) (m - 1)
    (by rw [show α + (m - 1) = m + (α - 1) by omega]; exact hξ)
  refine ⟨zpowZtwo isProP_two_unitsPadicInt (mUnit α) c, ?_, hc⟩
  exact Subgroup.topologicalClosure_mono
    (Subgroup.closure_mono (by simp : ({mUnit α} : Set ℤ_[2]ˣ) ⊆ {-1, mUnit α}))
    (mem_topologicalClosure_of_zpowZtwo (mUnit α) c)

/-- **Sufficiency on the `M` branch for every `α ≥ 2`**, strengthening
`evenRow_deepSupply_imChiM`, whose `α ≥ 3` came from the iterated-square-root route. -/
theorem evenRow_deepSupply_imChiM_of_two_le {α : ℕ} (hα : 2 ≤ α)
    (hrange : MonoidHom.range chi.toMonoidHom = imChiM α)
    (hv : ∀ i, v i ∈ Set.range chi) :
    EvenRowDeepFibreLiftSupply (α - 1) v G chi := by
  refine evenRow_deepSupply_of_powRoots (fun m hm y hyker ↦ ?_) hv
  obtain ⟨z, hzmem, hzy⟩ := evenRow_pow_root_imChiM hα hm (evenSharp_mem_ker_iff.mp hyker)
  obtain ⟨w, hw⟩ := evenRow_mem_range_of_mem_of_range_eq hrange hzmem
  exact ⟨w, by rw [hw, hzy]⟩

/-- **The `α = 2` verdict on the `N` branch: the committed seam holds.**  At `α = 2` the
depth is `s = 1`, which is the committed `RowExactLevelFibreLiftSupply`, so the statement
refuted for `α ≥ 3` is true at `α = 2` on the `N` branch too.  Unlike the `M` case this is
*not* because the character is surjective — `-1 ∉ imChiN 2` by `neg_one_notMem_imChiN` — so
this is a non-surjective character satisfying the committed seam, which is exactly what the
image-relative formulation of §1 of `StageAbstractionEvenRowSupply.lean` was built to
express. -/
theorem evenRow_rowSupply_imChiN_two
    (hrange : MonoidHom.range chi.toMonoidHom = imChiN 2)
    (hv : ∀ i, v i ∈ Set.range chi) :
    RowExactLevelFibreLiftSupply v G chi :=
  evenRow_rowSupply_of_deepSupply_one (evenRow_deepSupply_imChiN (le_refl 2) hrange hv)
...
#print axioms GQ2.Dyadic.EvenRowSupply.pivot_pow_surjective
#print axioms GQ2.Dyadic.EvenRowSupply.evenRow_pow_root_imChiN
#print axioms GQ2.Dyadic.EvenRowSupply.evenRow_deepSupply_imChiN
#print axioms GQ2.Dyadic.EvenRowSupply.evenRow_pow_root_imChiM
#print axioms GQ2.Dyadic.EvenRowSupply.evenRow_deepSupply_imChiM_of_two_le
#print axioms GQ2.Dyadic.EvenRowSupply.evenRow_rowSupply_imChiN_two
