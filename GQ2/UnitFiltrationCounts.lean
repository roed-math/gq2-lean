...
public import GQ2.UnitFiltrationTop
...
This file proves the **B13-3 component** of `dyadicUnitFiltration`.  For a finite extension `k/ℚ₂` it
...
@[simp] private theorem mem_maxIdeal {x : ↥(Osub k)} : x ∈ maxIdeal k ↔ ‖(x : ↥k)‖ < 1 := Iff.rfl
...
omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem mem_Osub {x : ↥k} : x ∈ Osub k ↔ ‖x‖ ≤ 1 := Iff.rfl
...
omit [FiniteDimensional ℚ_[2] ↥k] in
@[simp] private theorem normUnitToOsub_coe (u : ↥(normUnits k)) : (normUnitToOsub k u : ↥k) = (u.1 : ↥k) :=
  rfl
...
omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem gradeZeroHom_val (u : ↥(normUnits k)) :
    ((gradeZeroHom k u : (ResidueField k)ˣ) : ResidueField k)
      = Ideal.Quotient.mk (maxIdeal k) (normUnitToOsub k u) := rfl

omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem coe_normUnitToOsub_sub_one (u : ↥(normUnits k)) :
...
omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem gradeZeroHom_ker {π : ↥k} (hπlt : ‖π‖ < 1) (hπmax : ∀ y : ↥k, ‖y‖ < 1 → ‖y‖ ≤ ‖π‖) :
...
omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem gradeZeroHom_surjective : Function.Surjective (gradeZeroHom k) := by
  intro y
...
omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem card_gradeZero {π : ↥k} (hπlt : ‖π‖ < 1) (hπmax : ∀ y : ↥k, ‖y‖ < 1 → ‖y‖ ≤ ‖π‖) :
...
omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem depthToOsub_coe {π : ↥k} (hπne : π ≠ 0) (i : ℕ) (u : ↥(depthUnits k (π : ℚ̄₂) i)) :
...
omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem depthRes_one {π : ↥k} (hπne : π ≠ 0) (i : ℕ) :
...
omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem gradeIHom_apply {π : ↥k} (hπne : π ≠ 0) (hπlt : ‖π‖ < 1) {i : ℕ} (hi : 1 ≤ i)
...
omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem gradeIHom_ker {π : ↥k} (hπne : π ≠ 0) (hπlt : ‖π‖ < 1)
...
omit [FiniteDimensional ℚ_[2] ↥k] in
private theorem gradeIHom_surjective {π : ↥k} (hπne : π ≠ 0) (hπlt : ‖π‖ < 1) {i : ℕ} (hi : 1 ≤ i) :
...
/-- **The B13 capstone** (the deep-part proof discharge, B13-5): every finite extension `k/ℚ₂` inside `ℚ̄₂`
