...
*Status*: proved.  The terminal, `M`-stage, and `R`-stage lanes are all assembled; the latter
uses the proved `prop_8_9`.  Its literature-axiom footprint is reported by
`GQ2/AxiomLedger.lean`, with no `sorryAx`. -/
